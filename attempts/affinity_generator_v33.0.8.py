...
Version: affinity-rule-engine-v33.0.8 (Keyword Boost Stage 1)
"""
...
# --- Config Defaults ---
SCRIPT_VERSION = "affinity-rule-engine-v33.0.8 (Keyword Boost Stage 1)" # <-- Updated Version
CACHE_VERSION = "v20250421.affinity.33.0.8_default" # Default, overridden by config
...
def get_primary_label(uri: str, fallback: Optional[str] = None) -> str:
    ...
    if _taxonomy_concepts_cache and uri in _taxonomy_concepts_cache:
        ...
        for prop in ["skos:prefLabel", "rdfsLabel"]: # Add rdfsLabel check if potentially used
...
def build_keyword_label_index(taxonomy_concepts: Dict[str, Dict]) -> Optional[Dict[str, Set[str]]]:
    ...
    # Properties to index for keywords
...
# --- Loading Functions --- (load_affinity_config, load_taxonomy_concepts remain the same as v33.0.7)
...
# (Keep precompute_taxonomy_embeddings as refined previously, ensuring cache checks are robust)
...
# --- Embedding/KG Utils --- (get_concept_embedding, get_batch_embedding_similarity, get_kg_data remain same as v33.0.7)
...
# --- LLM Utils --- (get_openai_client, get_google_client, call_llm remain same as v33.0.7)
...
# --- Prompt Building --- (construct_llm_slotting_prompt, build_reprompt_prompt remain same as v33.0.7)
...
# --- <<< START MODIFIED prepare_evidence (v33.0.8) >>> ---
...
# --- <<< END MODIFIED prepare_evidence >>> ---

# --- Finalization Function --- (apply_rules_and_finalize remains same as v33.0.7)
    ...
    for theme_name, base_theme_data in base_themes.items():
        ...
        if assigned_uris_final and normalized_weight_for_theme > 0 and total_initial_score_in_theme > 0:
            ...
            for uri in assigned_uris_final: # Iterate over all URIs now associated with the theme
                if uri not in original_candidates_map_for_reprompt: continue # Skip if we don't have details
...
# --- Main Loop --- (generate_affinity_definitions_loop remains same as v33.0.7)
