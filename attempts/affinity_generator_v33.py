...
# -*- coding: utf-8 -*-
"""
Generate affinity definitions for travel concepts (v33.0 - Simplified LLM Slotting)
- Reverted to simpler prompt (no CoT) and disabled hint boosting.
...
- Includes fixes for weight calculation, config interpretation, and variable passing.

Version: affinity-rule-engine-v33.0.7 (Corrected + Simplified RDF Load)
"""
...
import pickle
import re
import sys # <-- CORRECT: Added import sys
import time
import traceback
...
# --- Config Defaults ---
SCRIPT_VERSION = "affinity-rule-engine-v33.0.7 (Corrected + Simplified RDF Load)" # <-- Updated Version
CACHE_VERSION = "v20250421.affinity.33.0.7_default" # Default, overridden by config
...
def normalize_concept(concept: Optional[str]) -> str:
    ...
    try:
        ...
        return norm
    except Exception as e: logger.debug(f"Normalize regex failed: {e}"); return concept.lower().strip()
...
def get_primary_label(uri: str, fallback: Optional[str] = None) -> str:
    ...
    if _taxonomy_concepts_cache and uri in _taxonomy_concepts_cache:
        ...
        for prop in ["skos:prefLabel", "rdfsLabel"]:
            val_list = details.get(prop, [])
            if isinstance(val_list, list) and val_list and val_list[0]: return str(val_list[0]).strip()
        alt_labels = details.get("skos:altLabel", [])
        if isinstance(alt_labels, list) and alt_labels and alt_labels[0]: return str(alt_labels[0]).strip()
        if fallback is None:
            ...
            if isinstance(definitions, list) and definitions and definitions[0]:
                ...
                if definition_text: label = definition_text[:60] + ("..." if len(definition_text) > 60 else "")
    if label is None or label == fallback:
        try:
            ...
            if parsed_label and parsed_label != uri:
                parsed_label = re.sub(r'(?<=[a-z0-9])([A-Z])', r' \1', parsed_label).replace('_', ' ').replace('-', ' ')
    ...
    if _taxonomy_concepts_cache and uri in _taxonomy_concepts_cache:
        ...
        if isinstance(type_uris, list):
            for type_uri_str in type_uris:
                label = get_primary_label(type_uri_str)
                if label != type_uri_str: type_labels.add(label)
...
# --- <<< Simplified load_taxonomy_concepts without LXML fallback >>> ---
def load_taxonomy_concepts(taxonomy_dir: str, cache_file: str, rebuild_cache: bool, current_cache_version: str, debug_mode: bool) -> Optional[Dict[str, Dict]]:
    ...
    if not cache_valid:
        ...
        try:
            ...
            props = {"skos:prefLabel": SKOS.prefLabel, "skos:altLabel": SKOS.altLabel, "rdfsLabel": RDFS.label, "skos:definition": SKOS.definition, "skos:scopeNote": SKOS.scopeNote, "type": RDF.type}
            ...
            processed_concepts = {uri: {k: sorted(list(set(v))) for k, v in data.items() if v} for uri, data in extracted_data.items() if data}
...
# --- <<< END Simplified load_taxonomy_concepts >>> ---

# (Keep precompute_taxonomy_embeddings as refined previously)
def precompute_taxonomy_embeddings(
    ...
    if not rebuild and os.path.exists(cache_file):
        ...
        try:
            ...
            if (cached.get("cache_version") == CACHE_VERSION and isinstance(cached.get("primary_embeddings"), dict) and isinstance(cached.get("uris_list"), list)):
                p_embs = cached["primary_embeddings"]; u_list = cached["uris_list"]
                sbert_dim = sbert_model.get_sentence_embedding_dimension()
                if not u_list or (u_list and u_list[0] in p_embs and p_embs.get(u_list[0]) is not None and p_embs[u_list[0]].shape[0] == sbert_dim): # Check key exists too
    ...
    if not cache_valid:
        ...
        texts_to_embed_map = defaultdict(list); properties_for_embedding = ["skos:prefLabel", "skos:altLabel", "rdfsLabel", "skos:definition"]
        ...
        for uri in tqdm(all_uris_processed, desc="Gathering Texts", disable=disable_tqdm):
            ...
            for prop_key in properties_for_embedding:
                values = data.get(prop_key, []); value_list = values if isinstance(values, list) else [values]
                for text_value in value_list:
                    if text_value and isinstance(text_value, str):
                        normalized_text = normalize_concept(text_value.strip())
        ...
        embedding_priority_order = ["skos:prefLabel", "rdfsLabel", "skos:altLabel", "skos:definition"]; num_primary_found = 0
        ...
        if not final_uris_with_embeddings: logger.warning("No primary embeddings determined."); _taxonomy_embeddings_cache = ({}, []); return None
...
# (Keep embedding utilities: get_concept_embedding, get_batch_embedding_similarity)
...
# (Keep KG data lookup: get_kg_data)
def get_kg_data(uris: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    for uri in uris:
        ...
        if uri in _taxonomy_concepts_cache:
            details = _taxonomy_concepts_cache[uri]; pref_label = get_primary_label(uri, fallback=uri)
            results[uri] = { "uri": uri, "prefLabel": pref_label, "skos:prefLabel": pref_label,
                             "skos:altLabel": details.get("skos:altLabel", []),
                             "skos:definition": (details.get("skos:definition", []) or [None])[0],
                             "skos:scopeNote": (details.get("skos:scopeNote", []) or [None])[0],
                             "type_labels": get_concept_type_labels(uri) }
    return results

# (Keep LLM clients: get_openai_client, get_google_client)
...
# (Keep LLM call function: call_llm)
def call_llm(system_prompt: str, user_prompt: str, model_name: str, timeout: int, max_retries: int, temperature: float, provider: str) -> Dict[str, Any]:
    ...
    return llm_result # Should technically be unreachable if max_retries > 0

# (Keep prompt builders: construct_llm_slotting_prompt, build_reprompt_prompt)
...
# (Keep Stage 1: prepare_evidence as refined previously)
def prepare_evidence(input_concept: str, concept_embedding: Optional[np.ndarray],
    ...
    normalized_concept = normalize_concept(input_concept); logger.info(f"Starting evidence prep: {normalized_concept} (Hints Disabled)")
    stage1_cfg = config.get('STAGE1_CONFIG', {}); initial_pool_size = int(stage1_cfg.get('INITIAL_CANDIDATE_POOL_SIZE', 150)); max_candidates_for_llm = int(stage1_cfg.get('MAX_CANDIDATES_FOR_LLM', 75)); min_similarity = float(stage1_cfg.get('EVIDENCE_MIN_SIMILARITY', 0.3))
    candidates_for_llm_details, original_candidates_map, anchor_candidate, candidate_evidence_scores = [], {}, None, {}
    if concept_embedding is None: logger.error("No embedding."); return candidates_for_llm_details, original_candidates_map, anchor_candidate, candidate_evidence_scores
    ...
    if not filtered_candidates: logger.warning(f"No candidates >= {min_similarity}."); return candidates_for_llm_details, original_candidates_map, anchor_candidate, candidate_evidence_scores
    sorted_uris = sorted(filtered_candidates, key=filtered_candidates.get, reverse=True); initial_pool_uris = sorted_uris[:initial_pool_size]
    logger.info(f"Found {len(initial_pool_uris)} candidates in initial pool.")
    initial_pool_details_map = get_kg_data(initial_pool_uris); candidates_with_details = []
    for uri in initial_pool_uris:
        if uri in initial_pool_details_map:
            details = initial_pool_details_map[uri].copy(); details['similarity_score'] = filtered_candidates.get(uri, 0.0)
            candidates_with_details.append(details)
    if not candidates_with_details: logger.warning("No KG details."); return candidates_for_llm_details, original_candidates_map, anchor_candidate, candidate_evidence_scores
    candidates_with_details.sort(key=lambda x: x['similarity_score'], reverse=True)
    anchor_candidate = candidates_with_details[0] if candidates_with_details else None; logger.info("Hint boosting DISABLED.")
    selected_uris = set()
    if anchor_candidate: candidates_for_llm_details.append(anchor_candidate); selected_uris.add(anchor_candidate['uri'])
    fill_needed = max_candidates_for_llm - len(candidates_for_llm_details)
    if fill_needed > 0:
        added = 0
        for cand in candidates_with_details:
             if cand['uri'] not in selected_uris: candidates_for_llm_details.append(cand); selected_uris.add(cand['uri']); added += 1
             if added >= fill_needed: break
    logger.info(f"Selected {len(candidates_for_llm_details)} candidates for LLM.")
    original_candidates_map = {c['uri']: c for c in candidates_for_llm_details}
    return candidates_for_llm_details, original_candidates_map, anchor_candidate, candidate_evidence_scores


# --- <<< START CORRECTED apply_rules_and_finalize (v33.0.7) >>> ---
def apply_rules_and_finalize(
    ...
    v33.0.7: Incorporates fixes from v33.0.6 (KeyError, rule name check, must_not_have reading, weight calc).
             Uses the full llm_call_result.
    """
    ...
    # --- <<< FIX: Check success status *before* trying to access response >>> ---
    ...
    for theme_name in base_themes.keys():
        ...
        # --- <<< FIX: Check for 'Must have 1' from config >>> ---
    ...
    for theme_name, base_theme_data in base_themes.items():
        ...
        if assigned_uris_final and normalized_weight_for_theme > 0 and total_initial_score_in_theme > 0:
            ...
            for uri in assigned_uris_final: # Iterate over all URIs now associated with the theme
                ...
                logger.debug(f"  [{uri}] Score:{initial_score:.4f}, Prop:{proportion:.4f}, FinalW:{final_attribute_weight:.6f}")
                ...
                if final_attribute_weight >= theme_attribute_min_weight:
                     ...
                     if is_fallback_add:
                         ...
                         logger.debug(f"    -> Attrib added (Initial): {final_attr}")
    ...
    # --- <<< FIX: Apply overrides to the CORRECT travel_category object >>> ---
    ...
    # --- <<< FIX: Read must_not_have correctly >>> ---
...
# --- <<< END CORRECTED apply_rules_and_finalize >>> ---


# --- <<< START CORRECTED generate_affinity_definitions_loop (v33.0.7) >>> ---
    ...
    for concept in tqdm(concepts_subset, desc="Processing Concepts", disable=disable_tqdm):
        ...
        try:
            ...
            if anchor_candidate and anchor_candidate.get('uri'):
                 # Fetch full details for the anchor if possible
            ...
            if not candidates_for_llm_details:
                ...
                system_prompt, user_prompt = construct_llm_slotting_prompt(concept, active_themes_prompt_list, candidates_for_llm_details, args)
            ...
            # --- <<< FIX: Pass the correct variable (llm_call_result) >>> ---
            stage2_output_data = apply_rules_and_finalize(
                 concept,
                 llm_call_result, # Pass the *entire* result dict from call_llm
                 config,
                 affinity_definition["travel_category"], # Pass the category initialized earlier
                 anchor_candidate,
                 original_candidates_map, # Map of candidates sent to LLM (or would have been sent)
...
# --- <<< END CORRECTED generate_affinity_definitions_loop >>> ---
...
def main():
    ...
    parser.add_argument("-i", "--input_concepts_file", type=str, required=True, help="Path to a JSON file containing a list of concepts to process.")
