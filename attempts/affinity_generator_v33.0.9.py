...
Version: affinity-rule-engine-v33.0.9 (Abstract Concept Prompting)
"""
...
# --- Config Defaults ---
SCRIPT_VERSION = "affinity-rule-engine-v33.0.9 (Abstract Concept Prompting)" # <-- Updated Version
CACHE_VERSION = "v20250421.affinity.33.0.9_default" # Default, overridden by config
...
# --- <<< END MODIFIED signature >>> ---
    ...
    system_prompt_base = """You are an expert travel taxonomist and semantic analyst tasked with defining travel concepts based on related evidence.
...
Do not include explanations or conversational text outside the JSON object."""
    ...
    # Conditionally add instructions for abstract concepts
    if is_abstract_concept:
        abstract_instruction = """
IMPORTANT: The input concept represents an abstract travel style or package deal (e.g., 'All Inclusive', 'Budget Travel'). When assigning themes, consider the *implied* characteristics and common associations. For example, 'All Inclusive' often implies included meals, drinks (relevant to Indoor Amenities), and potentially specific on-site Activities or entertainment."""
        system_prompt = system_prompt_base + abstract_instruction
    else:
