...
# -*- coding: utf-8 -*-
"""
Generate affinity definitions for travel concepts using combined scoring (v33.0.22 - Cleaned Logs).
...
SCRIPT_VERSION = "affinity-rule-engine-v33.0.22 (Cleaned Logs)"
DEFAULT_CACHE_VERSION = "v20250420.affinity.33.0.19" # Base version for Run 13
...
def build_keyword_index(
    ...
    """Builds or loads TF-IDF index and simple label index. Reduced logging."""
    global _tfidf_vectorizer, _tfidf_matrix, _tfidf_corpus_uris, _keyword_label_index
    # logger.info("Building/Loading keyword indexes (TF-IDF & Simple Label)...") # Removed Info
    ...
    uris_to_log_tfidf_doc = { # Keep this logging for specific URIs
        "urn:expediagroup:taxonomies:acs:#1f3da634-0df6-4498-a8d9-603f895c8f3f", # AirConditioning:Unavailable
        "urn:expediagroup:taxonomies:acsPCS:#AirConditioning",                 # AIR_CONDITIONING (PCS)
        "urn:expediagroup:taxonomies:lcm:#34528832-c2d8-312a-b52f-b27c483e5ec1", # All-inclusive resort (LCM)
        "urn:expediagroup:taxonomies:lcm:#b2fa2cb3-9226-305e-8943-02d82ba90975", # Resort (LCM)
        "urn:expediagroup:taxonomies:acsEnumerations:#dcecb8c7-bacf-4d04-b90f-95e4654ffa9f", # Allinclusive (Enum)
    }

    if use_tfidf:
        logger.info("TF-IDF Indexing enabled.") # Keep Enabled/Disabled info
        ...
        if "ngram_range" in tfidf_params and tfidf_params["ngram_range"] != (1,1):
             logger.warning(f"Run 13 requires UNIGRAMS. Found ngram_range={tfidf_params['ngram_range']} in config. Removing it.")
             del tfidf_params["ngram_range"]
        logger.info(f"Using TF-IDF Vectorizer Params: {tfidf_params}") # Keep params info
        ...
        if not rebuild_cache:
            # logger.info(f"Attempting load TF-IDF cache (Version: {cache_version})...") # Removed Info
            ...
            if cached_matrix is not None and cached_vec is not None and isinstance(cached_uris, list):
                 ...
                 if loaded_params_ok and hasattr(cached_matrix, 'shape') and cached_matrix.shape[0] == len(cached_uris):
                     ...
                     logger.info(f"TF-IDF loaded from cache ({len(corpus_uris)} URIs).") # Keep final status
                 elif loaded_params_ok:
                     logger.warning("TF-IDF cache dim/URI mismatch. Rebuilding.") # Keep warning
                 else:
                    # logger.info("TF-IDF cache param mismatch. Rebuilding.") # Redundant due to warning above
                    pass
            else:
                 logger.info("TF-IDF cache incomplete/invalid. Rebuilding.") # Keep info


        if not cache_valid:
            logger.info("Rebuilding TF-IDF index...") # Keep info
            try:
                docs, doc_uris = [], []
                # logger.info("Preparing docs for TF-IDF...") # Removed Info
                for uri, data in tqdm(sorted(taxonomy_concepts_cache.items()), desc="Prepare TF-IDF Docs"):
                    ...
                    for k in props:
                        vals = data.get(k, [])
                        texts.extend(str(v) for v in (vals if isinstance(vals, list) else [vals]) if v)
                    raw_doc_text = " ".join(texts)
                    norm_doc = normalize_concept(raw_doc_text)

                    # Log document content for specific URIs
                    if uri in uris_to_log_tfidf_doc:
                        label = get_primary_label(uri, taxonomy_concepts_cache, fallback=uri)
                        logger.debug(f"[TF-IDF DOC LOG] URI: {uri} Label: '{label}' Raw: '{raw_doc_text}' Norm: '{norm_doc}'") # Keep Debug

                    if norm_doc: docs.append(norm_doc); doc_uris.append(uri)

                if not docs: logger.warning("No documents for TF-IDF build.") # Keep Warning
                else:
                    # logger.info(f"Fitting TF-IDF Vectorizer on {len(docs)} documents...") # Removed Info
                    vectorizer = TfidfVectorizer(**tfidf_params)
                    tfidf_matrix = vectorizer.fit_transform(docs)
                    corpus_uris = doc_uris
                    # logger.info(f"TF-IDF Matrix: {tfidf_matrix.shape}, Corpus URIs: {len(corpus_uris)}") # Removed Info (part of debug block)
                    ...
                    logger.info("TF-IDF components rebuilt and saved.") # Keep final status
            except Exception as e: logger.error(f"TF-IDF build error: {e}", exc_info=True); tfidf_matrix, vectorizer, corpus_uris = None, None, None # Keep Error

        # Keep TF-IDF Diagnostics Block
    ...
    else: logger.info("TF-IDF Indexing disabled or sklearn unavailable.") # Keep Info

    # logger.info("Building/Loading simple keyword label index...") # Removed Info
    label_index = build_keyword_label_index(taxonomy_concepts_cache)
    if label_index is None: logger.warning("Failed simple label index build.") # Keep Warning
    else: logger.info(f"Simple label index ready ({len(label_index)} keywords).") # Keep final status
...
def get_candidate_concepts_keyword(
    ...
    # start_time = time.time() # Removed timing
    logger.debug(f"TF-IDF search query terms (first 10): {query_texts[:10]}{'...' if len(query_texts) > 10 else ''}") # Keep debug
    try:
        query_matrix = vectorizer.transform(query_texts)
        logger.debug(f"Query Matrix shape: {query_matrix.shape}, Non-zero elements: {query_matrix.nnz}") # Keep debug
        if query_matrix.nnz == 0:
            logger.warning(f"TF-IDF Query resulted in ZERO non-zero elements for query: {query_texts}") # Keep warning
            # Optional debug logging for missing terms (keep this)
        ...
        if query_matrix.shape[1] != tfidf_matrix.shape[1]: logger.error(f"TF-IDF vocab mismatch! Query:{query_matrix.shape[1]} vs Corpus:{tfidf_matrix.shape[1]}"); return [] # Keep error
        similarities = cosine_similarity(query_matrix, tfidf_matrix)
        # logger.debug(f"Similarities Shape: {similarities.shape}") # Removed Debug
        ...
        if agg_scores.size == 0: logger.debug("Aggregated TF-IDF scores empty."); return [] # Keep debug
        logger.debug(f"Agg Scores Shape: {agg_scores.shape}, Min: {np.min(agg_scores):.4f}, Max: {np.max(agg_scores):.4f}") # Keep debug
        if np.max(agg_scores) < min_score: logger.debug(f"Max TF-IDF score ({np.max(agg_scores):.4f}) < min_score ({min_score}). No candidates pass."); return [] # Keep debug

        num_cands = len(agg_scores); top_n = min(top_n, num_cands)
        if top_n <= 0: return []
        if top_n < num_cands // 2 : # Use partitioning for efficiency if top_n is small
        ...
        # logger.debug(f"Top {len(indices)} indices found. Top 5 Scores: {agg_scores[indices[:5]] if len(indices)>0 else 'N/A'}") # Removed Debug
        ...
        if filtered > 0: logger.debug(f"Filtered {filtered} TF-IDF candidates with score < {min_score}.") # Keep Debug
        # logger.info(f"TF-IDF search found {len(candidates)} candidates >= {min_score} ({time.time() - start_time:.2f}s).") # Removed Info
        return candidates
    except Exception as e: logger.error(f"TF-IDF search error: {e}", exc_info=True); return [] # Keep Error
...
def get_openai_client()  -> Optional[OpenAI_Type]:
    ...
    if _openai_client is None:
        try:
            ...
            if not api_key: logger.warning("OPENAI_API_KEY env var not set."); return None # Keep Warning
            ...
            # logger.info("OpenAI client initialized.") # Removed Info
        except Exception as e: logger.error(f"OpenAI client init failed: {e}"); return None # Keep Error
...
def get_google_client() -> Optional[Any]:
    ...
    if _google_client is None:
        try:
            ...
            if not api_key: logger.warning("GOOGLE_API_KEY env var not set."); return None # Keep Warning
            genai.configure(api_key=api_key)
            # logger.info("Google AI client configured.") # Removed Info
            _google_client = genai
        except Exception as e: logger.error(f"Google AI config failed: {e}"); return None # Keep Error
...
def call_llm(system_prompt: str, user_prompt: str, model_name: str, timeout: int, max_retries: int, temperature: float, provider: str) -> Dict[str, Any]:
    ...
    # logger.info(f"LLM call: {provider}, Model: {model_name}") # Removed Info
    if not model_name: logger.error("LLM model name missing."); return {"success": False, "error": "LLM model missing"} # Keep Error
    ...
    if not client: result["error"] = f"{provider} client unavailable."; logger.error(result["error"]); return result # Keep Error
    ...
    for attempt in range(max_retries + 1):
        ...
        logger.info(f"{provider} Attempt {attempt + 1}/{max_retries + 1}") # Keep Info for retries
        start = time.time()
        try:
            content = None
            if provider == "openai":
                ...
                if resp.prompt_feedback and resp.prompt_feedback.block_reason:
                    ...
                    logger.warning(f"Gemini blocked prompt/response. Reason: {block_reason}. System: '{system_prompt[:100]}...', User: '{user_prompt[:100]}...'") # Keep Warning
                    result["error"] = f"Blocked: {block_reason}"; return result
                if resp.candidates: content = resp.text
                else: logger.warning(f"Gemini response received but has no candidates."); content = None # Keep Warning
            else: result["error"] = f"Unsupported provider: {provider}"; logger.error(result["error"]); return result # Keep Error

            if content:
                try:
                    # Attempt to parse JSON, handling potential markdown fences
                    json_match = re.search(r"```json\s*(\{.*?\})\s*```", content, re.DOTALL | re.IGNORECASE)
                    if json_match:
                        cleaned = json_match.group(1)
                    elif '{' in content and '}' in content:
                         cleaned = content[content.find('{'):content.rfind('}')+1] # Fallback: Extract between first { and last }
                    else:
                         cleaned = content.strip().strip('`') # Fallback: Strip whitespace and backticks

                    output = json.loads(cleaned)
                    if isinstance(output, dict):
                        result["success"] = True; result["response"] = output
                        # logger.info(f"{provider} OK ({time.time() - start:.2f}s).") # Removed Info
                        return result
                    else: logger.error(f"{provider} response parsed but is not a dict. Type: {type(output)}. Raw: {content[:200]}...") # Keep Error
                except json.JSONDecodeError as e: logger.error(f"{provider} JSON parse error: {e}. Raw content start: {content[:500]}..."); result["error"] = f"JSON Parse Error: {e}" # Keep Error
                except Exception as e: logger.error(f"{provider} response processing error: {e}. Raw content start: {content[:500]}..."); result["error"] = f"Response Processing Error: {e}" # Keep Error
            else: logger.warning(f"{provider} response content was empty."); result["error"] = "Empty response from LLM" # Keep Warning

        except (APITimeoutError, APIConnectionError, RateLimitError) as e: logger.warning(f"{provider} API Error during call: {type(e).__name__}"); result["error"] = f"{type(e).__name__}" # Keep Warning
        except Exception as e: logger.error(f"{provider} Call Error: {e}", exc_info=True); result["error"] = str(e); return result # Keep Error
        ...
        if not should_retry: logger.error(f"LLM call failed permanently for {provider} after {attempt + 1} attempts. Last error: {result.get('error')}") # Keep Error
        if not result.get("error"): result["error"] = "Failed after retries." # Fallback error message
        if should_retry:
            wait = delay * (2**attempt) + np.random.uniform(0, delay*0.5); logger.info(f"Retrying in {wait:.2f}s... (Error: {result.get('error')})"); time.sleep(wait) # Keep Info for retries
        else:
             return result # Return immediately if not retrying
    return result # Return after final attempt if retries exhausted

# --- Prompt Building Functions ---
# (No logging in these functions, no changes needed)
...
def expand_keywords_with_llm(concept_label: str, config: Dict, args: argparse.Namespace) -> List[str]:
    ...
    # logger.info(f"[{concept_label}] LLM keyword expansion triggered (Temp: {temp})...") # Removed Info
    ...
    raw_phrases = []
    exp_diag = {} # Initialize diagnostic dict
    ...
    if result and result.get("success"):
        ...
        if isinstance(raw_phrases, list):
            ...
            logger.info(f"[{concept_label}] LLM generated {len(final_keyword_terms)} unique keyword terms from {len(raw_phrases)} phrases.") # Keep Info Summary
        else:
            logger.error(f"[{concept_label}] LLM expansion response 'keywords' field was not a list.") # Keep Error
            exp_diag["error"] = "LLM response format invalid"
    else:
        err_msg = result.get('error', 'N/A') if result else 'No result object'
        logger.warning(f"[{concept_label}] LLM keyword expansion failed: {err_msg}") # Keep Warning
...
# --- Updated Stage 1: Evidence Preparation ---
# --- Updated Stage 1: Evidence Preparation ---
# --- v33.0.24 - Run 15: Dynamic Alpha, Specific Aircon Filter, No Expansion, Manual Split, DampThresh=0.35, DampFactor=0.15 ---
def prepare_evidence(
    ...
    """
    Prepares evidence candidates using combined score with DYNAMIC ALPHA adjustment,
    ...
    v33.0.24 - Testing Dynamic Alpha (Thresh=0.55, Factor=0.1). Retains Specific Aircon Filter, NoExp, Manual Split, DampThresh=0.35, DampFactor=0.15.
    ...
    alpha = float(config.get("COMBINED_SCORE_ALPHA", COMBINED_SCORE_ALPHA)) # Default alpha
    ...
    logger.info(f"[{normalized_concept}] Implementing DYNAMIC ALPHA (SBERT<0.55 & TFIDF>0.5 => Alpha=0.1)") # Log Dynamic Alpha logic
    ...
    # --- Keyword Preparation ---
    # ... (Keyword prep logic remains the same) ...
    ...
    # --- TF-IDF Search ---
    # ... (TF-IDF search logic remains the same) ...
    ...
    # ... (SBERT calculation logic remains the same) ...
    ...
    if uris_to_filter_for_concept:
        ...
        for uri_to_filter in uris_to_filter_for_concept:
            if uri_to_filter in all_uris_set:
                ...
                all_uris_set.remove(uri_to_filter)
                removed_count += 1
            elif args.debug:
                 logger.debug(f"Specific filter URI {uri_to_filter} not found in candidate set for '{normalized_concept}'.")
        if removed_count > 0:
             logger.info(f"Removed {removed_count} URIs via concept-specific filter for '{normalized_concept}'.")
    ...
    if initial_unique_count != unique_candidates_before_ranking:
        logger.debug(f"[{normalized_concept}] Unique candidates changed from {initial_unique_count} to {unique_candidates_before_ranking} after seeding/filtering.")
    ...
    dampened_count = 0
    dynamic_alpha_applied_count = 0 # Counter for dynamic alpha

    # --- START: Dynamic Alpha Parameters ---
    dynamic_alpha_sbert_threshold = 0.55 # SBERT threshold below which dynamic alpha might apply
    dynamic_alpha_tfidf_threshold = 0.50 # TF-IDF threshold above which dynamic alpha might apply
    dynamic_alpha_factor = 0.10          # The reduced alpha value to use
    # --- END: Dynamic Alpha Parameters ---
    ...
    for uri in all_uris_list:
        ...
        k_score = kw_scores.get(uri, 0.0) # Raw TF-IDF score
        pref_label_for_log = all_details[uri].get('prefLabel', 'N/A')

        is_aikido_seed_uri = (normalized_concept == 'aikido' and uri in seed_uris)
        calculated_seed_sbert = False

        # Fetch SBERT score if seed was added and not in original sim_scores (Aikido specific)
        if uri not in sim_scores and is_aikido_seed_uri:
            # ... (Aikido SBERT calculation unchanged) ...
            if uri in primary_embeddings and concept_embedding is not None:
                 ...
                 sim_scores[uri] = s_score
                 calculated_seed_sbert = True
                 logger.debug(f"[Aikido Seed] Calculated SBERT for seeded URI {uri} ('{pref_label_for_log}'): {s_score:.6f}")
            else:
                 logger.warning(f"[{normalized_concept}] Could not calculate SBERT for seeded URI {uri}")
                 s_score = 0.0
        ...
        # ** Absolute SBERT Filter ** (Uses potentially boosted score)
        ...
        # *** Conditional TF-IDF Dampening *** (Uses potentially boosted score for condition)
        ...
        effective_alpha = alpha # Start with default alpha (e.g., 0.6)
        alpha_was_reduced = False
        # Check if SBERT is only moderate AND TF-IDF is reasonably high
        if boosted_s_score_for_calc < dynamic_alpha_sbert_threshold and k_score > dynamic_alpha_tfidf_threshold:
            effective_alpha = dynamic_alpha_factor # Reduce alpha significantly (e.g., to 0.1)
            alpha_was_reduced = True
            dynamic_alpha_applied_count += 1
            if args.debug:
                 logger.debug(f"DYNAMIC ALPHA: Reducing alpha for {uri} ('{pref_label_for_log}') to {effective_alpha:.2f} "
                              f"(SBERT={boosted_s_score_for_calc:.4f} < {dynamic_alpha_sbert_threshold:.2f} AND TFIDF={k_score:.4f} > {dynamic_alpha_tfidf_threshold:.2f})")

        # Normalize scores (Use the dampened k_score for norm_k)
        ...
        scored_list.append({
            ...
            "alpha_reduced": alpha_was_reduced, # Flag if dynamic alpha was applied
            "effective_alpha": effective_alpha, # Store the alpha used
    ...
    logger.info(f"[{normalized_concept}] Applied DYNAMIC ALPHA reduction (to {dynamic_alpha_factor:.2f}) for {dynamic_alpha_applied_count} candidates (SBERT<{dynamic_alpha_sbert_threshold:.2f} & TFIDF>{dynamic_alpha_tfidf_threshold:.2f}).") # Log dynamic alpha summary
    ...
    # Debug logging for Top 5 (Updated to show effective alpha)
    if args.debug:
        logger.debug(f"--- Top 5 Candidates for '{normalized_concept}' AFTER Sorting (DefaultAlpha={alpha}, DynamicAlphaFactor={dynamic_alpha_factor}, DampThresh={dampening_sbert_threshold}, DampFactor={dampening_factor}) ---") # Updated Log
        for i, c in enumerate(scored_list[:5]):
            ...
            alpha_info = f"(Alpha: {c.get('effective_alpha', alpha):.2f})" # Show alpha used
            label = c.get('details', {}).get('prefLabel', 'N/A')
            logger.debug(f"{i+1}. URI: {c.get('uri', 'MISSING')} Label: '{label}' "
    ...
    logger.info(f"[{normalized_concept}] Selected top {len(selected)}/{len(scored_list)} candidates based on combined score.")
         ...
         if uri and details:
              orig_map_entry = {
                  **details,
                  ...
                  "effective_alpha": c.get("effective_alpha", alpha) # Add effective alpha here too
              }
    ...
    # Anchor Logging (Updated to include effective alpha)
    if anchor:
        ...
        alpha_log = f"(Alpha: {anchor_data.get('effective_alpha', alpha):.2f})"
        ...
        if anchor_uri_from_data:
             logger.warning(f"    Reason: URI '{anchor_uri_from_data}' (from top candidate) was not found in orig_map keys.")
        else:
...
def apply_rules_and_finalize(
    ...
    candidate_evidence_scores: Dict[str, float], # Note: candidate_evidence_scores seems unused here, replaced by orig_map
    ...
    # logger.info(f"Starting finalization for: {norm_concept}") # Removed Info
    ...
    if llm_call_result and llm_call_result.get("success"):
        ...
        if validated is not None: llm_assigns = validated; logger.debug(f"[{norm_concept}] Validated LLM assignments.") # Keep debug
        else: logger.warning(f"[{norm_concept}] LLM validation failed."); output["diagnostics"]["stage2"]["error"] = diag_val.get("error", "LLM Validation Failed") # Keep Warning
    elif llm_call_result: logger.warning(f"[{norm_concept}] LLM slotting call unsuccessful: {llm_call_result.get('error')}"); output["diagnostics"]["stage2"]["error"] = f"LLM Call Failed: {llm_call_result.get('error')}" # Keep Warning
    # else: logger.debug(f"[{norm_concept}] No LLM result for slotting.") # Removed Debug
    ...
    for name in base_themes.keys():
        ...
        if rule == "Must have 1" and not theme_map.get(name):
             logger.warning(f"[{norm_concept}] Rule FAILED: Mandatory theme '{name}' has no assigned concepts.") # Keep Warning
    ...
    if failed_rules and original_candidates_map_for_reprompt and args.llm_provider != "none":
        logger.info(f"[{norm_concept}] Attempting LLM fallback for {len(failed_rules)} mandatory themes: {list(failed_rules.keys())}") # Keep Info
        for name in list(failed_rules.keys()):
            reprompt_diag["attempts"] += 1; base_cfg = base_themes.get(name)
            if not base_cfg: logger.error(f"[{norm_concept}] No config found for fallback theme '{name}'."); reprompt_diag["failures"] += 1; continue # Keep Error
            ...
            if fb_result and fb_result.get("success"):
                ...
                if new_uris:
                    logger.info(f"[{norm_concept}] Fallback SUCCESS for '{name}': Assigned {len(new_uris)} URIs.") # Keep Info
                    ...
                    logger.debug(f"[{norm_concept}] Added {added} unique URIs via fallback for '{name}'.") # Keep Debug
                    if name in theme_diag: theme_diag[name].update({"status": "Passed (Fallback)", "rule_failed": False})
                else: logger.warning(f"[{norm_concept}] Fallback LLM for '{name}' returned successfully but assigned 0 candidates."); reprompt_diag["failures"] += 1; theme_diag[name]["status"] = "Failed (Fallback - No Assigns)" # Keep Warning
            else: err = fb_result.get("error", "Unknown") if fb_result else "None"; logger.error(f"[{norm_concept}] Fallback LLM call failed for theme '{name}'. Error: {err}"); reprompt_diag["failures"] += 1; theme_diag[name]["status"] = "Failed (Fallback - API Error)" # Keep Error
    elif failed_rules: logger.warning(f"[{norm_concept}] Cannot attempt fallback for failed rules ({list(failed_rules.keys())}), LLM provider is 'none'.") # Keep Warning
    ...
    for name, base_data in base_themes.items():
        ...
        logger.debug(f"[{norm_concept}][{name}] URIs:{len(uris)}, Combined Score Sum:{total_score:.4f}, Theme Weight:{theme_w:.4f}") # Keep Debug

        theme_attrs = []
        if uris and theme_w > 0:
            n_uris = len(uris)
            if total_score < 1e-9 and n_uris > 0: # Handle case where all assigned concepts have zero combined score
                logger.warning(f"[{norm_concept}][{name}] Zero combined scores for assigned candidates. Using equal weight distribution for {n_uris} URIs.") # Keep Warning
                eq_w = (theme_w / n_uris)
                if eq_w >= min_weight:
                    for u in uris:
                ...
                else: logger.warning(f"[{norm_concept}][{name}] Equal weight {eq_w:.6f} < {min_weight}, skipping attributes for this theme.") # Keep Warning
            elif total_score > 1e-9:
                 for u in uris:
                      if u not in original_candidates_map_for_reprompt: continue
                      d = original_candidates_map_for_reprompt[u]
                      s = scores.get(u, 0.0)
                      prop = s / total_score if total_score > 0 else 0 # Proportional weight based on combined score
                      attr_w = theme_w * prop
                      if attr_w >= min_weight:
                           ...
                           if is_fb: attr["comment"] = "Fallback Assignment"
                           theme_attrs.append(attr); all_final_attrs.append(attr)
                      # else: logger.debug(f"Attribute {u} weight {attr_w:.6f} below threshold {min_weight} for theme {name}") # Removed Debug
    ...
    for attr in all_final_attrs:
        ...
        # Keep the attribute with the highest weight if it appears in multiple themes
        stored_weight = unique_attrs.get(uri, {}).get("concept_weight", -1.0)
        if uri not in unique_attrs or current_weight > stored_weight:
             # Don't copy 'comment' (like Fallback) into top attributes list
    ...
    else: logger.error(f"[{norm_concept}] Travel category structure invalid during override application?"); output["travel_category"] = {"uri": None, "name": norm_concept, "type": overrides.get("category_type", "Uncategorized"), "exclusionary_concepts": overrides.get("exclusionary_concepts", [])} # Keep Error
    ...
    # logger.info(f"[{norm_concept}] Finalization complete ({output['diagnostics']['stage2']['duration_seconds']:.2f}s).") # Removed Info
    return output
...
def generate_affinity_definitions_loop(
    ...
    if not taxonomy_concepts_cache: logger.critical("FATAL: Taxonomy concepts cache is empty or None."); return [] # Keep Critical
    limit = args.limit if args.limit and args.limit > 0 else len(concepts_to_process)
    concepts_subset = concepts_to_process[:limit]; logger.info(f"Processing {len(concepts_subset)}/{len(concepts_to_process)} concepts.") # Keep Info Summary

    if not concepts_subset: logger.warning("Concept subset to process is empty!"); return [] # Keep Warning

    disable_tqdm = not logger.isEnabledFor(logging.INFO) or args.debug # Disable progress bar if only warnings/errors shown, or if debug (too much output)
    ...
    for concept in tqdm(concepts_subset, desc="Processing Concepts", disable=disable_tqdm):
        ...
        logger.info(f"=== Processing Concept: '{concept}' ('{norm_concept}') ===") # Keep major marker

        affinity_def = { # Keep full structure initialization
        ...
        try:
            ...
            embed_text = f"{TRAVEL_CONTEXT}{norm_concept}"; logger.debug(f"[{norm_concept}] Embedding text: '{embed_text}'") # Keep Debug
            concept_emb = get_concept_embedding(embed_text, sbert_model)
            if concept_emb is None:
                logger.error(f"[{norm_concept}] Embedding failed."); diag1["error"] = "Embedding failed"; diag1["status"] = "Failed" # Keep Error
            # else: logger.debug(f"[{norm_concept}] Generated embedding.") # Removed Debug
            ...
            diag1.update({
                "status": "Completed" if diag1.get("error") is None and concept_emb is not None else "Failed", # Adjust status based on embedding too
            ...
            logger.info(f"[{norm_concept}] Stage 1 done ({stage1_dur:.2f}s). Status: {diag1['status']}. LLM Cands: {len(cand_details)}. Initial SBERT: {sbert_init_count}, TFIDF: {tfidf_init_count}.") # Keep Info Summary

            # Set travel category early, even if null anchor
            affinity_def["travel_category"] = anchor if anchor and anchor.get('uri') else {"uri": None, "name": concept, "type": "Unknown"}
            if not anchor and diag1["status"] == "Completed": logger.warning(f"[{norm_concept}] Stage 1 completed but no anchor candidate found/selected.") # Keep Warning
            ...
            if diag1["status"] == "Failed":
                logger.warning(f"[{norm_concept}] Skipping LLM slotting due to Stage 1 failure.") # Keep Warning
                diag_llm["status"] = "Skipped (Stage 1 Failed)"
            elif not cand_details:
                logger.warning(f"[{norm_concept}] Skipping LLM slotting (No candidates generated).") # Keep Warning
                ...
                logger.info(f"[{norm_concept}] Skipping LLM slotting (Provider is 'none').") # Keep Info
                ...
                if llm_result and llm_result.get("success"):
                    ...
                    # logger.info(f"[{norm_concept}] LLM slotting call successful.") # Removed Info
                else:
                    ...
                    logger.warning(f"[{norm_concept}] LLM slotting call failed. Error: {diag_llm['error']}") # Keep Warning

            diag_llm["duration_seconds"] = round(time.time() - llm_start, 2)
            logger.info(f"[{norm_concept}] LLM Slotting took {diag_llm['duration_seconds']:.2f}s. Status: {diag_llm['status']}") # Keep Info Summary

            # --- Stage 2: Finalization ---
            stage2_start = time.time()
            # Use orig_map for scores, sbert_scores is redundant if already in orig_map
            stage2_out = apply_rules_and_finalize(
                concept, llm_result, config, affinity_def["travel_category"], anchor,
                orig_map, {}, args, _taxonomy_concepts_cache # Passing empty dict for unused candidate_evidence_scores
            )
            stage2_dur = time.time() - stage2_start

            # Update main definition with stage 2 results
            affinity_def.update({k: v for k, v in stage2_out.items() if k != 'diagnostics'})
            if "diagnostics" in stage2_out: # Merge diagnostics safely
            ...
            # logger.info(f"[{norm_concept}] Stage 2 finalization completed in {affinity_def['diagnostics']['stage2']['duration_seconds']:.2f}s.") # Removed Info

            # Determine final processing status
            final_status = affinity_def["processing_metadata"]["status"]
            if final_status == "Started": # Only update if not already set to warning/error
            ...
            logger.error(f"Core processing loop failed unexpectedly for concept '{concept}': {e}", exc_info=True) # Keep Error
            affinity_def["processing_metadata"]["status"] = f"FATAL ERROR"
            affinity_def["diagnostics"]["error_details"] = traceback.format_exc()
            # Update stage statuses if error occurred mid-process
            ...
            # Log final status and duration for the concept
            ...
            log_func(f"--- Finished '{norm_concept}' ({duration:.2f}s). Status: {affinity_def['processing_metadata']['status']} ---") # Keep major marker
...
def main():
    ...
    if config is None: sys.exit(1) # load_affinity_config should log the error
    _config_data = config
    ...
    setup_logging(log_level, log_file, args.debug) # Setup logging first

    # Keep initial run information logs
    ...
    logger.info(f"COMBINED_SCORE_ALPHA: {config['COMBINED_SCORE_ALPHA']}")
    ...
    logger.info(f"Specific Handling: Aikido (Seed/NoExp/Boost), Aircon/AllInc (NoExp), Aircon (Manual Query Split), Aircon:Unavailable (Filtered)")
    logger.info(f"--- End Key Parameters ---")

    if args.llm_provider is None: args.llm_provider = config.get('LLM_PROVIDER', 'none')
    else: logger.warning(f"Overriding LLM provider from command line: '{args.llm_provider}'") # Keep Warning
    if args.llm_model is None: args.llm_model = config.get('LLM_MODEL', None)
    else: logger.warning(f"Overriding LLM model from command line: '{args.llm_model}'") # Keep Warning
    logger.info(f"Effective LLM Provider: {args.llm_provider}, Model: {args.llm_model or 'N/A'}") # Keep Info
    ...
    try:
        ...
        logger.info(f"Loaded {len(input_concepts)} concepts from '{args.input_concepts_file}'.") # Keep Info
    except FileNotFoundError: logger.critical(f"Input concepts file not found: '{args.input_concepts_file}'"); sys.exit(1) # Keep Critical
    except Exception as e: logger.critical(f"Failed to read input concepts file '{args.input_concepts_file}': {e}"); sys.exit(1) # Keep Critical
    ...
    if concepts_data is None: logger.critical("Taxonomy concepts loading failed."); sys.exit(1) # Keep Critical
    _taxonomy_concepts_cache = concepts_data; logger.info(f"Taxonomy concepts ready ({len(_taxonomy_concepts_cache)} concepts)."); # Keep Info

    try:
        build_keyword_index(config, _taxonomy_concepts_cache, args.cache_dir, cache_ver, args.rebuild_cache)
        if _tfidf_vectorizer is not None: logger.info(f"TF-IDF Index ready.") # Keep Info
        elif config.get("TFIDF_CONFIG", {}).get("enabled", False): logger.warning("TF-IDF is enabled in config but index failed to load/build.") # Keep Warning
        if _keyword_label_index is not None: logger.info(f"Simple Label Index ready.") # Keep Info
        else: logger.warning("Simple Label Index failed to build.") # Keep Warning
    except Exception as e: logger.critical(f"Index building process failed: {e}", exc_info=True); sys.exit(1) # Keep Critical

    # logger.info("Loading SBERT model..."); # Removed Info
    try:
        ...
        logger.info(f"SBERT model '{sbert_name or 'default'}' loaded. Dimension: {sbert_model.get_sentence_embedding_dimension()}") # Keep Info
    except Exception as e: logger.critical(f"SBERT model loading failed: {e}", exc_info=True); sys.exit(1) # Keep Critical

    # logger.info("Loading/Building taxonomy embeddings..."); # Removed Info
    ...
    if embed_data is None: logger.critical("Taxonomy embeddings loading/precomputation failed."); sys.exit(1) # Keep Critical
    _taxonomy_embeddings_cache = embed_data; primary_embs, uris_w_embeds = _taxonomy_embeddings_cache
    logger.info(f"Taxonomy embeddings ready ({len(uris_w_embeds)} concepts embedded)."); # Keep Info

    logger.info("Starting affinity definition generation loop...") # Keep Info
    ...
    end_loop = time.time(); logger.info(f"Generation loop finished ({end_loop - start_loop:.2f}s). Generated {len(results)} definitions.") # Keep Info Summary

    if results: save_results_json(results, out_file) # save_results_json should log success/failure
    else: logger.warning("No results were generated to save.") # Keep Warning
    logger.info(f"Script finished. Final log messages in: {log_file}"); # Keep Info
