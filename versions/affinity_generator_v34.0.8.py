...
(v34.0.8 - BM25 + ACS + Lodging Context + Namespace Biasing + Exact Match + Refined Output).
...
- **Refined Output Logic:**
    ...
    - Calculates a top-level `affinity_score` (60% subscores, 40% attributes).
- Relies on shared utility functions in utils.py (v34+ expected with NaN checks).

Changes from v34.0.6:
- Major refactoring of `apply_rules_and_finalize`:
    - Implemented theme weight redistribution (#2).
    - Implemented subscore calculation based on active themes and config (#6).
    - Added logic for VRBO property type rules (#7, #11).
    - Implemented VRBO minimum weight enforcement for Sentiment/GroupIntelligence (#8, #9).
    - Added top-level `affinity_score` calculation (#5).
    - Verified attribute weight summation (#3).
    - Populates `must_not_have` from config only (#10 deferred).
- Updated `apply_namespace_bias` for differentiated core boost and correct LCM handling.
- Added relevant fields to DEFAULT_CONFIG.
- Updated `prepare_evidence` to fix bug where exact match anchor data wasn't correctly retrieved.
- Updated conceptual SCRIPT_VERSION.
"""
...
try:
    # Ensure rdflib is available if needed by utils, even if not directly used here
...
try:
    ...
    print("Warning: rank-bm25 library not found. BM25 scoring will be disabled.", file=sys.stderr)
    RANK_BM25_AVAILABLE = False
    class BM25Okapi: pass # Dummy class
...
try:
    from utils import (
        ...
        normalize_concept, get_primary_label, get_concept_type_labels,
...
SCRIPT_VERSION = "affinity-rule-engine-v34.0.8 (BM25 + ACS + Lodging Context + NS Bias + Exact Match + Refined Output)"
DEFAULT_CACHE_VERSION = "v20250420.affinity.34.2.0" # Cache version for config/logic changes
DEFAULT_CONFIG_FILE = "./affinity_config_v34.7.json" # Expected config filename
...
DEFAULT_CONFIG = {
    ...
    "global_alpha": 0.6,
    "min_sbert_score": 0.12, # Lowered based on discussion
    "keyword_dampening_threshold": 0.35,
    "keyword_dampening_factor": 0.15,
    "prioritize_exact_prefLabel_match": True, # NEW: Enable exact match priority
    "preferred_namespaces": [
        "urn:expediagroup:taxonomies:core", # Highest priority
        ...
        "ontology.example.com",
         "urn:expediagroup:taxonomies:lcm:" # Treat LCM as relevant
    ],

    # Namespace Biasing Config
    "NAMESPACE_BIASING": {
        "enabled": True,
        "core_boost_factor": 1.10,      # NEW: Higher boost for core
        "boost_factor": 1.05,           # Standard boost
        "context_boost_factor": 1.02,
        "penalty_factor": 0.95,         # Penalty for mismatch/low prio
        "strong_penalty_factor": 0.85,
        "metadata_penalty_factor": 0.90,
        "lodging_amenity_ns": [ # Ensure core: is NOT listed here
            "urn:expediagroup:taxonomies:acs",
            "urn:expediagroup:taxonomies:spaces",
            "urn:expe:taxo:hospitality",
            "urn:expe:taxo:amenity-view-property-features",
            "urn:expediagroup:taxonomies:lcm:", # ADDED LCM
            "urn:expediagroup:taxonomies:acsPCS", # Added PCS here too
            "urn:expediagroup:taxonomies:acsBaseAttribute" # Added BaseAttribute
        ],
        ...
        "metadata_ns": [ "urn:expe:taxo:checkout:", "urn:expe:taxo:payments:", "urn:expe:taxo:policies:", "urn:expe:taxo:review_categories:", "urn:expe:taxo:review_category_values:", "urn:expe:taxo:reviews-attributes:", "urn:expe:taxo:text:", "urn:expe:taxo:data-element-values:", "urn:expediagroup:taxonomies:acsDomainType:", "urn:expediagroup:taxonomies:acsBaseTerms:", "urn:expediagroup:taxonomies:taxonomy_management:", "urn:expediagroup:taxonomies:acsEnumerations:" ], # Added Enumerations here
        "low_priority_ns": [
            #"urn:expediagroup:taxonomies:lcm:", # REMOVED LCM
    ...
    "KEYWORD_SCORING_CONFIG": {
        "enabled": True, "algorithm": "bm25", "bm25_min_score": 0.01,
        "bm25_top_n": 500, "bm25_params": { "k1": 1.5, "b": 0.75 }
    },

    # Field Weights for BM25 Doc Construction
    "KG_CONFIG": {
    ...
    "ACS_DATA_CONFIG": {
        ...
        "acs_name_weight": 4, # Redundant if KG_CONFIG handles all weights
        "acs_def_weight": 2  # Redundant if KG_CONFIG handles all weights
    },

    # Stage 1: Evidence Prep
    "STAGE1_CONFIG": {
    ...
    # LLM API Specifics
    "LLM_API_CONFIG": {
        "MAX_RETRIES": 5, "RETRY_DELAY_SECONDS": 5, "REQUEST_TIMEOUT": 180
    },

    # VRBO Specific Subscore Config (NEW)
    "vrbo_default_subscore_weights": {
          "SentimentScore": 0.1, # Base weight if added automatically
          "GroupIntelligenceScore": 0.1
    },
    # NOTE: vrbo_subscore_boost_factor is not used per clarification

    # Defaults for dynamic population
    "base_themes": {
        # Example structure including relevant_subscores
    ...
    "concept_overrides": {
        # Example for property_type override
        "some_vrbo_concept": {
            "property_type": "VRBO"
        }
    },
    "master_subscore_list": [] # Populated dynamically or from config
}
...
# (get_theme_definition_for_prompt, get_dynamic_theme_config, normalize_weights, deduplicate_attributes, validate_llm_assignments)
# (Unchanged from v34.0.5)
...
# --- START: Keyword Indexing Function (v34.0.8 + BM25 Fix) ---
def build_keyword_index(
    ...
    Reads all parameters including ACS weights from the merged config dictionary.
    Includes filtering for empty documents before BM25 initialization.
    """
    ...
    acs_name_weight = int(kg_cfg.get("acs_name_weight", 0)) # KG_CONFIG now holds these too
    acs_def_weight = int(kg_cfg.get("acs_def_weight", 0))   # KG_CONFIG now holds these too
    ...
    if use_bm25:
        ...
        if enrichment_enabled:
            logger.info(f"ACS Enrichment ENABLED: name x{acs_name_weight}, def x{acs_def_weight}")
            if acs_name_weight == 0 and acs_def_weight == 0:
                logger.warning("ACS Enrichment enabled but ACS field weights in KG_CONFIG are 0. ACS text will not affect BM25 score.")
        ...
        cache_params_for_filename = { "alg": "bm25", "k1": k1, "b": b, "kgw": kgw_str }
        ...
        if not rebuild_cache:
            ...
            if cached_model is not None and isinstance(cached_model, BM25Okapi) and isinstance(cached_uris, list):
                # Check internal state of the cached model for validity
                doc_len_attr = getattr(cached_model, 'doc_len_', getattr(cached_model, '_doc_len', None))
                avg_idf_attr = getattr(cached_model, 'avg_idf', None) # Check another key attribute
                corpus_size_attr = getattr(cached_model, 'corpus_size', getattr(cached_model, '_corpus_size', None))

                # Ensure all necessary attributes are present and match URI list length
                if (doc_len_attr is not None and avg_idf_attr is not None and corpus_size_attr is not None and
                        isinstance(doc_len_attr, (list, np.ndarray)) and # Check if list-like
                        len(doc_len_attr) == len(cached_uris) and
                        corpus_size_attr == len(cached_uris)):
                    ...
                    logger.warning(f"BM25 cache invalid/incomplete state (doc_len:{type(doc_len_attr)}, avg_idf:{avg_idf_attr is not None}, size:{corpus_size_attr}, uris:{len(cached_uris)}). Rebuilding.")
            else:
                logger.info(f"BM25 cache files missing or wrong type. Rebuilding.")

        if not cache_valid:
            logger.info(f"Rebuilding BM25 index...")
            try:
                docs, doc_uris = [], []; # Initialize lists to store document tokens and corresponding URIs
                ...
                # Iterate through all concepts in the taxonomy cache
                for uri, data in tqdm(sorted(taxonomy_concepts_cache.items()), desc="Prepare BM25 Docs", disable=disable_tqdm):
                    texts_to_join = []; # List to hold text snippets for the current document
                    if not isinstance(data, dict): continue # Skip if data is not a dictionary

                    # Extract labels and definitions, handling potential list/string variations
                    ...
                    kg_definition = None
                    if isinstance(definitions_raw, list): # Find first valid definition string
                    ...
                    # Add weighted text snippets to the list
                    ...
                    # ACS Enrichment (if enabled and data exists)
                    acs_name = ''; acs_def = ''; was_enriched = False
                    if enrichment_enabled and uri in _acs_data.index:
                        try:
                            acs_record = _acs_data.loc[uri]
                            if isinstance(acs_record, pd.DataFrame): acs_record = acs_record.iloc[0] # Handle potential duplicate URIs in ACS data
                            acs_name = acs_record.get('AttributeNameENG', '') if pd.notna(acs_record.get('AttributeNameENG', '')) else ''
                            acs_def = acs_record.get('ACS Definition', '') if pd.notna(acs_record.get('ACS Definition', '')) else ''

                            if acs_name or acs_def: # Check if there's anything to add
                            ...
                            logger.warning(f"Error enriching URI '{uri}': {type(acs_lookup_err).__name__}: {acs_lookup_err}.")

                    # Normalize and tokenize the combined text
                    raw_doc_text = " ".join(filter(None, texts_to_join)); norm_doc = normalize_concept(raw_doc_text)

                    # Log debug info for specific URIs if needed
                    ...
                    # Append the tokenized document and its URI
                    # Note: We append even if norm_doc is empty here, filtering happens later
                    docs.append(norm_doc.split()) # norm_doc.split() will be [] if norm_doc is empty
                    doc_uris.append(uri)

                # --- Check for and handle empty documents ---
                initial_doc_count = len(docs)
                if initial_doc_count == 0:
                    logger.warning("No documents generated for BM25 build.")
                    bm25_model = None
                    corpus_uris = []
                else:
                    valid_indices = [i for i, doc in enumerate(docs) if doc] # Find indices of non-empty token lists

                    if len(valid_indices) < initial_doc_count:
                        empty_count = initial_doc_count - len(valid_indices)
                        logger.warning(f"Found {empty_count} empty documents in BM25 input. Filtering them out.")

                        # Filter both docs and corresponding URIs
                        docs = [docs[i] for i in valid_indices]
                        if doc_uris is None or len(doc_uris) != initial_doc_count:
                            logger.error("FATAL: doc_uris list mismatch during empty doc filtering. Aborting BM25 build.")
                            bm25_model = None # Prevent further errors
                            corpus_uris = []
                        else:
                            doc_uris = [doc_uris[i] for i in valid_indices] # Filter URIs to match
                            corpus_uris = doc_uris # Assign filtered list to corpus_uris
                            logger.info(f"Proceeding with {len(docs)} non-empty documents for BM25.")
                    else:
                         corpus_uris = doc_uris # All docs were valid, assign directly
                         logger.info("All generated documents are non-empty.")
                    # --- End empty document handling ---

                    # Proceed only if we still have documents AND bm25_model wasn't set to None above
                    if docs and bm25_model is None:
                        logger.info(f"Building BM25 index for {len(docs)} documents ({enriched_count} enriched).")
                        try:
                            # **Create the BM25 model object**
                            bm25_model = BM25Okapi(docs, k1=k1, b=b)

                            # --- Add Debugging AFTER creation ---
                            if bm25_model:
                                logger.debug(f"BM25 Model created/rebuilt. Type: {type(bm25_model)}")
                                logger.debug(f"Docs len: {len(docs)}, URIs len: {len(corpus_uris)}") # Use filtered lists here
                                # Attempt to get internal attributes safely
                                doc_len_attr_val = getattr(bm25_model, 'doc_len_', getattr(bm25_model, '_doc_len', 'MISSING'))
                                corpus_size_val = getattr(bm25_model, 'corpus_size', getattr(bm25_model, '_corpus_size', 'MISSING'))
                                avg_idf_val = getattr(bm25_model, 'avg_idf', 'MISSING')
                                logger.debug(f"Post-creation check: doc_len_attr: {str(doc_len_attr_val)[:100]}..., corpus_size: {corpus_size_val}, avg_idf: {avg_idf_val}") # Log value safely
                                # Check if doc_len_attr_val looks like a list/array and has the expected length
                                if isinstance(doc_len_attr_val, (list, np.ndarray)):
                                    logger.debug(f"Post-creation check: Length of doc_len_attr: {len(doc_len_attr_val)}")
                                    if len(doc_len_attr_val) != len(docs):
                                         logger.warning(f"Post-creation WARNING: doc_len_attr length ({len(doc_len_attr_val)}) doesn't match filtered docs length ({len(docs)})!")
                                elif doc_len_attr_val != 'MISSING':
                                     logger.warning(f"Post-creation WARNING: doc_len_attr is not list/array, type: {type(doc_len_attr_val)}")
                                else:
                                     logger.warning("Post-creation WARNING: doc_len_attr is MISSING!")
                            else:
                                logger.error("BM25 model object is None immediately after creation attempt!")
                            # --- End Debugging ---

                        except Exception as bm25_init_err:
                             logger.error(f"Error during BM25Okapi initialization: {bm25_init_err}", exc_info=True)
                             bm25_model = None # Ensure model is None if init fails
                             corpus_uris = [] # Ensure uris list is empty too

                    # Save cache only if model was successfully created and URIs exist
                    if bm25_model and corpus_uris:
                        ...
                        logger.info("BM25 model and corpus URIs rebuilt and saved.")
                         ...
                         logger.warning("Skipping BM25 cache saving due to build issues.")

            except Exception as e:
                 logger.error(f"BM25 build error: {e}", exc_info=True)
                 bm25_model, corpus_uris = None, None # Ensure reset on error

        # Log diagnostics based on the final state of bm25_model and corpus_uris
        logger.debug(f"--- BM25 Index Diagnostics ---")
        bm25_size = getattr(bm25_model, 'corpus_size', getattr(bm25_model, '_corpus_size', '?')) if bm25_model else '?'
        doc_len_final = getattr(bm25_model, 'doc_len_', getattr(bm25_model, '_doc_len', None)) if bm25_model else None
        doc_len_len = len(doc_len_final) if isinstance(doc_len_final, (list, np.ndarray)) else 'N/A'

        if bm25_model and doc_len_final is not None:
            logger.debug(f"BM25 Ready (k1={getattr(bm25_model, 'k1', '?')}, b={getattr(bm25_model, 'b', '?')}), Docs: {bm25_size}, Internal DocLens: {doc_len_len}")
        elif bm25_model:
             logger.debug(f"BM25 Model created but internal state seems incomplete (Docs: {bm25_size}, Internal DocLens: {doc_len_len})")
    ...
    # Build simple label index (Independent of BM25)
    try:
        ...
        logger.error(f"Error building label index: {e}", exc_info=True); label_index = None
...
def get_candidate_concepts_keyword(
   ...
   # doc_len_attr = getattr(bm25_model, 'doc_len_', getattr(bm25_model, '_doc_len', None))
    #corpus_size_attr = getattr(bm25_model, 'corpus_size', getattr(bm25_model, '_corpus_size', None))
    #f doc_len_attr is None or corpus_size_attr is None or len(doc_len_attr) != len(corpus_uris):
     #    logger.error(f"BM25 model/URI size mismatch (Docs:{len(doc_len_attr) if doc_len_attr is not None else 'N/A'}, URIs:{len(corpus_uris)}).");
      #   return []
    ...
    try:
        ...
        if actual_top_n < num_cands * 0.5 and actual_top_n > 0:
            indices = np.argpartition(all_scores, -actual_top_n)[-actual_top_n:]
        else:
        ...
        for i in top_indices_sorted:
            ...
            if score >= min_score: # Apply threshold here
...
def call_llm(system_prompt: str, user_prompt: str, model_name: str, timeout: int, max_retries: int, temperature: float, provider: str) -> Dict[str, Any]:
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if provider == "openai":
                ...
                if hasattr(resp, 'prompt_feedback') and resp.prompt_feedback and resp.prompt_feedback.block_reason:
                    block_reason = getattr(resp.prompt_feedback, 'block_reason', '?'); logger.warning(f"Gemini blocked. Reason:{block_reason}"); result["error"] = f"Blocked:{block_reason}"; return result # Return immediately on block
            ...
            if content:
                try:
                    ...
                    if isinstance(output, dict): result["success"] = True; result["response"] = output; return result # Success!
        ...
        except Exception as e: logger.error(f"{provider} Call Error: {e}", exc_info=True); result["error"] = str(e); return result # Fatal error, don't retry

        # Retry logic
        should_retry = attempt < max_retries and (result["error"] is not None) and ("Blocked" not in str(result.get("error"))) # Don't retry if blocked
        if not result.get("error"): result["error"] = "Failed after retries." # Ensure error message exists if loop finishes
        if should_retry:
             ...
             if not result.get("success"): logger.error(f"LLM call failed after {attempt + 1} attempts. Final Error: {result.get('error')}")
             return result # Return final result after all retries or if not retrying
    return result # Should technically be unreachable if max_retries >= 0

# --- START: Prompt Building Functions ---
# (Unchanged from v34.0.5 - Already include Lodging Context)
...
# (apply_namespace_bias updated for core boost and correct lcm handling)
def infer_concept_domain(candidate_uris: List[str], config: Dict, top_n_check: int = 50) -> str:
    ...
    lodging_amenity_ns = tuple(bias_config.get("lodging_amenity_ns", ())) # Includes LCM now
    activity_ns = tuple(bias_config.get("activity_ns", ()))
    core_ns_prefix = "urn:expediagroup:taxonomies:core:" # Define core prefix
    ...
    for uri in candidate_uris[:top_n_check]:
        checked_count += 1
        if uri.startswith(core_ns_prefix): lodging_count += 1 # Count core as lodging
...
# --- Namespace Biasing Helper Function (v34.0.8 - Differentiated Core Boost, LCM Boosted) ---
def apply_namespace_bias(
    score: float,
    candidate_uri: str,
    inferred_domain: str,
    config: Dict
) -> Tuple[float, str]: # Return score and reason string
    """
    Applies a multiplicative bias based on namespace and inferred domain using detailed config.
    v34.0.8: Uses granular namespace categories, applies a potentially higher boost factor
             for the core: namespace, treats lcm: as a standard lodging namespace (boosted),
             and uses context boosts/penalties.
    Returns the new score and a reason string for logging.
    """
    bias_config = config.get("NAMESPACE_BIASING", {})
    if not bias_config.get("enabled", False):
        return score, "Neutral (Biasing Disabled)"

    # Read factors from config with defaults
    core_boost_factor = float(bias_config.get("core_boost_factor", 1.10)) # Specific boost for core
    boost_factor = float(bias_config.get("boost_factor", 1.05))          # Standard domain match boost
    ...
    # Define core prefix for checking
    core_ns_prefix = "urn:expediagroup:taxonomies:core:"

    # Read namespace lists from config (using tuples for startswith)
    lodging_amenity_ns = tuple(ns for ns in bias_config.get("lodging_amenity_ns", ()) if not ns.startswith(core_ns_prefix))
    activity_ns = tuple(bias_config.get("activity_ns", ()))
    ...
    low_priority_ns = tuple(bias_config.get("low_priority_ns", ())) # Should NOT contain lcm:

    reason = "Neutral"
    new_score = score
    initial_score = score # Keep track for applying boosts/penalties correctly

    # --- Penalties applied first (strongest overrides) ---
    ...
    if metadata_ns and candidate_uri.startswith(metadata_ns):
        ...
        return new_score, reason # We usually stop here for metadata
    ...
    # --- Domain-specific biasing (Only apply if no overriding penalty hit) ---
    if not reason.startswith("Penalty (Metadata"):
        domain_bias_applied = False
        if not penalty_applied: # Apply boosts only if no initial penalty was applied
            if inferred_domain == "Lodging/Amenity":
                if candidate_uri.startswith(core_ns_prefix):
                    ...
                    domain_bias_applied = True
                     ...
                     domain_bias_applied = True
                elif preference_context_ns and candidate_uri.startswith(preference_context_ns):
                     new_score = initial_score * context_boost_factor
        ...
        # Apply penalties for domain mismatches ONLY if no penalty was applied yet AND no boost/context was applied
        ...
        # If no domain-specific bias applied AND no initial penalty, reset reason
        if not domain_bias_applied and not penalty_applied:
            reason = "Neutral"

    # Final sanity check
    if new_score <= 0:
        logger.warning(f"Bias resulted in non-positive score ({new_score:.4f}) for {candidate_uri}. Resetting to small positive.")
        new_score = 1e-9
...
# --- Stage 1: Evidence Preparation (v34.0.8 - Corrected Exact Match Anchor Handling) ---
def prepare_evidence(
    ...
    """
    Prepares evidence candidates using combined score (BM25 + SBERT), config-driven overrides,
    refined contextual namespace biasing, and optional exact prefLabel match priority
    (with simple plural handling and bug fix for anchor selection).

    v34.0.8 Changes:
    - Fixed bug where exact match anchor URI was selected but data wasn't correctly retrieved,
      causing unnecessary fallback to combined score.
    - Uses updated apply_namespace_bias function.
    """
    start_time_stage1 = time.time()
    normalized_input_concept = normalize_concept(input_concept)
    selection_method_log = "Combined BM25+SBERT" # Default
    ...
    kw_top_n = int(kw_scoring_cfg.get('bm25_top_n', 500)); abs_min_sbert = float(config.get("min_sbert_score", 0.12))
    ...
    if prioritize_exact_match:
        logger.info(f"[{normalized_input_concept}] Tie-Break Sort (Exact Match): Namespace>SBERT")

    sim_scores, kw_scores = {}, {}; exp_diag = {"attempted": False, "successful": False, "count": 0, "terms": [], "keyword_count": 0, "error": None}

    # Prepare keyword query terms
    base_kws = set(kw for kw in normalized_input_concept.split() if len(kw)>2); final_query_texts = list(base_kws)
    initial_kw_count = 0;
    ...
    if kw_exp_enabled and needs_exp:
        exp_diag["attempted"] = True; logger.info(f"[{normalized_input_concept}] Attempting LLM KW expansion (Initial:{initial_kw_count}<{kw_trigger} or abstract).")
        final_query_texts, exp_success, exp_error = expand_keywords_with_llm(input_concept, config, args)
    ...
    if args.debug: logger.debug(f"[BM25 QUERY] '{normalized_input_concept}', Terms({len(final_query_texts)}): {final_query_texts}")
    ...
    if bm25_model and keyword_corpus_uris:
        if final_query_texts:
            ...
            kw_cands_raw = get_candidate_concepts_keyword(final_query_texts, bm25_model, keyword_corpus_uris, bm25_top_n, 0.0)
            kw_scores = {c['uri']: c['score'] for c in kw_cands_raw if c['score'] >= bm25_min}; kw_cand_count_init = len(kw_scores); exp_diag["keyword_count"] = kw_cand_count_init
            logger.debug(f"[{normalized_input_concept}] Found {kw_cand_count_init} BM25 candidates >= {bm25_min} (out of {len(kw_cands_raw)} raw).")
        else: logger.warning(f"[{normalized_input_concept}] No keywords for BM25 search."); kw_scores={}; exp_diag["keyword_count"]=0
    else: logger.warning(f"[{normalized_input_concept}] BM25 unavailable."); kw_scores={}; exp_diag["keyword_count"]=0
    ...
    if concept_embedding is None: logger.error(f"[{normalized_input_concept}] No anchor embed. Skipping SBERT."); sim_scores={}
        ...
        logger.debug(f"[{normalized_input_concept}] Found {sbert_cand_count_init} SBERT candidates >= {sbert_min_sim} (out of {len(sbert_cands_raw_map)} raw).")
    ...
    # --- Exact prefLabel Match Check (with simple plural handling) ---
    exact_match_uris = []
    if prioritize_exact_match:
        logger.debug(f"[{normalized_input_concept}] Checking for exact prefLabel matches (with simple plural check)...")
        match_count = 0
        norm_input = normalized_input_concept

        for uri in all_uris_for_exact_match:
            if uri not in all_uris and uri not in seed_uris: continue
            ...
            if pref_label:
                ...
                if is_match:
                    ...
                    match_count += 1
                    if args.debug: logger.debug(f"  {match_type} match found: {uri} (Input: '{norm_input}', Label: '{norm_label}')")
        logger.info(f"[{normalized_input_concept}] Found {match_count} exact prefLabel matches (incl. simple plurals).")
    # --- END: Exact prefLabel Match Check ---
    ...
    if ns_bias_enabled:
        initial_candidate_uris_for_domain = list(set([c['uri'] for c in kw_cands_raw] + list(sbert_cands_raw_map.keys())))
        inferred_domain = infer_concept_domain(initial_candidate_uris_for_domain, config)
        logger.info(f"[{normalized_input_concept}] Inferred domain (based on raw cands): {inferred_domain}")

    # Get Details & Score/Rank/Bias FOR ALL candidates
    all_details = get_kg_data(all_uris_list, taxonomy_concepts_cache); scored_list = []
    abs_filt = 0; damp_cnt = 0; bias_cnt = 0; alpha_overridden = abs(effective_alpha - global_alpha) > 1e-9
    for uri in all_uris_list:
        ...
        if current_sbert_for_filter < abs_min_sbert:
            abs_filt += 1
            if args.debug: logger.debug(f"[{normalized_input_concept}] URI {uri} filtered by abs_min_sbert ({current_sbert_for_filter:.4f} < {abs_min_sbert})")
            continue
        ...
        norm_s = max(0.0, min(1.0, current_sbert_for_filter)); norm_k = max(0.0, min(1.0, k_damp))
        ...
        if ns_bias_enabled and not boosted:
             combined, bias_reason = apply_namespace_bias(combined_unbiased, uri, inferred_domain, config) # Uses updated function
             if abs(combined - combined_unbiased) > 1e-9: biased = True; bias_cnt +=1

        scored_list.append({
            "uri":uri, "details":all_details[uri],
            "sim_score":s_orig, "boosted_sim_score":s_boosted if boosted else None,
            "kw_score":k_raw, "dampened_kw_score":k_damp if dampened else None,
            "alpha_overridden":alpha_overridden, "effective_alpha":effective_alpha,
            "combined_score_unbiased": combined_unbiased, "combined_score": combined,
    ...
    # Sort the *full* list by combined score
    logger.debug(f"[{normalized_input_concept}] Sorting {len(scored_list)} candidates by combined score...")
    ...
    # Log Top 5 based on combined score
    if args.debug:
        logger.debug(f"--- Top 5 for '{normalized_input_concept}' (Sorted by Combined Score) ---")
        logger.debug(f"    (EffAlpha:{effective_alpha:.2f}, DampThresh:{damp_thresh}, DampFactor:{damp_factor}), Domain: {inferred_domain}, Bias:{ns_bias_enabled}")
        for i, c in enumerate(scored_list[:5]):
            damp=f"(Damp:{c.get('dampened_kw_score'):.4f})" if c.get('dampened_kw_score') is not None else ""; boost=f"(Boost:{c.get('boosted_sim_score'):.4f})" if c.get('boosted_sim_score') is not None else ""; alpha=f"(EffA:{c.get('effective_alpha'):.2f})"; prio=f"(P:{get_sort_priority(c.get('uri',''))})"; lbl=get_primary_label(c.get('uri', '?'), taxonomy_concepts_cache, c.get('uri', '?'))
            bias_info = f" -> Biased:{c.get('combined_score'):.6f} ({c.get('bias_reason','?')})" if c.get('biased') else ""
    ...
    # Select Top-N candidates for LLM based on combined score ranking
    selected = scored_list[:max_cands]; logger.info(f"[{normalized_input_concept}] Selected top {len(selected)} candidates for LLM based on combined score.")
    ...
    # Prepare outputs for LLM context
    ...
    # --- Anchor Selection Logic (v34.0.8 - Bug Fix) ---
    anchor_uri = None
    anchor_data_dict = None # Will hold the dict from scored_list for the chosen anchor

    if prioritize_exact_match and exact_match_uris:
        selection_method_log = "Exact prefLabel Match (Plural Check)"
        logger.info(f"[{normalized_input_concept}] Prioritizing anchor selection from {len(exact_match_uris)} exact prefLabel matches (incl. plurals).")
        ...
        for uri in exact_match_uris:
            ...
            if sbert_score >= abs_min_sbert:
                exact_match_candidates_to_sort.append({"uri": uri, "sbert_score": sbert_score})
            else:
                 logger.warning(f"[{normalized_input_concept}] Exact match URI {uri} excluded due to low SBERT score ({sbert_score:.4f} < {abs_min_sbert}).")

        if exact_match_candidates_to_sort:
            exact_match_candidates_to_sort.sort(key=lambda x: x['sbert_score'], reverse=True)
            exact_match_candidates_to_sort.sort(key=lambda x: get_sort_priority(x['uri']), reverse=False)
            selected_exact_match_uri = exact_match_candidates_to_sort[0]['uri']
            logger.info(f"[{normalized_input_concept}] Selected exact match anchor URI: {selected_exact_match_uri} (Prio: {get_sort_priority(selected_exact_match_uri)}, SBERT: {exact_match_candidates_to_sort[0]['sbert_score']:.4f})")

            # *** BUG FIX: Find the full data for this anchor from scored_list ***
            found_data = False
            for item in scored_list:
                if item['uri'] == selected_exact_match_uri:
                    anchor_uri = selected_exact_match_uri # Confirm selection
                    anchor_data_dict = item # Assign the full data dictionary
                    found_data = True
                    break

            if not found_data:
                 logger.error(f"[{normalized_input_concept}] CRITICAL INCONSISTENCY: Could not find full data for selected exact match anchor {selected_exact_match_uri} in scored_list! Falling back to combined score.")
                 anchor_uri = None # Force fallback
                 selection_method_log = "Combined BM25+SBERT (Exact Match Inconsistency Fallback)"
        else:
             logger.warning(f"[{normalized_input_concept}] No valid exact prefLabel matches remain after SBERT filter. Falling back to combined score.")
             selection_method_log = "Combined BM25+SBERT (Exact Match Fallback)"

    # Fallback or default anchor selection based on combined score
    if anchor_uri is None: # This triggers if prioritize is false, no exact matches, or exact matches failed filter/lookup
        if not scored_list:
             logger.error(f"[{normalized_input_concept}] No candidates in scored_list for fallback anchor selection.")
             anchor_data_dict = None
        else:
             anchor_data_dict = scored_list[0] # Top candidate from combined score sort
             anchor_uri = anchor_data_dict.get('uri')
             if prioritize_exact_match and exact_match_uris: # Log only if we intended to use exact match but fell back
                 logger.info(f"[{normalized_input_concept}] Using fallback anchor (combined score): {anchor_uri}")
             selection_method_log = "Combined BM25+SBERT"

    # Populate the final anchor dictionary using anchor_data_dict
    anchor = None
    if anchor_data_dict and anchor_uri:
        ...
        score_display = f"Exact Match (Prio: {get_sort_priority(anchor_uri)}, SBERT: {anchor.get('sbert_score', 0.0):.4f})" if selection_method_log.startswith("Exact prefLabel Match") else f"Score: {anchor.get('combined_score'):.6f}{' - Biased' if anchor.get('biased') else ''}"
        logger.info(f"[{normalized_input_concept}] Anchor ({selection_method_log}): {anchor_label} ({score_display})")

    elif anchor_uri: # Should not happen if anchor_data_dict logic is correct
        logger.error(f"[{normalized_input_concept}] Anchor URI '{anchor_uri}' selected but corresponding data missing!")
        anchor = {"uri": anchor_uri, "name": input_concept, "type": "Unknown", "error": "Data missing"}
    else:
        logger.error(f"[{normalized_input_concept}] Failed to select any anchor.")
        anchor = {"uri": None, "name": input_concept, "type": "Unknown", "error": "No anchor selected"}

    sbert_scores_final = {uri: d["sbert_score"] for uri, d in orig_map.items() if "sbert_score" in d}
...
# --- Stage 2: Finalization (v34.0.8 - Refactored Output Logic) ---
def apply_rules_and_finalize(
    input_concept: str, llm_call_result: Optional[Dict[str, Any]], config: Dict,
    travel_category: Optional[Dict], anchor_candidate: Optional[Dict], # anchor_candidate is same as travel_category?
    original_candidates_map_for_reprompt: Dict[str, Dict], # Map URI -> full candidate data from stage 1
    candidate_evidence_scores: Dict[str, float], # Map URI -> SBERT score (use this for attr weighting)
    args: argparse.Namespace, taxonomy_concepts_cache: Dict[str, Dict]
) -> Dict[str, Any]:
    """
    Applies rules, handles fallbacks, weights attributes & subscores, structures final output.
    v34.0.8: Implements theme weight redistribution, subscore calculation based on themes,
             VRBO-specific subscore rules, and affinity_score calculation.
    """
    start_stage2 = time.time(); norm_concept = normalize_concept(input_concept)
    # --- Get Configs & Overrides ---
    ...
    vrbo_sentiment_min = 0.10 # Requirement #8
    vrbo_groupintel_min = 0.05 # Requirement #9
    ...
    # --- Process LLM Assignments & Fallback ---
    ...
    theme_map = defaultdict(list) # Maps theme_name -> [assigned_uris]
    ...
    if failed_rules and original_candidates_map_for_reprompt and llm_refinement_enabled and args.llm_provider != "none":
        ...
        for name in list(failed_rules.keys()):
            ...
            fb_result = call_llm(sys_p, user_p, args.llm_model, fb_timeout, fb_retries, fb_temp, args.llm_provider)
    ...
    # --- Calculate Initial Theme & Subscore Weights based on Config ---
    initial_theme_weights = {}
    initial_subscore_contributions = defaultdict(float)
    ...
    for name, theme_data in base_themes_config.items():
        ...
        if isinstance(relevant_subscores_cfg, dict):
            for subscore_name, base_subscore_weight in relevant_subscores_cfg.items():
                 contribution = weight * float(base_subscore_weight)
                 initial_subscore_contributions[subscore_name] += contribution
    ...
    # --- Identify Active Themes & Calculate Redistributed Weights (#2) ---
    active_themes = set(name for name, uris in theme_map.items() if uris)
    logger.debug(f"[{norm_concept}] Active themes (with attributes): {active_themes}")
    ...
    total_weight_of_active_themes = sum(normalized_initial_theme_weights.get(name, 0) for name in active_themes)
    ...
    for name in base_themes_config:
        if name in active_themes and total_weight_of_active_themes > 0:
            ...
            redistributed_share = (initial_norm_w / total_weight_of_active_themes) * weight_to_redistribute if total_weight_of_active_themes > 0 else 0
            final_theme_weights[name] = initial_norm_w + redistributed_share
        else:
            final_theme_weights[name] = 0.0

    logger.debug(f"[{norm_concept}] Final theme weights after redistribution: { {k: round(v, 4) for k, v in final_theme_weights.items()} }")


    # --- Process Themes & Attributes using FINAL weights (#3) ---
    ...
    for name, base_data in base_themes_config.items():
        ...
        rule, _, subscore_affinity_name, _ = get_dynamic_theme_config(norm_concept, name, config)
        uris = theme_map.get(name, [])
        theme_attrs = []

        if uris and final_norm_w > 1e-9:
            scores = {u: candidate_evidence_scores.get(u, 0.0) for u in uris if u in candidate_evidence_scores}
        ...
        if theme_diag[name].get("status") == "Pending":
        ...
        final_themes_out.append({
            "theme_name": name, "theme_type": base_data.get("type", "?"), "rule_applied": rule,
            "normalized_theme_weight": round(final_norm_w, 6), # Use final weight
            "subScore": subscore_affinity_name or f"{name}Affinity", "llm_summary": None,
            "attributes": theme_attrs
        })
    output["themes"] = final_themes_out
    logger.debug(f"[{norm_concept}] Sum of final attribute weights: {total_attribute_weight_sum:.4f} (Expected ~1.0 if attributes exist)")
    ...
    # --- Calculate Final Subscore Weights (#6, #7, #11, #8/#9) ---
    final_subscore_weights = defaultdict(float)
    logger.debug(f"[{norm_concept}] Calculating subscore weights from active themes...")
    for name, theme_data in base_themes_config.items():
        if name in active_themes:
            ...
            if isinstance(relevant_subscores_cfg, dict):
                for subscore_name, base_subscore_weight in relevant_subscores_cfg.items():
                     ...
                     logger.debug(f"  Theme '{name}' (w={theme_final_weight:.3f}) adds {contribution:.4f} to '{subscore_name}'")

    if property_type == "VRBO":
        logger.info(f"[{norm_concept}] Applying VRBO subscore rules.")
        # Use a small non-zero default weight if adding missing scores
        ...
        if "SentimentScore" not in final_subscore_weights:
            ...
            logger.debug(f"  Added missing SentimentScore for VRBO (initial weight: {sentiment_default:.2e})")
        if "GroupIntelligenceScore" not in final_subscore_weights:
             final_subscore_weights["GroupIntelligenceScore"] = groupintel_default
             logger.debug(f"  Added missing GroupIntelligenceScore for VRBO (initial weight: {groupintel_default:.2e})")
    ...
    if total_subscore_weight_before_vrbo_min > 1e-9:
        ...
        logger.info(f"[{norm_concept}] No relevant subscores found or calculated total weight is zero.")

    # VRBO minimum weight enforcement & re-normalization
    if property_type == "VRBO" and normalized_subscores:
        made_adjustments = False
        # Ensure keys exist before comparison/assignment
        ...
        if normalized_subscores["SentimentScore"] < vrbo_sentiment_min:
            logger.debug(f"  Boosting SentimentScore weight from {normalized_subscores['SentimentScore']:.4f} to minimum {vrbo_sentiment_min}")
        ...
        if normalized_subscores["GroupIntelligenceScore"] < vrbo_groupintel_min:
             logger.debug(f"  Boosting GroupIntelligenceScore weight from {normalized_subscores['GroupIntelligenceScore']:.4f} to minimum {vrbo_groupintel_min}")
        ...
        if made_adjustments:
            ...
            if current_sum > 1e-9 and abs(current_sum - 1.0) > 1e-9 :
                logger.debug(f"  Re-normalizing subscore weights after VRBO minimum enforcement (Current sum: {current_sum:.4f})")
                renorm_factor = 1.0 / current_sum
                normalized_subscores = {name: weight * renorm_factor for name, weight in normalized_subscores.items()}

    # Populate the final output list
    output["additional_relevant_subscores"] = [
        {"subscore_name": name, "weight": round(weight, 6)}
        for name, weight in sorted(normalized_subscores.items(), key=lambda item: item[1], reverse=True)
        if weight > 1e-9 # Filter out negligible scores
    ]
    logger.debug(f"[{norm_concept}] Final subscores: {output['additional_relevant_subscores']}")

    # --- Calculate Affinity Score (#5) ---
    subscore_component = sum(item['weight'] for item in output["additional_relevant_subscores"])
    ...
    output["affinity_score"] = round((0.60 * subscore_component) + (0.40 * attribute_component), 6)
    logger.info(f"[{norm_concept}] Calculated Affinity Score: {output['affinity_score']:.4f} (Subscores: {subscore_component:.3f}*0.6, Attributes: {attribute_component:.3f}*0.4)")

    # --- Populate Must Not Have (#10) ---
    ...
    output["diagnostics"]["stage2"]["status"] = "Completed"; output["diagnostics"]["stage2"]["duration_seconds"] = round(time.time() - start_stage2, 2)

    return output
# --- END: Stage 2 Finalization ---

# --- Main Processing Loop ---
# (v34.0.8 - Corrected Finalize Call variable name)
def generate_affinity_definitions_loop(
    ...
    """ Main loop processing each concept. """
    all_definitions = []
    ...
    for concept in tqdm(concepts_subset, desc="Processing Concepts", disable=disable_tqdm):
        ...
        affinity_def = { # Initialize output structure
            ...
            "failed_fallback_themes": {}, "affinity_score": 0.0,
            ...
            "diagnostics": {
                "stage1": { "status": "Not Started", "error": None, "selection_method": "?", "expansion": {}, "sbert_candidate_count_initial": 0, "keyword_candidate_count_initial": 0, "unique_candidates_before_ranking": 0, "llm_candidate_count": 0 },
        ...
        try:
            ...
            cand_details, orig_map, anchor, sbert_scores_final, exp_diag, sbert_init, kw_init, unique_count = prepare_evidence(
            ...
            # Update diag1 with results from prepare_evidence, including duration
            diag1.update({
                "status": "Completed" if diag1.get("error") is None and concept_emb is not None else "Failed",
                "error": diag1.get("error"), # Keep existing error if any
                "selection_method": exp_diag.get("selection_method", "Combined BM25+SBERT"),
                "expansion": exp_diag,
                ...
                "duration_seconds": round(stage1_dur, 2) # Add duration here
            })
            ...
            stage2_out = apply_rules_and_finalize(
                 ...
                 config=config,
                 travel_category=anchor, # Pass the anchor dict selected in stage 1
                 anchor_candidate=anchor, # Pass the anchor dict again
                 original_candidates_map_for_reprompt=orig_map,
                 candidate_evidence_scores=sbert_scores_final, # Pass correct variable
                 args=args,
                 taxonomy_concepts_cache=_taxonomy_concepts_cache
            )

            # Merge results back into the main affinity_def dictionary
            affinity_def.update({k: v for k, v in stage2_out.items() if k != 'diagnostics'})
            if "diagnostics" in stage2_out: # Merge diagnostics carefully
            ...
            # Determine final status based on stages
            final_status = affinity_def["processing_metadata"]["status"]
            if final_status == "Started": # If not already set to Warning/Error
...
def main():
    ...
    # 2. Start with defaults, merge user config (recursive update)
    ...
    # 4. Set global config and update args namespace
    _config_data = config
    ...
    logger.info(f"BM25 DOC: KG(pref x{kg_config.get('pref_label_weight', '?')}, alt x{kg_config.get('alt_label_weight', '?')}, def x{kg_config.get('definition_weight', '?')}) + ACS(En:{acs_enabled_log}, name x{kg_config.get('acs_name_weight', '?')}, def x{kg_config.get('acs_def_weight', '?')})")
    ...
    logger.info(f"Namespace Biasing: {ns_bias_cfg.get('enabled', False)} (Core Boost: {ns_bias_cfg.get('core_boost_factor', 'N/A')})")
