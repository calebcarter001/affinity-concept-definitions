# -*- coding: utf-8 -*-
"""
Generate affinity definitions for travel concepts using combined scoring (v33.0.28 - Config-Driven Overrides).
...
Changes from v33.0.27:
- Refactored prepare_evidence and apply_rules_and_finalize to remove hardcoded logic.
- All overrides are now read from 'concept_overrides' section of config JSON.
- Corrected expand_keywords_with_llm return signature and diagnostic handling.
- Updated SCRIPT_VERSION and relevant logs.
- NOTE: Still relies on weighted TF-IDF cache (requires rebuild if weights change or coming from pre-v33.0.27).
"""
...
SCRIPT_VERSION = "affinity-rule-engine-v33.0.28 (Config-Driven Overrides)" # Updated version
# Cache version default - should ideally match the one in config if specified
DEFAULT_CACHE_VERSION = "v20250420.affinity.33.0.28"
        ...
        if not cache_valid:
            ...
            try:
                ...
                for uri, data in tqdm(sorted(taxonomy_concepts_cache.items()), desc="Prepare TF-IDF Docs", disable=disable_tqdm):
                    ...
                    if uri in uris_to_log_tfidf_doc and args.debug: # Check debug flag here
...
def get_candidate_concepts_keyword(
    ...
    # Reduce debug noise unless explicitly needed
    ...
    try:
        ...
        if top_n < num_cands * 0.5 : # Heuristic threshold
            # Ensure we handle cases where fewer than top_n elements meet the threshold
            potential_indices = np.where(agg_scores >= min_score)[0]
            if len(potential_indices) == 0: return []
            scores_above_min = agg_scores[potential_indices]
            num_above_min = len(scores_above_min)
            actual_top_n = min(top_n, num_above_min)
            if actual_top_n == 0: return []

            if actual_top_n < num_above_min * 0.5:
                idx_unsorted_above_min = np.argpartition(scores_above_min, -actual_top_n)[-actual_top_n:]
                sorted_indices_within_partition = np.argsort(scores_above_min[idx_unsorted_above_min])[::-1]
                indices = potential_indices[idx_unsorted_above_min[sorted_indices_within_partition]]
            else:
                indices = potential_indices[np.argsort(scores_above_min)[::-1][:actual_top_n]]
        else: # If top_n is large relative to num_cands, simpler sort might be okay
            indices = np.argsort(agg_scores)[::-1]
            indices = [i for i in indices if agg_scores[i] >= min_score][:top_n]


        candidates = [{"uri": corpus_uris[i], "score": float(agg_scores[i]), "method": "keyword_tfidf"} for i in indices]
        # logger.debug(f"Found {len(candidates)} TF-IDF candidates >= {min_score} (Top {top_n})")
        return candidates
...
# (expand_keywords_with_llm updated for v33.0.28a - Returns status/error)
...
# --- START: Stage 1: Evidence Preparation (v33.0.28a) ---
def prepare_evidence(
    ...
    """
    Prepares evidence candidates using combined score and config-driven overrides.
    v33.0.28a: Reads concept-specific overrides (alpha, seeding, filtering, expansion,
               query split, boosting) from config['concept_overrides'].
               Retains tie-breaking sort, TF-IDF weighting, dampening, abs SBERT filter.
    ...
    # Expansion is enabled if globally enabled AND provider available AND NOT skipped by override
    ...
    # Initialize diagnostics dictionary HERE
    ...
    query_texts_set = set(base_split_kws) # Start with base terms
    final_query_texts = list(query_texts_set) # Initialize final list
    ...
    # Determine if expansion is needed based on effective kw_exp_enabled and triggers
    ...
    if kw_exp_enabled:
        if needs_exp:
            ...
            if exp_success:
                 expansion_info = f"Successful, final query terms count: {len(final_query_texts)}"
                 # logger.debug(f"[{normalized_concept}] LLM expansion successful. Final terms: {final_query_texts}") # Potentially noisy
            else:
                 expansion_info = f"Attempted, but failed or no new terms. Error: {exp_error}"
    ...
    if manual_query_terms is not None:
        if isinstance(manual_query_terms, list) and all(isinstance(t, str) for t in manual_query_terms):
            if set(final_query_texts) != set(manual_query_terms):
                ...
                # Update diagnostics if manual split overrides expansion results
    ...
    # Update final diagnostics based on the final query terms if not manually overridden
    if manual_query_terms is None:
        exp_diag["terms"] = final_query_texts
        exp_diag["count"] = len(final_query_texts)


    # Log TF-IDF Query Terms
    concepts_to_log_tfidf_query = {'airconditioning', 'allinclusive'} # Examples, remove if not needed
    ...
    if tfidf_vectorizer and tfidf_matrix is not None and tfidf_corpus_uris:
        if final_query_texts:
            ...
            exp_diag["tfidf_count"] = tfidf_candidate_count_initial # Store count after TFIDF search
        ...
        for uri_to_filter in uris_to_filter_for_concept:
            if uri_to_filter in all_uris_set:
                ...
                uris_to_remove.add(uri_to_filter)
                 ...
                 logger.debug(f"Specific filter URI {uri_to_filter} from config not found in candidate set for '{normalized_concept}'.")
    ...
    if initial_unique_count != unique_candidates_before_ranking and not seed_uris_for_concept and not uris_to_filter_for_concept:
        logger.debug(f"[{normalized_concept}] Unique candidates count changed unexpectedly.") # Should only change due to seeding/filtering
    ...
    dampened_count = 0
    alpha_override_applied_this_concept = abs(effective_alpha - global_alpha) > 1e-9 # Check if alpha was actually changed

    for i, uri in enumerate(all_uris_list): # Use i for first pass logs if needed
        ...
        if is_seed_uri and boost_config.get("enabled", False):
            boost_threshold = float(boost_config.get("threshold", 0.80)) # Read threshold from config
        ...
        if boosted_s_score_for_calc < abs_min_sbert:
            if args.debug:
                 ...
                 prefix = "[Seed] FILTERED seed " if is_seed_uri else "Excluding "
                 logger.debug(f"{prefix}{uri} ('{pref_label_for_log}') due to {filter_reason}. (TFIDF: {k_score:.4f})")
            if is_seed_uri:
                 logger.warning(f"[Seed] FILTERED seed URI {uri} ('{pref_label_for_log}') due to low SBERT {boosted_s_score_for_calc:.4f} (after potential boost).")
        ...
        # No specific logic needed here, effective_alpha variable holds the correct value
        ...
        scored_list.append({
            ...
            "alpha_overridden": alpha_override_applied_this_concept, # Flag if non-default alpha used for this concept
         ...
         if uri and details:
              orig_map_entry = {
                  **details,
                  ...
                  "effective_alpha": c.get("effective_alpha", global_alpha) # Use global_alpha as fallback
              }
...
# --- END: Stage 1: Evidence Preparation (v33.0.28a) ---
...
# (generate_affinity_definitions_loop unchanged from v33.0.26)
def generate_affinity_definitions_loop(
    ...
    all_definitions = []
    # Use cache version defined in config if available, else default
...
def main():
    ...
    # NOTE: Assign parsed args to global 'args' *early* so build_keyword_index can use it
    ...
    # Ensure cache_version from config is used if present
    cache_ver = config.get("CACHE_VERSION", DEFAULT_CACHE_VERSION)
    ...
    logger.info(f"ALPHA MECHANISM: Handled via config['concept_overrides']['<concept>']['effective_alpha']") # Updated log
    logger.info(f"SORTING: Tie-Breaking Enabled (Score > Namespace Prio > SBERT)")
    logger.info(f"TF-IDF STRATEGY: Weighted Field Concatenation (prefLabel x5, altLabel x3, definition x1)")
    ...
    logger.info(f"Specific Handling (Seeding/Filtering/Boosting/Splitting/Expansion): Driven by config['concept_overrides']") # Updated log
