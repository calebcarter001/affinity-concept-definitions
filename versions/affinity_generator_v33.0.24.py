...
# -*- coding: utf-8 -*-
"""
Generate affinity definitions for travel concepts using combined scoring (v33.0.24 - Dynamic Alpha).

Implements:
- Dynamic Alpha adjustment based on SBERT/TF-IDF score thresholds.
...
SCRIPT_VERSION = "affinity-rule-engine-v33.0.24 (Dynamic Alpha)" # Updated version
DEFAULT_CACHE_VERSION = "v20250420.affinity.33.0.19" # Keep base cache version unless TFIDF params change
...
def prepare_evidence(
    ...
    v33.0.24 - Testing Dynamic Alpha (Thresh=0.55, TFIDF>0.5, Factor=0.1). Retains Specific Aircon Filter, NoExp, Manual Split, DampThresh=0.35, DampFactor=0.15.
    ...
    # --- START: Get parameters from Config or use constants ---
    ...
    logger.info(f"[{normalized_concept}] Implementing DYNAMIC ALPHA (SBERT<0.55 & TFIDF>0.5 => Alpha=0.1)")
    ...
    dampened_count = 0
    dynamic_alpha_applied_count = 0

    # --- Dynamic Alpha Parameters ---
    dynamic_alpha_sbert_threshold = 0.61
    dynamic_alpha_tfidf_threshold = 0.50
    dynamic_alpha_factor = 0.10
    ...
    for uri in all_uris_list:
        ...
        if boosted_s_score_for_calc < dynamic_alpha_sbert_threshold and k_score > dynamic_alpha_tfidf_threshold:
            effective_alpha = dynamic_alpha_factor # Reduce alpha significantly
        ...
        scored_list.append({
            ...
            "alpha_reduced": alpha_was_reduced,
    ...
    logger.info(f"[{normalized_concept}] Applied DYNAMIC ALPHA reduction (to {dynamic_alpha_factor:.2f}) for {dynamic_alpha_applied_count} candidates (SBERT<{dynamic_alpha_sbert_threshold:.2f} & TFIDF>{dynamic_alpha_tfidf_threshold:.2f}).")
    ...
    if args.debug:
        logger.debug(f"--- Top 5 Candidates for '{normalized_concept}' AFTER Sorting (DefaultAlpha={alpha}, DynamicAlphaFactor={dynamic_alpha_factor}, DampThresh={dampening_sbert_threshold}, DampFactor={dampening_factor}) ---")
...
# (apply_rules_and_finalize unchanged from v33.0.22)
...
def generate_affinity_definitions_loop(
    ...
    for concept in tqdm(concepts_subset, desc="Processing Concepts", disable=disable_tqdm):
        ...
        try:
            embed_text = f"{TRAVEL_CONTEXT}{norm_concept}"; logger.debug(f"[{norm_concept}] Embedding text: '{embed_text}'")
            ...
            stage2_out = apply_rules_and_finalize(
                ...
                orig_map, {}, args, _taxonomy_concepts_cache
            )
...
# --- Main Execution ---
# (main function unchanged from v33.0.22)
def main():
    ...
    logger.info(f"DYNAMIC ALPHA: Enabled (SBERT<0.55 & TFIDF>0.5 => Alpha=0.1)") # Note dynamic alpha
