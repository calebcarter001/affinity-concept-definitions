...
# -*- coding: utf-8 -*-
"""
Generate affinity definitions for travel concepts using combined scoring (v33.0.17).

Implements:
- more stuff implemented... didn't document
- Script version mentioned in docstring updated to v33.0.17
...
- LLM-assisted theme slotting with prompt enhancements (v33.0.17).
...
try:
    ...
    def tqdm(iterable, *args, **kwargs):
        return iterable  # Define dummy on the next line
...
try:
    ...
    logging.debug("Imported OpenAI library.")
except ImportError:
...
try:
    ...
    logging.debug("Imported google.generativeai library.")
except ImportError:
    logging.warning("google.generativeai library not found.")
except Exception as e:
...
# --- Config Defaults ---
SCRIPT_VERSION = "affinity-rule-engine-v33.0.16 (ABSOLUTE_MIN_SBERT_SCORE = 0.15 constant and Added loading of abs_min_sbert from stage1_cfg)"  # Updated version marker
...
DAMPENING_SBERT_THRESHOLD = EVIDENCE_MIN_SIMILARITY  # Default to using the SBERT evidence threshold
DAMPENING_FACTOR = 0.5  # Default to halving TF-IDF if SBERT is below threshold
...
def get_dynamic_theme_config(normalized_concept: str, theme_name: str, config: Dict) -> Tuple[
    str, float, Optional[str], Optional[Dict]]:
...
def deduplicate_attributes(attributes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for attr in attributes:
        ...
        try:
            current_weight = float(current.get("concept_weight", -1.0)) if current else -1.0
        except (ValueError, TypeError):
            current_weight = -1.0
...
def validate_llm_assignments(llm_response_data: Optional[Dict[str, Any]], uris_sent: Set[str], valid_themes: Set[str],
                             concept_label: str, diag_llm: Dict) -> Optional[Dict[str, List[str]]]:
    """Validates the structure and content of the LLM theme assignments."""
    if not llm_response_data or "theme_assignments" not in llm_response_data: logger.error(
        f"[{concept_label}] LLM response missing key."); diag_llm["error"] = "Missing 'theme_assignments'."; return None
    assigns = llm_response_data["theme_assignments"]
    if not isinstance(assigns, dict): logger.error(f"[{concept_label}] LLM assignments not dict."); diag_llm[
        "error"] = "Assignments structure not dict."; return None
    ...
    if missing: logger.warning(f"[{concept_label}] LLM response missing {len(missing)} URIs. Adding empty."); [
    ...
    for uri, themes in assigns.items():
        ...
        uris_proc += 1
        if not isinstance(themes, list): logger.warning(f"[{concept_label}] Invalid themes format for {uri}.");
        validated[uri] = []; continue
...
def build_keyword_index(
    ...
    use_tfidf = tfidf_cfg.get("enabled", False) and UTILS_SK_AVAILABLE  # Use flag from utils
    ...
    if use_tfidf:
        ...
        if not rebuild_cache:
            ...
            if cached_matrix is not None and cached_vec is not None and isinstance(cached_uris, list):
                ...
                logger.info("TF-IDF cache incomplete/invalid. Rebuilding.")

        if not cache_valid:
            logger.info("Rebuilding TF-IDF index...")
            try:
                docs, doc_uris = [], [];
                logger.info("Preparing docs for TF-IDF...")
                for uri, data in tqdm(sorted(taxonomy_concepts_cache.items()), desc="Prepare TF-IDF Docs"):
                    texts = [];
                    props = ["skos:prefLabel", "rdfs:label", "skos:altLabel", "skos:definition"]
                    for k in props: vals = data.get(k, []); texts.extend(
                        str(v) for v in (vals if isinstance(vals, list) else [vals]) if v)
                    norm_doc = normalize_concept(" ".join(texts))  # From utils
                    if norm_doc: docs.append(norm_doc); doc_uris.append(uri)
                if not docs:
                    logger.warning("No documents for TF-IDF build.")
                else:
                ...
                logger.error(f"TF-IDF build error: {e}",
                             exc_info=True); tfidf_matrix, vectorizer, corpus_uris = None, None, None
        ...
        if vectorizer and hasattr(vectorizer, 'vocabulary_'):
            ...
            logger.debug("Vocab: Not available.")
...
def get_candidate_concepts_keyword(
    ...
    logger.debug(f"TF-IDF search query: {query_texts[:5]}...")
    try:
        query_matrix = vectorizer.transform(query_texts);
        logger.debug(f"Query Matrix: {query_matrix.shape}")
        ...
        agg_scores = np.max(similarities, axis=0) if similarities.shape[0] > 1 else similarities[0] if \
        similarities.shape[0] == 1 else np.array([])
        if agg_scores.size == 0: logger.debug("Aggregated TF-IDF scores empty."); return []
        logger.debug(
            f"Agg Scores Shape: {agg_scores.shape}, Min: {np.min(agg_scores):.4f}, Max: {np.max(agg_scores):.4f}")
        num_cands = len(agg_scores);
        top_n = min(top_n, num_cands)
        if top_n <= 0: return []
        if top_n < num_cands // 2:
            idx_unsorted = np.argpartition(agg_scores, -top_n)[-top_n:]; indices = idx_unsorted[
                np.argsort(agg_scores[idx_unsorted])[::-1]]
        else:
            indices = np.argsort(agg_scores)[::-1][:top_n]
        ...
        for i in indices:
            ...
            if score >= min_score:
                candidates.append({"uri": corpus_uris[i], "score": score, "method": "keyword_tfidf"})
            else:
                filtered += 1;  # break # Optional early stop
        logger.debug(f"Filtered {filtered} TF-IDF candidates < {min_score}.")
        logger.info(
            f"TF-IDF search found {len(candidates)} candidates >= {min_score} ({time.time() - start_time:.2f}s).")
        return candidates
    except Exception as e:
        logger.error(f"TF-IDF search error: {e}", exc_info=True); return []
...
def get_openai_client() -> Optional[OpenAI_Type]:
    ...
    if _openai_client is None:
        try:
            ...
            logger.error(f"OpenAI client init failed: {e}"); return None
...
def get_google_client() -> Optional[Any]:
    ...
    if _google_client is None:
        try:
            ...
            logger.error(f"Google AI config failed: {e}"); return None
    return _google_client


def call_llm(system_prompt: str, user_prompt: str, model_name: str, timeout: int, max_retries: int, temperature: float,
    ...
    delay = _config_data.get("LLM_API_CONFIG", {}).get("RETRY_DELAY_SECONDS",
                                                       LLM_RETRY_DELAY_SECONDS) if _config_data else LLM_RETRY_DELAY_SECONDS
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if provider == "openai":
                ...
                gen_cfg = client.types.GenerationConfig(candidate_count=1, temperature=temperature,
                ...
                if resp.prompt_feedback and resp.prompt_feedback.block_reason: block_reason = getattr(
                    resp.prompt_feedback, 'block_reason', 'N/A'); logger.warning(f"Gemini blocked: {block_reason}.");
                result["error"] = f"Blocked: {block_reason}"; return result
                if resp.candidates:
                    content = resp.text
                else:
                    logger.warning("Gemini response no candidates."); content = None
            else:
                result["error"] = f"Unsupported provider: {provider}"; logger.error(result["error"]); return result

            if content:
                try:
                    json_match = re.search(r"```json\s*(\{.*?\})\s*```", content, re.DOTALL | re.IGNORECASE)
                    cleaned = json_match.group(1) if json_match else content[content.find('{'):content.rfind(
                        '}') + 1] if '{' in content else content.strip('`')
                    output = json.loads(cleaned)
                    if isinstance(output, dict):
                        result["success"] = True; result["response"] = output; logger.info(
                            f"{provider} OK ({time.time() - start:.2f}s)."); return result
                    else:
                        logger.error(f"{provider} response not dict. Type: {type(output)}.")
                except json.JSONDecodeError as e:
                    logger.error(f"{provider} JSON parse error: {e}. Raw: {content[:500]}..."); result[
                        "error"] = f"JSON Parse Error: {e}"
                except Exception as e:
                    logger.error(f"{provider} response processing error: {e}. Raw: {content[:500]}..."); result[
                        "error"] = f"Response Processing Error: {e}"
            else:
                logger.warning(f"{provider} response empty."); result["error"] = "Empty response from LLM"

        except (APITimeoutError, APIConnectionError, RateLimitError) as e:
            logger.warning(f"{provider} API Error: {type(e).__name__}"); result["error"] = f"{type(e).__name__}"
        except Exception as e:
            logger.error(f"{provider} Call Error: {e}", exc_info=True); result["error"] = str(e); return result

        should_retry = attempt < max_retries and (
                    "Error" in str(result.get("error")) or "Empty response" in str(result.get("error")))
...
def construct_llm_slotting_prompt(input_concept: str, theme_definitions: List[Dict[str, Any]],
                                  candidate_details: List[Dict[str, Any]], args: argparse.Namespace) -> Tuple[str, str]:
    ...
    theme_defs_str = "\n".join(
        [f"- **{t.get('name', 'N/A')}{' (Mandatory)' if t.get('is_must_have') else ''}**: {t.get('description', 'N/A')}"
...
def build_reprompt_prompt(input_concept: str, theme_name: str, theme_config: Dict,
                          original_candidates_details_map: Dict[str, Dict]) -> Tuple[str, str]:
    ...
    cand_list = "".join(
        f"\n{i + 1}. URI: {uri}\n   Label: \"{cand.get('prefLabel', 'N/A')}\"\n   Types: {', '.join(cand.get('type_labels', []))}"
        for i, (uri, cand) in enumerate(original_candidates_details_map.items())) or "\nNo candidates."
...
def expand_keywords_with_llm(concept_label: str, config: Dict, args: argparse.Namespace) -> List[str]:
    ...
    if result and result.get("success"):
        ...
        if isinstance(raw_phrases, list):
            ...
            logger.info(
                f"[{concept_label}] LLM generated {len(final_keyword_terms)} unique keyword terms from {len(raw_phrases)} phrases.")
        else:
...
# --- Updated Stage 1: Evidence Preparation (Combined Score + Abs SBERT Filter + Conditional Dampening + Aikido Seed + No Aikido Expansion) ---
def prepare_evidence(
    ...
    """
    Prepares evidence candidates using combined score, absolute SBERT filtering,
    conditional TF-IDF dampening, specific seeding for 'aikido', and disables
    LLM expansion *only* for 'aikido'.
    v33.0.18 (No Aikido Expansion) - Keeps Alpha=0.6, Seeds Aikido, Disables Aikido Expansion.
    ...
    logger.info(
        f"Starting evidence prep for: '{normalized_concept}' (Combined Score + Filters + Dampening + Aikido Seed + No Aikido Expansion)")  # Updated log message
    ...
    min_kw = float(stage1_cfg.get('KEYWORD_MIN_SCORE', KEYWORD_MIN_SCORE))
    ...
    # Keep Alpha=0.6 as per Run 5, explicitly check config value
    alpha = float(config.get('COMBINED_SCORE_ALPHA', COMBINED_SCORE_ALPHA));
    if abs(alpha - 0.6) > 1e-6: logger.warning(f"Alpha in config ({alpha}) differs from expected 0.6 for this run.")
    kw_exp_enabled = config.get("ENABLE_KW_EXPANSION", ENABLE_KW_EXPANSION) and args.llm_provider != "none"
    ...
    logger.info(f"Using Combined Score Alpha: {alpha}")  # Log the alpha being used
    logger.info(f"Using Absolute Min SBERT Threshold: {abs_min_sbert}")
    logger.info(f"Using Conditional Dampening: Threshold={dampening_sbert_threshold}, Factor={dampening_factor}")

    # --- Keyword Preparation (Mandatory Split + Optional Expansion, disabled for aikido) ---
    ...
    # <<< START CODE MODIFICATION FOR NO AIKIDO EXPANSION >>>
    # Check if expansion is generally enabled AND if the concept meets the criteria AND if it's NOT 'aikido'
    needs_exp = kw_exp_enabled and \
                (initial_kw_count < kw_trigger or normalized_concept in ABSTRACT_CONCEPTS_LIST) and \
                normalized_concept != 'aikido'
    ...
    if needs_exp:
        ...
        logger.info(
            f"Attempting LLM keyword expansion for '{normalized_concept}' (Trigger: initial matches {initial_kw_count} < {kw_trigger} or abstract concept, and NOT 'aikido').")
        try:
        ...
        if kw_exp_enabled and normalized_concept == 'aikido':
            logger.info(f"Skipping LLM KW expansion specifically for '{normalized_concept}'.")
            exp_diag["attempted"] = True  # Mark as attempted but skipped due to rule
            exp_diag["successful"] = False
            exp_diag["error"] = "Skipped for 'aikido'"
        elif not kw_exp_enabled:
            logger.info(f"Skipping LLM KW expansion for '{normalized_concept}' (Globally disabled or provider='none').")
        else:
            logger.info(
                f"Skipping LLM KW expansion for '{normalized_concept}' (Initial matches {initial_kw_count} >= {kw_trigger} and not abstract).")

    final_query_texts = list(query_texts_set)
    exp_diag["count"] = len(final_query_texts)
    exp_diag["terms"] = final_query_texts
    logger.info(
        f"Using {len(final_query_texts)} unique keywords for TF-IDF search: {final_query_texts if args.debug else '...'}")
    ...
    if tfidf_vectorizer and tfidf_matrix is not None and tfidf_corpus_uris:
        if final_query_texts:
            kw_cands = get_candidate_concepts_keyword(final_query_texts, tfidf_vectorizer, tfidf_matrix,
                                                      tfidf_corpus_uris, kw_top_n, min_kw)
    ...
    initial_unique_count = len(all_uris_set)  # Count before seeding

    # <<< START AIKIDO SEEDING LOGIC (Unchanged from v33.0.17) >>>
    if normalized_concept == 'aikido':
        seed_uris = {
            "urn:expediagroup:taxonomies:activities:#c7b903d8-14be-4bcd-bcb3-d6d35724e7cc",  # Martial Arts classes
            "urn:expediagroup:taxonomies:core:#f02b51fe-eabe-4dab-961a-456bed9664e8"  # Martial Arts
        }
        added_seeds = seed_uris - all_uris_set  # Find seeds not already present
        if added_seeds:
            logger.info(f"SEEDING 'aikido' with {len(added_seeds)} missing relevant URIs: {added_seeds}")
            all_uris_set.update(added_seeds)
        else:
            logger.info(f"Seed URIs for 'aikido' were already present in initial candidates.")
    # <<< END AIKIDO SEEDING LOGIC >>>

    all_uris_list = list(all_uris_set)  # Convert back to list for fetching details
    unique_candidates_before_ranking = len(all_uris_list)  # Final count including seeds
    if initial_unique_count != unique_candidates_before_ranking:
        logger.info(
            f"Total unique candidates increased from {initial_unique_count} to {unique_candidates_before_ranking} after seeding.")
    ...
    logger.info(f"Processing {unique_candidates_before_ranking} unique candidates from TF-IDF, SBERT, and seeds.")
    all_details = get_kg_data(all_uris_list, taxonomy_concepts_cache)  # Fetch details for potentially newly added seeds

    scored_list: List[Dict] = []
    abs_sbert_filtered_count = 0
    dampened_count = 0

    for uri in all_uris_list:  # Iterate over the potentially seeded list
        ...
        # Fetch SBERT score if seed was added and not in original sim_scores
        if uri not in sim_scores and normalized_concept == 'aikido' and uri in seed_uris:
            if uri in primary_embeddings and concept_embedding is not None:
                sim_result = get_batch_embedding_similarity(concept_embedding, {uri: primary_embeddings[uri]})
                s_score = sim_result.get(uri, 0.0)
                sim_scores[uri] = s_score  # Add to dict for consistency
                logger.info(f"Calculated SBERT score for seeded URI {uri}: {s_score:.4f}")
            else:
                logger.warning(f"Could not calculate SBERT for seeded URI {uri}")
                s_score = 0.0  # Treat as 0 if embedding unavailable

        # ** Absolute SBERT Filter **
        if s_score < abs_min_sbert:
            if args.debug:
                logger.debug(
                    f"Excluding candidate {uri} ('{all_details[uri].get('prefLabel', 'N/A')}') due to SBERT score {s_score:.4f} < {abs_min_sbert:.4f}. (TFIDF: {k_score:.4f})")
            abs_sbert_filtered_count += 1
            continue

        # *** Conditional TF-IDF Dampening (Factor = 0.25 from config) ***
        dampened_k_score = k_score
        if s_score < dampening_sbert_threshold:
            dampened_k_score *= dampening_factor  # Apply stronger dampening
            if k_score > 0:  # Only log if dampening actually happened
                if args.debug: logger.debug(
                    f"Dampening TF-IDF for {uri} ('{all_details[uri].get('prefLabel', 'N/A')}'): SBERT={s_score:.4f} < {dampening_sbert_threshold:.4f}. TF-IDF reduced from {k_score:.4f} to {dampened_k_score:.4f}.")
                dampened_count += 1
        # *** END Dampening ***

        # Normalize scores (using potentially dampened TF-IDF score)
        norm_s = max(0.0, min(1.0, s_score))
        norm_k = max(0.0, min(1.0, dampened_k_score))  # Use dampened score here

        # Calculate combined score (Alpha = 0.6 from config)
        combined = (alpha * norm_k) + ((1.0 - alpha) * norm_s)

        scored_list.append({
            "uri": uri,
            "details": all_details[uri],
            "sim_score": s_score,  # Store original SBERT
            "kw_score": k_score,  # Store original TF-IDF
            "dampened_kw_score": dampened_k_score if dampened_k_score != k_score else None,
            # Store dampened score if different
            "combined_score": combined
        })

    logger.info(f"Excluded {abs_sbert_filtered_count} candidates with SBERT score < {abs_min_sbert}.")
    logger.info(
        f"Conditionally dampened TF-IDF score (Factor={dampening_factor}) for {dampened_count} candidates with SBERT score < {dampening_sbert_threshold}.")

    if not scored_list:
        logger.warning(f"No candidates remaining after filtering/dampening for '{normalized_concept}'.")
    ...
    logger.info(
        f"Selected top {len(selected)}/{len(scored_list)} candidates based on combined score (alpha={alpha}) after filtering/dampening.")

    if not selected:
        logger.warning(f"No candidates selected after scoring and filtering/dampening for '{normalized_concept}'.")
        return [], {}, None, {}, exp_diag, sbert_candidate_count_initial, tfidf_candidate_count_initial, unique_candidates_before_ranking

    # Prepare outputs
    llm_details = [c["details"] for c in selected]
    # Include all original and combined scores in the map for analysis/downstream use
    orig_map = {c['uri']: {**c["details"], "sbert_score": c["sim_score"], "keyword_score": c["kw_score"],
                           "combined_score": c["combined_score"]} for c in selected}
    anchor_data = selected[0]
    anchor = orig_map.get(anchor_data['uri'])
    if anchor:
        # Log the anchor and its *original* scores for clarity, plus dampened if applicable
        damp_info = f"(Dampened TFIDF: {anchor_data['dampened_kw_score']:.4f})" if anchor_data.get(
            'dampened_kw_score') is not None else ""
        logger.info(
            f"Anchor selected: {anchor.get('prefLabel', anchor['uri'])} (Combined: {anchor_data['combined_score']:.4f}, SBERT: {anchor_data['sim_score']:.4f}, TFIDF: {anchor_data['kw_score']:.4f} {damp_info})")
    else:
...
def apply_rules_and_finalize(
        ...
        # SBERT scores (retained for potential future use, but NOT used for weighting now)
    ...
    logger.info(f"Starting finalization for: {norm_concept}")
    output: Dict[str, Any] = {"applicable_lodging_types": "Both",
                              "travel_category": travel_category or {"uri": None, "name": input_concept,
                                                                     "type": "Unknown"}, "top_defining_attributes": [],
                              "themes": [], "additional_relevant_subscores": [], "must_not_have": [],
                              "failed_fallback_themes": {}, "diagnostics": {"theme_processing": {},
                                                                            "reprompting_fallback": {"attempts": 0,
                                                                                                     "successes": 0,
                                                                                                     "failures": 0},
                                                                            "final_output": {},
                                                                            "stage2": {"status": "Started",
    ...
    min_weight = float(final_cfg.get('THEME_ATTRIBUTE_MIN_WEIGHT', THEME_ATTRIBUTE_MIN_WEIGHT));
    top_n_attrs = int(final_cfg.get('TOP_N_DEFINING_ATTRIBUTES', TOP_N_DEFINING_ATTRIBUTES_DEFAULT))

    llm_assigns: Dict[str, List[str]] = {};
    diag_val = {}
    if llm_call_result and llm_call_result.get("success"):
        validated = validate_llm_assignments(llm_call_result.get("response"),
                                             set(original_candidates_map_for_reprompt.keys()), set(base_themes.keys()),
                                             norm_concept, diag_val)
        if validated is not None:
            llm_assigns = validated; logger.debug(f"Validated LLM assignments.")
        else:
            logger.warning(f"LLM validation failed."); output["diagnostics"]["stage2"]["error"] = diag_val.get("error",
                                                                                                               "LLM Validation Failed")
    elif llm_call_result:
        logger.warning(f"LLM call unsuccessful: {llm_call_result.get('error')}"); output["diagnostics"]["stage2"][
            "error"] = f"LLM Call Failed: {llm_call_result.get('error')}"
    else:
        logger.info(f"No LLM result.")
    ...
    for name in base_themes.keys():
        diag = theme_diag[name] = {"llm_assigned_count": len(theme_map.get(name, [])), "attributes_after_weighting": 0,
                                   "status": "Pending", "rule_failed": False}
        rule, _, _, _ = get_dynamic_theme_config(norm_concept, name, config)
        if rule == "Must have 1" and not theme_map.get(name): logger.warning(f"Rule FAILED: '{name}'"); failed_rules[
            name] = {"reason": "No assigns."}; diag.update({"status": "Failed Rule", "rule_failed": True})
    ...
    if failed_rules and original_candidates_map_for_reprompt and args.llm_provider != "none":
        ...
        for name in list(failed_rules.keys()):
            ...
            fb_result = call_llm(sys_p, user_p, args.llm_model, LLM_TIMEOUT, LLM_MAX_RETRIES, LLM_TEMPERATURE,
                                 args.llm_provider)
            if fb_result and fb_result.get("success"):
                fb_assigns = fb_result.get("response", {}).get("theme_assignments", {});
                new_uris = set(uri for uri, ts in fb_assigns.items() if isinstance(ts,
                                                                                   list) and name in ts and uri in original_candidates_map_for_reprompt) if isinstance(
                    fb_assigns, dict) else set()
                if new_uris:
                    logger.info(f"Fallback SUCCESS for '{name}': Assigned {len(new_uris)} URIs.");
                    reprompt_diag["successes"] += 1;
                    fixed.add(name);
                    added = 0
                    for uri in new_uris:
                        if uri not in theme_map.get(name, []): theme_map[name].append(uri); fallback_adds.append(
                            {"uri": uri, "assigned_theme": name}); added += 1
                    logger.debug(f"Added {added} unique URIs via fallback for '{name}'.")
                    if name in theme_diag: theme_diag[name].update(
                        {"status": "Passed (Fallback)", "rule_failed": False})
                else:
                    logger.warning(f"Fallback LLM for '{name}' assigned 0."); reprompt_diag["failures"] += 1;
                    theme_diag[name]["status"] = "Failed (Fallback - No Assigns)"
            else:
                err = fb_result.get("error", "Unknown") if fb_result else "None"; logger.error(
                    f"Fallback LLM failed for '{name}'. Error: {err}"); reprompt_diag["failures"] += 1;
                theme_diag[name]["status"] = "Failed (Fallback - API Error)"
    elif failed_rules:
        logger.warning(f"Cannot attempt fallback, LLM provider is 'none'.")

    final_themes_out = [];
    all_final_attrs = []
    theme_w_cfg = {n: get_dynamic_theme_config(norm_concept, n, config)[1] for n in base_themes.keys()};
    norm_w = normalize_weights(theme_w_cfg)

    for name, base_data in base_themes.items():
        diag = theme_diag[name];
        rule, _, subscore, _ = get_dynamic_theme_config(norm_concept, name, config)
        theme_w = norm_w.get(name, 0.0);
        ...
        logger.debug(
            f"[{norm_concept}][{name}] URIs:{len(uris)}, Combined Score Sum:{total_score:.4f}, Theme Weight:{theme_w:.4f}")
        # *** MODIFICATION END ***
            ...
            if total_score < 1e-9 and n_uris > 0:
                ...
                if eq_w >= min_weight:
                    for u in uris:
                        ...
                        attr = {"uri": u, "skos:prefLabel": get_primary_label(u, taxonomy_concepts_cache, u),
                                "concept_weight": round(eq_w, 6), "type": d.get('type_labels', [])};  # Use util
                        if is_fb: attr["comment"] = "Fallback"
                        theme_attrs.append(attr);
                        all_final_attrs.append(attr)
                else:
                    logger.warning(f"Equal weight {eq_w:.6f} < {min_weight} for '{name}', skipping attributes.")
                ...
                for u in uris:
                    ...
                    if attr_w >= min_weight:
                        is_fb = any(f["uri"] == u and f["assigned_theme"] == name for f in fallback_adds)
                        attr = {"uri": u, "skos:prefLabel": get_primary_label(u, taxonomy_concepts_cache, u),
                                "concept_weight": round(attr_w, 6), "type": d.get('type_labels', [])};  # Use util
                        if is_fb: attr["comment"] = "Fallback"
                        theme_attrs.append(attr);
                        all_final_attrs.append(attr)

        theme_attrs.sort(key=lambda x: x['concept_weight'], reverse=True);
        diag["attributes_after_weighting"] = len(theme_attrs)
        if diag["status"] == "Pending": diag["status"] = "Processed (Initial)" if not diag.get("rule_failed") else diag[
            "status"]
        final_themes_out.append(
            {"theme_name": name, "theme_type": base_data.get("type", "unknown"), "rule_applied": rule,
             "normalized_theme_weight": round(theme_w, 6), "subScore": subscore or f"{name}Affinity",
             "llm_summary": None, "attributes": theme_attrs})
    ...
    for attr in all_final_attrs:
        ...
        if uri not in unique_attrs or current_weight > stored_weight:
            unique_attrs[uri] = {k: v for k, v in attr.items() if
                                 k != 'comment'}  # Exclude comment field from top attributes
    ...
    if isinstance(tc, dict):
        tc["type"] = overrides.get("category_type", tc.get("type", "Uncategorized")); tc[
            "exclusionary_concepts"] = overrides.get("exclusionary_concepts", [])
    else:
        logger.error(f"travel_category invalid?"); output["travel_category"] = {"uri": None, "name": norm_concept,
                                                                                "type": overrides.get("category_type",
                                                                                                      "Uncategorized"),
                                                                                "exclusionary_concepts": overrides.get(
                                                                                    "exclusionary_concepts", [])}
    mnh = overrides.get("must_not_have", []);
    mnh_uris = set(i["uri"] for i in mnh if isinstance(i, dict) and "uri" in i) if isinstance(mnh, list) else set()
    output["must_not_have"] = [
        {"uri": u, "skos:prefLabel": get_primary_label(u, taxonomy_concepts_cache, u), "scope": None} for u in
        sorted(list(mnh_uris))]  # Pass cache
    add_scores = overrides.get("additional_subscores", []);
    output["additional_relevant_subscores"] = add_scores if isinstance(add_scores, list) else []

    # --- Final Diagnostic Counts ---
    final_diag = output["diagnostics"]["final_output"]
    final_diag["must_not_have_count"] = len(output["must_not_have"]);
    final_diag["additional_subscores_count"] = len(output["additional_relevant_subscores"])
    final_diag["themes_count"] = len(output["themes"]);
    output["failed_fallback_themes"] = {n: r for n, r in failed_rules.items() if n not in fixed}
    ...
    output["diagnostics"]["stage2"]["duration_seconds"] = round(time.time() - start, 2)
...
def generate_affinity_definitions_loop(
    ...
    """ Main loop processing each concept (v33.0.16 DAMPENING_SBERT_THRESHOLD and DAMPENING_FACTOR constants with defaults). """  # Updated version marker
    ...
    logger.info(f"Processing {len(concepts_subset)} concepts.")
    ...
    for concept in tqdm(concepts_subset, desc="Processing Concepts", disable=disable_tqdm):
        ...
        try:
            ...
            embed_text = f"{TRAVEL_CONTEXT}{norm_concept}";
            logger.debug(f"Embedding: '{embed_text}'")
            ...
            diag1.update({
                "status": "Completed" if diag1.get("error") is None else "Failed",
                # Update status based on embedding success/failure
            ...
            logger.info(
                f"Stage 1 done ({stage1_dur:.2f}s). LLM Cands: {len(cand_details)}. Initial SBERT: {sbert_init_count}, TFIDF: {tfidf_init_count}.")

            # Set travel category based on anchor
            affinity_def["travel_category"] = anchor if anchor and anchor.get('uri') else {"uri": None, "name": concept,
                                                                                           "type": "Unknown"}
            ...
            llm_result = None
            llm_start = time.time()
            diag_llm.update({"llm_provider": args.llm_provider,
                             "llm_model": args.llm_model if args.llm_provider != "none" else None})
            ...
            if not cand_details:
                ...
                llm_result = call_llm(sys_p, user_p, args.llm_model, LLM_TIMEOUT, LLM_MAX_RETRIES, LLM_TEMPERATURE,
                                      args.llm_provider)
                ...
                if llm_result and llm_result.get("success"):
                    ...
                    diag_llm["error"] = llm_result.get("error",
                                                       "Unknown Error") if llm_result else "LLM Call resulted in None"
            ...
            if "diagnostics" in stage2_out:
                ...
                affinity_def["diagnostics"]["reprompting_fallback"].update(
                    s2d.get("reprompting_fallback", {}))  # Use update to merge counts
                affinity_def["diagnostics"]["final_output"] = s2d.get("final_output", {})
                affinity_def["diagnostics"]["stage2"] = s2d.get("stage2",
                                                                {"status": "Unknown"})  # Ensure stage2 dict exists
            else:
            ...
            logger.info(
                f"Stage 2 finalization completed in {affinity_def['diagnostics']['stage2']['duration_seconds']:.2f}s.")
            ...
            if final_status == "Started":  # Only update if not already set to a warning/error state earlier
                if affinity_def.get("failed_fallback_themes"):
                    ...
                    final_status = f"Warning - Finalization Error"
                elif diag1["status"] == "Failed":
                    final_status = "Failed - Stage 1 Error"  # Check if Stage 1 failed
                else:
            ...
            if diag_llm["status"] not in ["Completed", "Failed", "Skipped (No Candidates)", "Skipped (Provider None)"]:
                diag_llm["status"] = "Failed (Exception)"
            if affinity_def["diagnostics"]["stage2"]["status"] not in ["Completed", "Failed"]:
...
def main():
    ...
    parser.add_argument("-t", "--taxonomy-dir", dest="taxonomy_dir", type=str, default=DEFAULT_TAXONOMY_DIR,
                        help="Taxonomy RDF directory.")
    ...
    parser.add_argument("--llm-provider", type=str, choices=['openai', 'google', 'none'], default=None,
    ...
    setup_logging(log_level, log_file, args.debug)  # Util
    ...
    if args.llm_provider is None:
        args.llm_provider = config.get('LLM_PROVIDER', 'none')
    else:
        logger.warning(f"Override LLM provider: '{args.llm_provider}'")
    if args.llm_model is None:
        args.llm_model = config.get('LLM_MODEL', None)
    else:
        logger.warning(f"Override LLM model: '{args.llm_model}'")
    ...
    try:
        ...
        logger.critical(f"Read input file failed: {e}"); sys.exit(1)

    concepts_cache_f = get_cache_filename('concepts', cache_ver, args.cache_dir, extension=".json")  # Util
    concepts_data = load_taxonomy_concepts(args.taxonomy_dir, concepts_cache_f, args.rebuild_cache, cache_ver,
                                           args.debug)  # Util
    if concepts_data is None: logger.critical("Taxonomy load failed."); sys.exit(1)
    _taxonomy_concepts_cache = concepts_data;
    logger.info(f"Taxonomy concepts ready ({len(_taxonomy_concepts_cache)}).");
    print(f">>> DEBUG: Taxonomy concepts ready.")
    ...
    try:  # Build indexes
        ...
        if _tfidf_vectorizer is not None:
            logger.info(f"TF-IDF Index ready.")
        elif config.get("TFIDF_CONFIG", {}).get("enabled", False):
            logger.warning("TF-IDF failed but enabled.")
        if _keyword_label_index is not None:
            logger.info(f"Simple Label Index ready.")
        else:
            logger.warning("Simple Label Index failed.")
    except Exception as e:
        logger.critical(f"Index building failed: {e}", exc_info=True); sys.exit(1)

    logger.info("Loading SBERT model...");
    print(">>> DEBUG: Loading SBERT model...")
    try:
        sbert_name = config.get("SBERT_MODEL_NAME");
        sbert_model = get_sbert_model(sbert_name)  # Util
        if sbert_model is None: raise RuntimeError("SBERT model load failed")
        logger.info(
            f"SBERT model load successful. Dimension: {sbert_model.get_sentence_embedding_dimension()}")  # Corrected log
        print(">>> DEBUG: SBERT model loaded.")
    except Exception as e:
        logger.critical(f"SBERT load failed: {e}", exc_info=True); sys.exit(1)
    ...
    embed_data = precompute_taxonomy_embeddings(_taxonomy_concepts_cache, sbert_model, embed_cache_f, cache_ver,
                                                args.rebuild_cache, args.debug)  # Util
    ...
    print(f">>> DEBUG: Embeddings ready.")
    ...
    results = generate_affinity_definitions_loop(input_concepts, config, args, sbert_model, primary_embs,
                                                 _taxonomy_concepts_cache, _keyword_label_index, _tfidf_vectorizer,
                                                 _tfidf_matrix, _tfidf_corpus_uris)  # Local loop
    print(f">>> DEBUG: Returned from loop. Result count: {len(results)}")  # Check after return
    end_loop = time.time();
    logger.info(f"Generation loop finished ({end_loop - start_loop:.2f}s).")

    if results:
        save_results_json(results, out_file)  # Util
    else:
...
if __name__ == "__main__":
    ...
    if not all([UTILS_ST_AVAILABLE, UTILS_SK_AVAILABLE, RDFLIB_AVAILABLE]):
        print("CRITICAL: Required libs unavailable.", file=sys.stderr);
        sys.exit(1)
