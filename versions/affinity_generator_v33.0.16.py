...
# --- Updated Stage 1: Evidence Preparation (Combined Score + Abs SBERT Filter + Conditional Dampening) ---
# --- Updated Stage 1: Evidence Preparation (Combined Score + Abs SBERT Filter + Conditional Dampening + Aikido Seeding) ---
def prepare_evidence(
    input_concept: str, concept_embedding: Optional[np.ndarray], primary_embeddings: Dict[str, np.ndarray], # Correct parameter name
    ...
    conditional TF-IDF dampening, and specific seeding for 'aikido'.
    v33.0.17 (Seed Aikido) - Added seeding logic for 'aikido'. Expansion enabled by default.
    ...
    """
    normalized_concept = normalize_concept(input_concept); logger.info(f"Starting evidence prep for: '{normalized_concept}' (Combined Scoring + Filters + Dampening + Aikido Seed)")
    ...
    alpha = float(config.get('COMBINED_SCORE_ALPHA', COMBINED_SCORE_ALPHA)); # Alpha now expected to be 0.5 from config
    ...
    expanded_kws = set()
    if needs_exp:
        # NOTE: Expansion is now ENABLED for aikido if it meets the criteria (unlike Run 4)
        ...
        try:
            expanded_terms_list = expand_keywords_with_llm(input_concept, config, args) # Calls function which now needs the stricter prompt
            # Process phrases: normalize and split into terms
        ...
        logger.info(f"Skipping LLM KW expansion for '{normalized_concept}'.")

    final_query_texts = list(query_texts_set)
    exp_diag["count"] = len(final_query_texts)
    exp_diag["terms"] = final_query_texts # No longer hiding terms if debug is off, useful for diagnostics
    ...
    if concept_embedding is None:
        ...
        sim_scores = get_batch_embedding_similarity(concept_embedding, primary_embeddings) # Use the correct parameter name 'primary_embeddings'
    ...
    # <<< START CODE MODIFICATION FOR SEEDING >>>
    ...
    if initial_unique_count != unique_candidates_before_ranking:
        logger.info(f"Total unique candidates increased from {initial_unique_count} to {unique_candidates_before_ranking} after seeding.")
    ...
    for uri in all_uris_list: # Iterate over the potentially seeded list
        ...
        if uri not in sim_scores and normalized_concept == 'aikido' and uri in seed_uris:
             # ***** CORRECTION HERE *****
            if uri in primary_embeddings and concept_embedding is not None: # Use 'primary_embeddings'
                 sim_result = get_batch_embedding_similarity(concept_embedding, {uri: primary_embeddings[uri]}) # Use 'primary_embeddings'
             # ***** END CORRECTION *****
        ...
        if s_score < dampening_sbert_threshold:
            ...
            if k_score > 0: # Only log if dampening actually happened
                 if args.debug: logger.debug(f"Dampening TF-IDF for {uri} ('{all_details[uri].get('prefLabel', 'N/A')}'): SBERT={s_score:.4f} < {dampening_sbert_threshold:.4f}. TF-IDF reduced from {k_score:.4f} to {dampened_k_score:.4f}.")
                 dampened_count += 1
        # *** END Dampening ***
        ...
        # Calculate combined score (Alpha = 0.5 from config)
        ...
        scored_list.append({
            ...
            "dampened_kw_score": dampened_k_score if dampened_k_score != k_score else None, # Store dampened score if different
    ...
    if anchor:
        ...
        damp_info = f"(Dampened TFIDF: {anchor_data['dampened_kw_score']:.4f})" if anchor_data.get('dampened_kw_score') is not None else ""
        logger.info(f"Anchor selected: {anchor.get('prefLabel', anchor['uri'])} (Combined: {anchor_data['combined_score']:.4f}, SBERT: {anchor_data['sim_score']:.4f}, TFIDF: {anchor_data['kw_score']:.4f} {damp_info})")
    else:
