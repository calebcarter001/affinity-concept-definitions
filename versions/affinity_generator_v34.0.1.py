# -*- coding: utf-8 -*-
"""
Generate affinity definitions for travel concepts using combined scoring (v34.0.1 - BM25 + ACS Enrichment - Refactored Config).

Implements:
- Okapi BM25 for keyword relevance scoring.
- Enriches BM25 document construction using data from an external ACS tracker CSV file.
...
Changes from v34.0.0:
- MAJOR REFACTOR: Centralized all default configuration into the DEFAULT_CONFIG dictionary.
- Removed redundant global constants (e.g., LLM_TIMEOUT, COMBINED_SCORE_ALPHA).
- Ensured all functions consistently read parameters from the merged config object (_config_data).
- Fixed inconsistencies in parameter usage (e.g., fallback LLM call in apply_rules_and_finalize).
- **FIXED:** Defined missing LOG_FILE_TEMPLATE and OUTPUT_FILE_TEMPLATE constants.
- **FIXED:** Corrected variable name mismatch for sbert/keyword initial counts in main loop.
- Updated SCRIPT_VERSION and related logs.
- Requires `pandas`, `rank-bm25` libraries if respective features enabled in config.
"""
...
SCRIPT_VERSION = "affinity-rule-engine-v34.0.1 (BM25 + ACS Enrichment - Refactored Config)"
DEFAULT_CACHE_VERSION = "v20250420.affinity.34.0.1"
DEFAULT_CONFIG_FILE = "./affinity_config_v34.1.json"
# ***** ADDED MISSING TEMPLATE DEFINITIONS *****
...
DEFAULT_CONFIG = {
    ...
    "ACS_DATA_CONFIG": {
        "acs_data_path": "/datasources/transformed_acs_tracker.csv", # Default path
...
def validate_llm_assignments(llm_response_data: Optional[Dict[str, Any]], uris_sent: Set[str], valid_themes: Set[str], concept_label: str, diag_llm: Dict) -> Optional[Dict[str, List[str]]]:
     ...
     if not llm_response_data or "theme_assignments" not in llm_response_data: logger.error(f"[{concept_label}] LLM response missing 'theme_assignments'."); diag_llm["error"] = "Missing 'theme_assignments'."; return None
     assigns = llm_response_data["theme_assignments"]
     if not isinstance(assigns, dict): logger.error(f"[{concept_label}] LLM assignments not dict."); diag_llm["error"] = "Assignments not dict."; return None
     ...
     for uri, themes in assigns.items():
         ...
         uris_proc += 1
         if not isinstance(themes, list): logger.warning(f"[{concept_label}] Invalid themes format for {uri}. Setting empty."); validated[uri] = []; continue
...
# --- START: Keyword Indexing Function (v34.0.1 - Handles Duplicate URIs in ACS) ---
def build_keyword_index(
    ...
    v34.0.1: Reads config, incorporates ACS enrichment, handles duplicate URIs in ACS lookup.
    """
    ...
    definition_weight = int(kg_cfg.get("definition_weight", 1))
            ...
            try:
                ...
                for uri, data in tqdm(sorted(taxonomy_concepts_cache.items()), desc="Prepare BM25 Docs", disable=disable_tqdm):
                    ...
                    if enrichment_enabled and uri in _acs_data.index:
                        try:
                            # Lookup potentially returning Series or string
                            acs_name_lookup = _acs_data.loc[uri, 'AttributeNameENG']
                            acs_def_lookup = _acs_data.loc[uri, 'ACS Definition']

                            # Handle acs_name lookup result
                            if isinstance(acs_name_lookup, pd.Series):
                                if not acs_name_lookup.empty:
                                    acs_name = str(acs_name_lookup.iloc[0]) # Ensure string type
                                    if len(acs_name_lookup) > 1 and args.debug:
                                        logger.debug(f"Duplicate URI '{uri}'. Using first 'AttributeNameENG'.")
                                else: acs_name = ''
                            elif pd.notna(acs_name_lookup) and isinstance(acs_name_lookup, str): acs_name = acs_name_lookup
                            else: acs_name = ''

                            # Handle acs_def lookup result
                            if isinstance(acs_def_lookup, pd.Series):
                                if not acs_def_lookup.empty:
                                    acs_def = str(acs_def_lookup.iloc[0]) # Ensure string type
                                    if len(acs_def_lookup) > 1 and args.debug:
                                        logger.debug(f"Duplicate URI '{uri}'. Using first 'ACS Definition'.")
                                else: acs_def = ''
                            elif pd.notna(acs_def_lookup) and isinstance(acs_def_lookup, str): acs_def = acs_def_lookup
                            else: acs_def = ''

                            # Check if enrichment actually yielded text
                            if acs_name or acs_def:
                                was_enriched = True
                                enriched_count += 1
                                # Apply weights only if text exists and weight > 0
                                ...
                                was_enriched = False # No actual text added

                        except Exception as acs_lookup_err:
                             # Log specific error during lookup for this URI
                             logger.warning(f"Error enriching URI '{uri}': {type(acs_lookup_err).__name__}: {acs_lookup_err}.")
                             was_enriched = False # Ensure enrichment status is false on error
                    # --- END ACS Enrichment ---
                    ...
                    if uri in uris_to_log_bm25_doc and args.debug:
                        ...
                        logger.debug(f"[BM25 DOC LOG{enrich_status}] URI: {uri} Label: '{label}' Raw: '{raw_snippet}' Norm: '{norm_snippet}'")
        ...
        try:
            ...
            if provider == "openai":
                ...
                if resp.prompt_feedback and resp.prompt_feedback.block_reason:
                    ...
                    logger.warning(f"Gemini blocked. Reason: {block_reason}. System: '{system_prompt[:100]}...', User: '{user_prompt[:100]}...'")
...
def construct_keyword_expansion_prompt(input_concept: str) -> Tuple[str, str]:
    """Creates prompts for Stage 1 LLM keyword expansion."""
...
def construct_llm_slotting_prompt(input_concept: str, theme_definitions: List[Dict[str, Any]], candidate_details: List[Dict[str, Any]], args: argparse.Namespace) -> Tuple[str, str]:
    """Constructs the main LLM prompt for theme slotting."""
    system_prompt = """You are an expert travel taxonomist creating structured definitions. Given a core concept, semantic themes, and candidate concepts, analyze EACH candidate and determine relevant themes based on labels, definitions, and types. Output ONLY a valid JSON object with key "theme_assignments". Value is a dictionary mapping EACH candidate URI to a list of relevant theme names (empty list if none). Ensure ALL input URIs are keys in the output. Focus solely on semantic relevance."""
    ...
    schema_example = '```json\n{\n  "theme_assignments": {\n    "URI_1": ["ThemeA", "ThemeB"],\n    "URI_2": [],\n    "URI_3": ["ThemeC"]\n    // ... include ALL URIs ...\n  }\n}\n```'
    task = f"""Task: For EACH candidate URI below, determine relevant themes. Ensure candidates matching '{input_concept}' are slotted appropriately. Ensure EVERY URI is in the output JSON. Focus on semantic relevance."""
    user_prompt = f"""Analyze concept: '{input_concept}'\nAvailable Themes:\n{theme_defs_str}\nMandatory Themes: [{must_haves}]\nCandidates:\n{cand_details_str}\n{task}\nOutput Format: Output ONLY the JSON strictly matching:\n{schema_example}"""
    return system_prompt.strip(), user_prompt.strip()
...
def build_reprompt_prompt(input_concept: str, theme_name: str, theme_config: Dict, original_candidates_details_map: Dict[str, Dict]) -> Tuple[str, str]:
    ...
    system_prompt = """You are assisting in refining travel definitions. A mandatory theme needs assignments. Review the original candidates ONLY for relevance to the specific theme. Output ONLY valid JSON like {"theme_assignments": {"URI_Relevant_1": ["ThemeName"], ...}}. If none relevant, output {"theme_assignments": {}}."""
    ...
    user_prompt = f"""Re-evaluating '{input_concept}' for MANDATORY theme '{theme_name}'.\nTheme Definition:\n- Name: {theme_name}\n- Description: {desc}\n{hints_str}\nOriginal Candidates:\n{cand_list}\nInstructions: Identify ONLY candidates relevant to '{theme_name}'. Assign at least one if plausible. Output ONLY valid JSON matching the schema.\nExample Output Schema:\n{schema}\nYour Output:"""
    return system_prompt.strip(), user_prompt.strip()


# --- Keyword Expansion Helper (v34.0.1 - Reads config) ---
# (expand_keywords_with_llm - unchanged from previous correct version)
    ...
    try:
        ...
        if result and result.get("success"):
            ...
            if isinstance(raw_phrases, list):
                added_terms = set(term for kw in raw_phrases if isinstance(kw, str) and kw.strip() for term in normalize_concept(kw).split() if len(term) > 2)
                newly_expanded_kws = added_terms - original_normalized_words
                if newly_expanded_kws: final_keyword_terms.update(newly_expanded_kws); success = True; logger.debug(f"[{concept_label}] LLM generated {len(final_keyword_terms)} terms ({len(newly_expanded_kws)} new).")
                else: error_message = "LLM returned no new useful terms"; logger.info(f"[{concept_label}] LLM expansion gave no new terms.")
            else: error_message = "LLM response format invalid"; logger.error(f"[{concept_label}] LLM expansion 'keywords' not list.")
        else: err_msg = result.get('error', 'Unknown') if result else 'None result'; error_message = f"LLM API Call Failed: {err_msg}"; logger.warning(f"[{concept_label}] LLM KW expansion failed: {err_msg}")
...
# --- Stage 1: Evidence Preparation (v34.0.1 - Reads config) ---
# (prepare_evidence - unchanged from previous correct version)
def prepare_evidence(
    ...
    """ Prepares evidence candidates using combined score (BM25 + SBERT) and config overrides. """
    ...
    overrides = config.get("concept_overrides", {}).get(normalized_concept, {}); skip_expansion = overrides.get("skip_expansion", False)
    ...
    logger.info(f"[{normalized_concept}] Abs Min SBERT: {abs_min_sbert}"); logger.info(f"[{normalized_concept}] KW Dampen: Thresh={damp_thresh}, Factor={damp_factor}")
    if boost_cfg.get("enabled"): logger.info(f"[{normalized_concept}] Seed Boost: ON (Thresh: {boost_cfg.get('threshold', '?')}) URIs: {seed_uris}")
    logger.info(f"[{normalized_concept}] LLM KW Expansion: {kw_exp_enabled}"); logger.info(f"[{normalized_concept}] Tie-Break Sort: ON")
    ...
    if kw_exp_enabled and needs_exp:
        exp_diag["attempted"] = True; logger.info(f"[{normalized_concept}] Attempting LLM KW expansion (Initial matches {initial_kw_count} < {kw_trigger} or abstract).")
    ...
    if bm25_model and keyword_corpus_uris:
        if final_query_texts:
            bm25_min = float(config.get("KEYWORD_SCORING_CONFIG", {}).get("bm25_min_score", 0.01))
            bm25_top_n = int(config.get("KEYWORD_SCORING_CONFIG", {}).get("bm25_top_n", 500))
    ...
    if concept_embedding is None: logger.error(f"[{normalized_concept}] No anchor embed. Skipping SBERT."); sim_scores={}
        ...
        sbert_min_sim = float(config.get("STAGE1_CONFIG", {}).get('EVIDENCE_MIN_SIMILARITY', 0.30))
    ...
    if filter_uris: # Filtering
        ...
        if to_remove: logger.info(f"Applying filter: Removing {len(to_remove)} URIs: {to_remove}"); all_uris.difference_update(to_remove); logger.info(f"New count after filter: {len(all_uris)}")
    ...
    for uri in all_uris_list: # Scoring/Ranking
        if uri not in all_details: continue
        s_orig = sim_scores.get(uri, 0.0); k_raw = kw_scores.get(uri, 0.0)
    ...
    if args.debug: # Log top 5
        logger.debug(f"--- Top 5 for '{normalized_concept}' ---")
        for i, c in enumerate(scored_list[:5]):
            damp=f"(Damp:{c.get('dampened_kw_score'):.4f})" if c.get('dampened_kw_score') else ""; boost=f"(Boost:{c.get('boosted_sim_score'):.4f})" if c.get('boosted_sim_score') else ""; alpha=f"(EffA:{c.get('effective_alpha'):.2f})"; prio=f"(P:{get_sort_priority(c.get('uri',''))})"; lbl=c.get('details',{}).get('prefLabel','N/A')
    ...
    llm_details = [c["details"] for c in selected]; orig_map = {} # Prepare outputs
    for c in selected:
    ...
    anchor_data = selected[0]; anchor_uri = anchor_data.get('uri'); anchor = None # Select anchor
    ...
    else: logger.warning(f"[{normalized_concept}] No anchor.")

    sbert_scores_final = {uri: d["sbert_score"] for uri, d in orig_map.items()}
    # Return counts using the correct internal variable names from this function scope
...
# --- Stage 2: Finalization (v34.0.1 - Reads config) ---
# (apply_rules_and_finalize - unchanged from previous correct version)
def apply_rules_and_finalize(
    ...
    if failed_rules and original_candidates_map_for_reprompt and llm_refinement_enabled and args.llm_provider != "none":
        ...
        fallback_timeout = int(llm_api_cfg.get("REQUEST_TIMEOUT", 180)); fallback_retries = int(llm_api_cfg.get("MAX_RETRIES", 5)); fallback_temp = float(llm_stage2_cfg.get("LLM_TEMPERATURE", 0.2))
        for name in list(failed_rules.keys()):
            ...
            fb_result = call_llm(sys_p, user_p, args.llm_model, fallback_timeout, fallback_retries, fallback_temp, args.llm_provider)
    ...
    for name, base_data in base_themes.items(): # Process themes and attributes
    ...
    for attr in all_final_attrs:
        uri = attr.get("uri");
        if not uri: continue
        current_weight = float(attr.get("concept_weight", 0.0)); stored_weight = unique_attrs.get(uri, {}).get("concept_weight", -1.0)
...
def generate_affinity_definitions_loop(
    ...
    for concept in tqdm(concepts_subset, desc="Processing Concepts", disable=disable_tqdm):
        ...
        try:
            ...
            # ***** CORRECTED VARIABLE NAMES HERE for logging/diagnostics *****
            diag1["keyword_candidate_count_initial"] = keyword_candidate_count_initial
            diag1.update({
                "status": "Completed" if diag1.get("error") is None and concept_emb is not None else "Failed",
                "duration_seconds": round(stage1_dur, 2),
                "sbert_candidate_count_initial": sbert_candidate_count_initial,
                "unique_candidates_before_ranking": unique_count,
                "llm_candidate_count": len(cand_details)
            })
            logger.info(f"[{norm_concept}] Stage 1 done ({stage1_dur:.2f}s). Status: {diag1['status']}. LLM Cands: {len(cand_details)}. Initial SBERT: {sbert_candidate_count_initial}, KW: {keyword_candidate_count_initial}.")
            # ***** END CORRECTION *****
            ...
            if diag1["status"] == "Failed": logger.warning(f"[{norm_concept}] Skipping LLM (Stage 1 failed)."); diag_llm["status"] = "Skipped (Stage 1 Failed)"
                ...
                else: diag_llm["llm_call_success"] = False; diag_llm["status"] = "Failed"; diag_llm["error"] = llm_result.get("error", "Unknown") if llm_result else "None result"; logger.warning(f"[{norm_concept}] LLM slotting failed. Error: {diag_llm['error']}")
...
def main():
    ...
    acs_config = config.get("ACS_DATA_CONFIG", {}); acs_enabled_log = acs_config.get("enable_acs_enrichment", False)
    logger.info(f"ACS Enrichment Enabled: {acs_enabled_log} (Path: {acs_config.get('acs_data_path', 'N/A')})")
    ...
    try:
        ...
        if config.get("ACS_DATA_CONFIG", {}).get("enable_acs_enrichment", False): logger.info(f"ACS data status: {'Loaded' if _acs_data is not None else 'Load Failed'}")
    ...
    try:
        ...
        if kw_enabled and config.get("KEYWORD_SCORING_CONFIG", {}).get("algorithm", "").lower() == 'bm25': logger.info(f"BM25 Index Status: {'Ready' if _bm25_model is not None else 'Failed/Unavailable'}")
