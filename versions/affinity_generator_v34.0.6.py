...
(v34.0.6 - Conceptual - BM25 + ACS + Lodging Context Prompts + Namespace Biasing).
...
- Contextual namespace biasing during candidate ranking.
- Relies on shared utility functions in utils.py (v34+ expected with NaN checks).

Changes from v34.0.2:
- Added Contextual Namespace Biasing logic in prepare_evidence.
- Added helper functions infer_concept_domain and apply_namespace_bias.
- Modified LLM prompts in construct_llm_slotting_prompt and build_reprompt_prompt
  to focus on lodging context and traveler preferences.
- Expects NAMESPACE_BIASING section in config JSON.
...
SCRIPT_VERSION = "affinity-rule-engine-v34.0.6 (BM25 + ACS + Lodging Context + Namespace Biasing)" # Updated Version
DEFAULT_CACHE_VERSION = "v20250420.affinity.34.5.0" # Increment cache version for new logic/config
DEFAULT_CONFIG_FILE = "./affinity_config_v34.6.json" # Expect new config filename
...
DEFAULT_CONFIG = {
    ...
    # Namespace Biasing Config (NEW SECTION)
    "NAMESPACE_BIASING": {
        "enabled": True,
        "boost_factor": 1.05,
        ...
        "lodging_amenity_ns": [ "urn:expediagroup:taxonomies:acs", "urn:expediagroup:taxonomies:core", "urn:expediagroup:taxonomies:spaces", "urn:expe:taxo:hospitality", "urn:expe:taxo:amenity-view-property-features" ],
        ...
        "low_priority_ns": [ "urn:expediagroup:taxonomies:lcm:", "urn:expediagroup:taxonomies:tmpt:" ]
    },
    ...
    # Field Weights for BM25 Doc Construction (*** ENSURE THIS IS SET FOR ACS ON RUN ***)
    "KG_CONFIG": {
        "pref_label_weight": 3,
        "alt_label_weight": 1,
        "definition_weight": 0, # KG Definitions weight (0 to disable)
        "acs_name_weight": 4,   # Set > 0 if ACS is ON & using enrichment fields
        "acs_def_weight": 2     # Set > 0 if ACS is ON & using enrichment fields
    },

    # ACS Data Enrichment
    "ACS_DATA_CONFIG": {
        "acs_data_path": "./datasources/transformed_acs_tracker.csv",
        "enable_acs_enrichment": True, # *** ENSURE THIS IS TRUE FOR ACS ON RUN ***
        "acs_name_weight": 4, # Redundant if KG_CONFIG handles all weights, but keep for clarity
...
def load_acs_data(path: Optional[str]) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # Ensure crucial columns exist and fill NaNs with empty strings
...
# --- START: Keyword Indexing Function (v34.0.5 - Read ACS weights from KG_CONFIG) ---
def build_keyword_index(
    ...
    v34.0.5: Reads all parameters including ACS weights from the merged config dictionary.
    """
    ...
    # Read *all* field weights from KG_CONFIG
    ...
    definition_weight = int(kg_cfg.get("definition_weight", 0)) # Default to 0 if not specified
    acs_name_weight = int(kg_cfg.get("acs_name_weight", 0)) # Default to 0
    acs_def_weight = int(kg_cfg.get("acs_def_weight", 0))   # Default to 0
    ...
    if use_bm25:
        ...
        # Build cache filename including effective field weights
        ...
        if not rebuild_cache:
            ...
            if cached_model is not None and isinstance(cached_model, BM25Okapi) and isinstance(cached_uris, list):
                ...
                else: logger.warning("BM25 cache mismatch (doc count). Rebuilding.")
        ...
        if not cache_valid:
            ...
            try:
                ...
                for uri, data in tqdm(sorted(taxonomy_concepts_cache.items()), desc="Prepare BM25 Docs", disable=disable_tqdm):
                    ...
                    # Apply KG Weights (from KG_CONFIG)
                    ...
                    # ACS Enrichment (Adds text based on weights from KG_CONFIG)
                    acs_name = ''; acs_def = ''; was_enriched = False
                    if enrichment_enabled and uri in _acs_data.index:
                        try:
                            # Simplified lookup assuming index is unique URI
...
def call_llm(system_prompt: str, user_prompt: str, model_name: str, timeout: int, max_retries: int, temperature: float, provider: str) -> Dict[str, Any]:
    ...
    for attempt in range(max_retries + 1):
        ...
        # Retry logic
        should_retry = attempt < max_retries and (result["error"] is not None) and ("Blocked" not in str(result["error"])) # Don't retry if blocked
...
# --- START: Prompt Building Functions (v34.0.5 - Lodging Context) ---
def construct_keyword_expansion_prompt(input_concept: str) -> Tuple[str, str]:
    """Constructs prompts for LLM keyword expansion."""
...
def construct_llm_slotting_prompt(
    ...
    args: argparse.Namespace # Keep args if needed for other logic
) -> Tuple[str, str]:
    """
    Constructs the system and user prompts for the main LLM theme slotting task.
    v34.0.5: Modified prompts to focus on lodging context and traveler relevance.
    """
...
def build_reprompt_prompt(
    ...
    v34.0.5: Modified prompts to focus on lodging context.
    """
    # MODIFIED System Prompt: Added lodging focus
...
# --- START: Namespace Biasing Helper Functions (v34.0.5) ---
def infer_concept_domain(candidate_uris: List[str], config: Dict, top_n_check: int = 50) -> str:
    ...
    # Get namespace lists from config, providing empty tuples as defaults
    lodging_amenity_ns = tuple(bias_config.get("lodging_amenity_ns", ()))
    activity_ns = tuple(bias_config.get("activity_ns", ()))
    # Add other domain categories if needed for inference later (e.g., Location, Visual)
    ...
    for uri in candidate_uris[:top_n_check]:
        checked_count += 1
        # Prioritize lodging/amenity match
        if lodging_amenity_ns and uri.startswith(lodging_amenity_ns):
            lodging_count += 1
        elif activity_ns and uri.startswith(activity_ns):
            activity_count += 1
        # Add more domain checks here if refining inference

    if checked_count == 0: return "Unknown"

    # Inference logic (adjust thresholds as needed)
    lodging_ratio = lodging_count / checked_count
    activity_ratio = activity_count / checked_count

    # Prioritize Lodging/Amenity if strong signal, then Activity
    if lodging_ratio >= 0.4: # Needs >40% lodging/amenity candidates
        return "Lodging/Amenity"
    elif activity_ratio >= 0.4: # Needs >40% activity candidates
        return "Activity"

    # Fallback if signals are weak or mixed
...
# --- Namespace Biasing Helper Function (v34.0.7 - Differentiated Core Boost, LCM Boosted) ---
def apply_namespace_bias(
    ...
    v34.0.7: Uses granular namespace categories, applies a potentially higher boost factor
    ...
    # Ensure core: is NOT listed here if handled separately by core_boost_factor
    # Ensure lcm IS listed here if it should get standard boost
    ...
    if metadata_ns and candidate_uri.startswith(metadata_ns):
        ...
        # We usually stop here for metadata
    ...
    if not penalty_applied and clearly_wrong_ns and candidate_uri.startswith(clearly_wrong_ns):
        ...
        penalty_applied = True
        # Allow low_priority to stack, so don't return yet

    if low_priority_ns and candidate_uri.startswith(low_priority_ns):
        # Apply penalty factor ON TOP of any existing penalty
        ...
        # Update reason, potentially overwriting "Wrong Domain" if applicable, or setting if first penalty
        reason = f"Penalty (LowPrio NS {penalty_factor:.2f})" # Or combine reasons if needed
        penalty_applied = True

    # --- Domain-specific biasing (Only apply if no overriding penalty hit) ---
    # We allow boosts/context adjustments even if a low_priority penalty was applied,
    # but typically not if metadata or clearly_wrong penalty was applied.
    if not reason.startswith("Penalty (Metadata"): # Check if we should proceed
        ...
        if inferred_domain == "Lodging/Amenity":
            ...
            if candidate_uri.startswith(core_ns_prefix):
                new_score = initial_score * core_boost_factor # Apply boost to original score
                reason = f"Boost (Core NS Match {core_boost_factor:.2f})"
                domain_bias_applied = True
            # *** Then check other lodging/amenity NS (including LCM) ***
            elif lodging_amenity_ns and candidate_uri.startswith(lodging_amenity_ns):
                new_score = initial_score * boost_factor # Apply standard boost to original score
                reason = f"Boost (Lodging Domain Match {boost_factor:.2f})"
                domain_bias_applied = True
            # Apply context boosts only if no primary boost was applied
            elif not penalty_applied: # Don't boost if already penalized
                if location_context_ns and candidate_uri.startswith(location_context_ns):
                     ...
                     domain_bias_applied = True
            # Apply penalties for domain mismatches ONLY if no penalty was applied yet
            # AND no boost was applied
            if not domain_bias_applied and not penalty_applied:
                if activity_ns and candidate_uri.startswith(activity_ns):
                    ...
                    domain_bias_applied = True
                # Strong penalty for clearly_wrong already handled above
             ...
             # Apply context boosts only if no primary boost was applied
                 ...
                 if location_context_ns and candidate_uri.startswith(location_context_ns):
                      ...
                      reason = f"Boost (Preference Context {context_boost_factor:.2f})"
                      domain_bias_applied = True
             # Apply penalties for mismatches ONLY if no penalty was applied yet
             # AND no boost was applied
             if not domain_bias_applied and not penalty_applied:
                 # Check core AND other lodging namespaces
        ...
        # If no specific domain bias/penalty was applied, ensure the reason reflects any initial penalty
        if not domain_bias_applied and penalty_applied:
             pass # Reason already set by initial penalty checks
        elif not domain_bias_applied and not penalty_applied:
             reason = "Neutral" # Truly neutral

    # Final sanity check if score ended up zero or negative
    if new_score <= 0:
        logger.warning(f"Bias resulted in non-positive score ({new_score:.4f}) for {candidate_uri}. Resetting to small positive.")
        new_score = 1e-9 # Prevent issues with log or division later
...
# --- Stage 1: Evidence Preparation (v34.0.5 - Integrates refined biasing) ---
# -*- coding: utf-8 -*-
# --- Stage 1: Evidence Preparation (v34.0.6 - Adds Exact prefLabel Match Priority) ---
def prepare_evidence(
    ...
    v34.0.6 Changes:
    - Added logic to prioritize candidates with an exact prefLabel match (case-insensitive, normalized)
      if the 'prioritize_exact_prefLabel_match' config flag is true.
    - Uses namespace priority and SBERT score as tie-breakers for exact matches.
    - Falls back to combined scoring + biasing if no exact match is found or flag is false.
    - Always calculates combined scores for all candidates to select the LLM context list.
    """
    normalized_input_concept = normalize_concept(input_concept)
    selection_method_log = "Combined BM25+SBERT" # Default, may be overridden
    ...
    def get_sort_priority(item_uri: str) -> int:
        ...
        for i, ns_prefix in enumerate(preferred_ns):
            # Use startswith for flexibility with trailing # or /
            if item_uri.startswith(ns_prefix):
                return i
        return len(preferred_ns) + 10 # Lower priority if not in preferred list
    ...
    # *** NEW: Read exact match priority flag ***
    prioritize_exact_match = config.get("prioritize_exact_prefLabel_match", False)
    ...
    # *** NEW: Log exact match setting ***
    logger.info(f"[{normalized_input_concept}] Prioritize Exact prefLabel Match: {prioritize_exact_match}")
    ...
    if bm25_model and keyword_corpus_uris:
        if final_query_texts:
            ...
            kw_cands_raw = get_candidate_concepts_keyword(final_query_texts, bm25_model, keyword_corpus_uris, bm25_top_n, 0.0) # Get raw scores first
    ...
    if seed_uris:
        ...
        if added: logger.info(f"SEEDING '{normalized_input_concept}' with {len(added)} URIs: {added}"); all_uris.update(added)
        needed_sbert = added - set(sbert_cands_raw_map.keys()) # Check raw map
        if needed_sbert and concept_embedding is not None:
             ...
             if seed_embs:
                 ...
                 sbert_cands_raw_map.update(new_sims) # Add raw scores for seeds
                 # Update sim_scores only if above threshold (consistent with main sbert step)
    ...
    # --- START: Exact prefLabel Match Check ---
    exact_match_uris = []
    if prioritize_exact_match:
        logger.debug(f"[{normalized_input_concept}] Checking for exact prefLabel matches...")
        match_count = 0
        for uri in all_uris_list:
            concept_data = taxonomy_concepts_cache.get(uri)
            if not concept_data: continue
            pref_label_raw = concept_data.get('skos:prefLabel')
            pref_label = None
            # Handle list or single string for prefLabel
            if isinstance(pref_label_raw, list) and pref_label_raw:
                pref_label = str(pref_label_raw[0]) # Take the first one if it's a list
            elif isinstance(pref_label_raw, str):
                pref_label = pref_label_raw

            if pref_label:
                normalized_pref_label = normalize_concept(pref_label)
                if normalized_pref_label == normalized_input_concept:
                    exact_match_uris.append(uri)
                    match_count += 1
        logger.info(f"[{normalized_input_concept}] Found {match_count} exact prefLabel matches.")
    # --- END: Exact prefLabel Match Check ---


    # Infer Domain based on *initial raw* candidates (BM25+SBERT before filtering/seeding)
    inferred_domain = "Unknown"
    if ns_bias_enabled:
        # Ensure we consider raw candidates before filtering for domain inference
    ...
    # Get Details & Score/Rank/Bias FOR ALL candidates (needed for LLM context list)
    ...
    for uri in all_uris_list:
        ...
        # Use the raw SBERT score from the initial batch calculation
        s_orig = sbert_cands_raw_map.get(uri, 0.0)
        k_raw = kw_scores.get(uri, 0.0) # Uses scores >= bm25_min
        ...
        if s_boosted < abs_min_sbert: abs_filt += 1; continue # Absolute SBERT filter

        k_damp = k_raw; dampened = False
        if s_boosted < damp_thresh: # Apply dampening based on potentially boosted SBERT
            k_damp *= damp_factor
            if k_raw > 0 and abs(k_raw - k_damp) > 1e-9: dampened = True; damp_cnt += 1

        # Use potentially boosted SBERT and dampened KW for combined score calculation
        norm_s = max(0.0, min(1.0, s_boosted)); norm_k = max(0.0, min(1.0, k_damp))
        ...
        # Apply namespace bias only if not boosted by seed config
        ...
        scored_list.append({
            ...
            "sim_score":s_orig, # Original SBERT for reference/tie-breaking
            "boosted_sim_score":s_boosted if boosted else None,
            "kw_score":k_raw, # Raw keyword score
            "dampened_kw_score":k_damp if dampened else None,
            "alpha_overridden":alpha_overridden,
            "effective_alpha":effective_alpha,
            "combined_score_unbiased": combined_unbiased, # Score before bias
            "combined_score": combined, # Final score after bias
            "biased": biased,
    ...
    # Sort the *full* list by combined score for LLM candidate selection and fallback anchor
    logger.debug(f"[{normalized_input_concept}] Sorting {len(scored_list)} candidates by combined score...")
    scored_list.sort(key=lambda x: x.get('sim_score', 0.0), reverse=True) # Tie-break 3: SBERT
    scored_list.sort(key=lambda x: get_sort_priority(x.get('uri', '')), reverse=False) # Tie-break 2: Namespace Prio
    scored_list.sort(key=lambda x: x['combined_score'], reverse=True) # Primary sort: Final Combined Score

    # Log Top 5 based on combined score (always useful)
    if args.debug:
        ...
        for i, c in enumerate(scored_list[:5]):
            damp=f"(Damp:{c.get('dampened_kw_score'):.4f})" if c.get('dampened_kw_score') is not None else ""; boost=f"(Boost:{c.get('boosted_sim_score'):.4f})" if c.get('boosted_sim_score') is not None else ""; alpha=f"(EffA:{c.get('effective_alpha'):.2f})"; prio=f"(P:{get_sort_priority(c.get('uri',''))})"; lbl=c.get('details',{}).get('prefLabel','?')
    ...
    for c in selected:
         ...
         if uri and details:
              # Capture all relevant scores and flags for the selected candidates
              entry={
                  **details,
    ...
    anchor_uri = None
    anchor_data_from_scored_list = None # Store the full entry from scored_list

    if prioritize_exact_match and exact_match_uris:
        selection_method_log = "Exact prefLabel Match"
        logger.info(f"[{normalized_input_concept}] Prioritizing anchor selection from {len(exact_match_uris)} exact prefLabel matches.")

        # Create list of dicts for sorting exact matches
        exact_match_candidates_to_sort = []
        for uri in exact_match_uris:
            # Need original SBERT score for tie-breaking
            # Use sbert_cands_raw_map which holds scores before any boosting or filtering < abs_min_sbert
            sbert_score = sbert_cands_raw_map.get(uri, 0.0)
            # Exclude if below absolute minimum SBERT (should already be filtered by scored_list, but double-check)
            if sbert_score >= abs_min_sbert:
                exact_match_candidates_to_sort.append({"uri": uri, "sbert_score": sbert_score})
            else:
                 logger.warning(f"[{normalized_input_concept}] Exact match URI {uri} excluded due to low SBERT score ({sbert_score} < {abs_min_sbert}).")

        if exact_match_candidates_to_sort:
            # Tie-breaking sort: 1. Namespace Priority (Ascending), 2. SBERT Score (Descending)
            ...
            anchor_uri = exact_match_candidates_to_sort[0]['uri']
            logger.info(f"[{normalized_input_concept}] Selected exact match anchor: {anchor_uri} (Score N/A, Prio: {get_sort_priority(anchor_uri)}, SBERT: {exact_match_candidates_to_sort[0]['sbert_score']:.4f})")

             # Find the full data for this anchor from the already computed scored_list
            for item in scored_list:
                if item['uri'] == anchor_uri:
                    anchor_data_from_scored_list = item
                    break
            if anchor_data_from_scored_list is None:
                 logger.error(f"[{normalized_input_concept}] Could not find data for exact match anchor {anchor_uri} in scored_list! Falling back to combined score.")
                 anchor_uri = None # Reset anchor_uri to trigger fallback
    ...
    if anchor_uri is None:
        if not scored_list: # Should have been caught earlier, but safety check
             logger.error(f"[{normalized_input_concept}] No candidates in scored_list for fallback anchor selection.")
             anchor_data_from_scored_list = None
        else:
             anchor_data_from_scored_list = scored_list[0] # Top candidate from combined score sort
             anchor_uri = anchor_data_from_scored_list.get('uri')
             if prioritize_exact_match: # Log only if we intended to use exact match but fell back
                 logger.info(f"[{normalized_input_concept}] Using fallback anchor (combined score): {anchor_uri}")
             selection_method_log = "Combined BM25+SBERT" # Ensure it's set correctly

    # Populate the final anchor dictionary
    anchor = None
    if anchor_data_from_scored_list and anchor_uri:
        # Use the data from the matched entry in scored_list
        # Ensure the anchor object reflects the scores used in its selection path if needed for downstream use,
        # but primarily use the comprehensive data from scored_list entry.
        anchor = {
            "uri": anchor_uri,
            **anchor_data_from_scored_list.get('details', {}), # Add KG details like prefLabel, altLabel, def, type
            # Include key scores for transparency, reflecting the final state before selection
            "sbert_score": anchor_data_from_scored_list.get("sim_score"), # Original SBERT
            "keyword_score": anchor_data_from_scored_list.get("kw_score"), # Raw KW score
            "combined_score": anchor_data_from_scored_list.get("combined_score"), # Final combined score after bias
            "combined_score_unbiased": anchor_data_from_scored_list.get("combined_score_unbiased"),
            "biased": anchor_data_from_scored_list.get("biased"),
            "effective_alpha": anchor_data_from_scored_list.get("effective_alpha")
        }
        # Add optional scores if they exist
        if anchor_data_from_scored_list.get("boosted_sim_score") is not None:
             anchor["boosted_sbert_score"] = anchor_data_from_scored_list.get("boosted_sim_score")
        if anchor_data_from_scored_list.get("dampened_kw_score") is not None:
             anchor["dampened_keyword_score"] = anchor_data_from_scored_list.get("dampened_kw_score")
        if anchor_data_from_scored_list.get("bias_reason") != "Neutral":
             anchor["bias_reason"] = anchor_data_from_scored_list.get("bias_reason")

        # Log the final selected anchor
        anchor_label = anchor.get('prefLabel', anchor.get('skos:prefLabel', anchor_uri))
        score_display = f"Exact Match (Prio: {get_sort_priority(anchor_uri)}, SBERT: {anchor.get('sbert_score', 0.0):.4f})" if selection_method_log == "Exact prefLabel Match" else f"Score: {anchor.get('combined_score'):.6f}{' - Biased' if anchor.get('biased') else ''}"
        logger.info(f"[{normalized_input_concept}] Anchor ({selection_method_log}): {anchor_label} ({score_display})")

    elif anchor_uri:
        # Should ideally not happen if anchor_data_from_scored_list was found
    ...
    # Final SBERT scores map for selected LLM candidates
    sbert_scores_final = {uri: d["sbert_score"] for uri, d in orig_map.items() if "sbert_score" in d}

    # Update diagnostics with the actual selection method used
...
# (No changes needed in apply_rules_and_finalize itself for this step,
#  it already reads theme config dynamically. The LLM prompts called
#  from here were updated earlier)
def apply_rules_and_finalize(
    ...
    if failed_rules and original_candidates_map_for_reprompt and llm_refinement_enabled and args.llm_provider != "none":
        ...
        for name in list(failed_rules.keys()):
            ...
            sys_p, user_p = build_reprompt_prompt(input_concept, name, base_cfg, original_candidates_map_for_reprompt) # Uses updated prompt
    ...
    for name, base_data in base_themes.items():
        ...
        scores = {u: original_candidates_map_for_reprompt.get(u, {}).get("sbert_score", 0.0) for u in uris if u in original_candidates_map_for_reprompt} # Use SBERT for attribute weighting
        total_score = sum(scores.values()); theme_attrs = []
        if uris and theme_w > 0:
            n_uris = len(uris)
            if total_score < 1e-9 and n_uris > 0: # Handle zero scores (equal weight)
                eq_w = (theme_w / n_uris)
                if eq_w >= min_weight:
                    for u in uris:
            ...
            elif total_score > 1e-9: # Normal weighting by SBERT score proportion
                 for u in uris:
                      if u not in original_candidates_map_for_reprompt: continue
    ...
    # Deduplicate and select top defining attributes (using combined score for ranking now)
    unique_attrs: Dict[str, Dict[str, Any]] = {}
    for attr in all_final_attrs: # Use attributes assigned to *any* theme
        uri = attr.get("uri")
        if not uri: continue
        # Get the combined score used for ranking from the original map
        combined_score = original_candidates_map_for_reprompt.get(uri, {}).get("combined_score", 0.0)
        # Use combined_score for deciding which attribute entry to keep if duplicated
        if uri not in unique_attrs or combined_score > unique_attrs[uri].get("_score_for_dedupe", -1.0):
             attr_copy = {k: v for k, v in attr.items() if k != 'comment'} # Keep stored copy clean
             attr_copy["_score_for_dedupe"] = combined_score # Store score temporarily for sorting
             unique_attrs[uri] = attr_copy

    # Sort by the combined score used for ranking, then remove the temp score
    sorted_top = sorted(unique_attrs.values(), key=lambda x: x.get('_score_for_dedupe', 0.0), reverse=True)
    for attr in sorted_top: attr.pop('_score_for_dedupe', None) # Clean up temp key
...
# --- Main Processing Loop (v34.0.2 - Reads config) ---
def generate_affinity_definitions_loop(
    ...
    # ... (Initialization remains the same as v34.0.2) ...
    all_definitions = []
    ...
    for concept in tqdm(concepts_subset, desc="Processing Concepts", disable=disable_tqdm):
        ...
        try:
            ...
            # prepare_evidence now includes biasing logic
            cand_details, orig_map, anchor, sbert_scores, exp_diag, sbert_init, kw_init, unique_count = prepare_evidence(
            ...
            diag1["sbert_candidate_count_initial"] = sbert_init # Update correct key
            diag1["keyword_candidate_count_initial"] = kw_init   # Update correct key
            diag1.update({ "status": "Completed" if diag1.get("error") is None and concept_emb is not None else "Failed", "duration_seconds": round(stage1_dur, 2), "unique_candidates_before_ranking": unique_count, "llm_candidate_count": len(cand_details) })
            ...
            # LLM Slotting (uses updated prompts via construct_llm_slotting_prompt)
            ...
            if diag1["status"] == "Failed": logger.warning(f"[{norm_concept}] Skipping LLM (Stage 1 failed)."); diag_llm["status"] = "Skipped (Stage 1 Failed)"
                ...
                sys_p, user_p = construct_llm_slotting_prompt(concept, themes_for_prompt, cand_details, args) # This now uses the updated prompt function
                llm_api_cfg = config.get("LLM_API_CONFIG", {}); llm_stage2_cfg = config.get("STAGE2_CONFIG", {})
            ...
            # Stage 2 Finalization (uses updated fallback prompt via build_reprompt_prompt)
...
def main():
    ...
    # --- Argument Parsing (unchanged from v34.0.2) ---
    ...
    # 4. Set global config and update args
    _config_data = config
    ...
    setup_detailed_loggers(output_dir, cache_ver) # Assumes this util exists

    # --- Logging and checks (remain the same) ---
    logger.info(f"Starting {SCRIPT_VERSION}"); logger.info(f"Cmd: {' '.join(sys.argv)}")
    # ... (log effective settings as before) ...
    ...
    logger.info(f"Namespace Biasing: {config.get('NAMESPACE_BIASING', {}).get('enabled', False)}")
    ...
    embed_data = precompute_taxonomy_embeddings(_taxonomy_concepts_cache, sbert_model, embed_cache_f, cache_ver, args.rebuild_cache, args.debug) # Assumes utils handles filtering
