...
- v33.0.11: Added Stage 1 LLM Keyword Expansion for weak concepts. Reverted main prompt.
- v33.0.10: Added Refined CoT Prompting for abstract concepts.
- v33.0.9: Added conditional system prompt instruction for abstract concepts.
...
Version: affinity-rule-engine-v33.0.11 (Stage1 LLM Expansion)
"""
...
# --- Config Defaults ---
SCRIPT_VERSION = "affinity-rule-engine-v33.0.11 (Stage1 LLM Expansion)" # <-- Updated Version
CACHE_VERSION = "v20250421.affinity.33.0.11_default" # <-- Updated Version Default, overridden by config
DEFAULT_TAXONOMY_DIR = "./datasources/"
DEFAULT_CACHE_DIR = "./cache_v33/"
DEFAULT_CONFIG_FILE = "./affinity_config_v33.0.json" # Keep consistent unless changed
...
# --- <<< NEW CONSTANT >>> ---
MIN_KEYWORD_CANDIDATES_FOR_EXPANSION = 5 # Threshold to trigger LLM expansion

# --- List of known abstract/package concepts for tailored prompting (Still useful for identifying weak concepts) ---
...
# --- Loading Functions --- (load_affinity_config, load_taxonomy_concepts remain same as v33.0.8, except main ensures config loaded first)
...
def load_taxonomy_concepts(taxonomy_dir: str, cache_file: str, rebuild_cache: bool, current_cache_version: str, debug_mode: bool) -> Optional[Dict[str, Dict]]:
    ...
    if not rebuild_cache and os.path.exists(cache_file):
        ...
        try:
            ...
            else: logger.info(f"Concept cache invalid/version mismatch (Cache: {cached_data.get('cache_version')}, Expected: {current_cache_version}). Rebuilding.")
...
def precompute_taxonomy_embeddings(
    ...
    global _taxonomy_embeddings_cache, CACHE_VERSION # Need access to the *effective* cache version
    ...
    effective_cache_version = _config_data.get("CACHE_VERSION", CACHE_VERSION) if _config_data else CACHE_VERSION
    ...
    if not rebuild and os.path.exists(cache_file):
        ...
        try:
            ...
            if (cached.get("cache_version") == effective_cache_version and
                ...
                if u_list and u_list[0] in p_embs and p_embs.get(u_list[0]) is not None and p_embs[u_list[0]].shape == (sbert_dim,):
                    ...
                    logger.info(f"Loaded {len(u_list)} primary embeddings from cache (Version: {effective_cache_version}).")
                ...
                else: logger.warning(f"Embeddings cache dim mismatch/empty/key error (Cache: {cached.get('cache_version')}, Expected: {effective_cache_version}). Rebuilding.")
            else: logger.info(f"Embeddings cache version mismatch or structure invalid (Cache: {cached.get('cache_version')}, Expected: {effective_cache_version}). Rebuilding.")
    ...
    if not cache_valid:
        ...
        try:
            os.makedirs(os.path.dirname(cache_file), exist_ok=True)
            # --- <<< Ensure correct cache version is saved >>> ---
            data_to_cache = {"cache_version": effective_cache_version, "primary_embeddings": _taxonomy_embeddings_cache[0], "uris_list": _taxonomy_embeddings_cache[1]}
            with open(cache_file, 'wb') as f: pickle.dump(data_to_cache, f, protocol=pickle.HIGHEST_PROTOCOL)
            logger.info(f"Saved embeddings cache: {cache_file} (Version: {effective_cache_version})")
...
def call_llm(system_prompt: str, user_prompt: str, model_name: str, timeout: int, max_retries: int, temperature: float, provider: str) -> Dict[str, Any]:
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if content:
                try:
                    # Handle potential markdown ```json ... ``` wrappers
                    match = re.search(r"```json\s*(\{.*?\})\s*```", content, re.DOTALL | re.IGNORECASE)
                    if match:
                        cleaned = match.group(1)
                        logger.debug("Extracted JSON from markdown block.")
                    else:
                        cleaned = content.strip().strip('`') # Basic cleaning if no markdown block
...
# --- <<< NEW FUNCTION (v33.0.11) >>> ---
def construct_keyword_expansion_prompt(input_concept: str) -> Tuple[str, str]:
    """Creates system and user prompts for the Stage 1 LLM keyword expansion call."""
    system_prompt = """You are a helpful assistant specializing in travel concepts. Your task is to expand a given travel concept into a list of highly relevant keywords and short phrases that could be used to search a knowledge graph for related concepts. Focus on specific characteristics, amenities, activities, or synonyms. Output ONLY a valid JSON object containing a single key "keywords" whose value is a list of strings."""

    user_prompt = f"""Given the input travel concept: '{input_concept}'

Please generate a list of 5-10 specific, highly relevant keywords or short phrases that capture the core meaning and typical associations of this concept in a travel context. These should be suitable for searching a knowledge graph.

Examples:
- Input: 'allinclusive' -> Output: ["meals included", "drinks included", "on-site activities", "resort amenities", "buffet dining", "unlimited beverages"]
- Input: 'pet friendly' -> Output: ["dogs allowed", "cats allowed", "pet amenities", "animal friendly", "pets welcome"]
- Input: 'aikido' -> Output: ["martial art", "Japanese martial art", "dojo", "self defense", "Ueshiba Morihei", "aikido techniques"]
- Input: 'budget travel' -> Output: ["cheap travel", "low cost", "hostel", "affordable accommodation", "backpacker", "value travel"]

Output ONLY a valid JSON object containing a single key "keywords", where the value is a list of the generated keyword strings.

Example Output Schema:
```json
{{
  "keywords": ["keyword1", "short phrase 2", "keyword3"]
}}
```
...
def construct_llm_slotting_prompt(
    ...
    args: argparse.Namespace
    # No is_abstract_concept parameter needed now
) -> Tuple[str, str]:
...
Do not include explanations or conversational text outside the JSON object."""
    ...
    # --- <<< MODIFIED Task Instruction (v33.0.12) >>> ---
...
Pay attention also to candidates whose labels or definitions directly match or define the input concept (e.g., the candidate 'Family-friendly' for the input 'family friendly'); assign themes to these definitional candidates if their details align with the theme's description and provide supporting context.
...
{task_instruction} # Insert the task instruction
...
def prepare_evidence(
    ...
    v33.0.11: Adds Stage 1 LLM Keyword Expansion for concepts with few initial keyword hits or in ABSTRACT_CONCEPTS_LIST.
    ...
    logger.info(f"Starting evidence prep for: '{normalized_concept}' (Stage 1 LLM Expansion Enabled)")
    ...
    expansion_diag: Dict[str, Any] = {} # Initialize diagnostics for expansion

    # --- Step 0: Initial Keyword Check & Potential Expansion ---
    initial_keyword_matched_uris = set()
    expanded_keywords = set() # Store keywords generated by LLM
    llm_expansion_attempted = False
    llm_expansion_successful = False
    input_keywords = set() # Initialize here

    if _keyword_label_index is None:
        logger.warning("Keyword label index not built. Skipping keyword matching and expansion.")
    else:
        # Extract keywords from the *original* input concept
        input_keywords = set(kw for kw in normalized_concept.split() if len(kw) > 2)
        logger.debug(f"Input keywords for initial check: {input_keywords}")

        # Find initial matches based *only* on the input concept's keywords
        if input_keywords:
            for keyword in input_keywords:
                if keyword in _keyword_label_index:
                    initial_keyword_matched_uris.update(_keyword_label_index[keyword])
            logger.info(f"Found {len(initial_keyword_matched_uris)} candidates via initial keyword match.")
        else:
             logger.warning(f"No usable keywords extracted from '{normalized_concept}' for initial check.")

        # Determine if expansion is needed
        is_weak_concept = (len(initial_keyword_matched_uris) < MIN_KEYWORD_CANDIDATES_FOR_EXPANSION) or \
                          (normalized_concept in ABSTRACT_CONCEPTS_LIST)

        if is_weak_concept and args.llm_provider != "none":
            logger.info(f"Concept '{normalized_concept}' identified as weak (Initial Matches: {len(initial_keyword_matched_uris)}, Abstract: {normalized_concept in ABSTRACT_CONCEPTS_LIST}). Attempting LLM keyword expansion.")
            llm_expansion_attempted = True
            try:
                expansion_sys_prompt, expansion_user_prompt = construct_keyword_expansion_prompt(input_concept)
                # Use main LLM settings for expansion for now
                expansion_llm_result = call_llm(
                    expansion_sys_prompt, expansion_user_prompt,
                    args.llm_model, LLM_TIMEOUT, LLM_MAX_RETRIES, LLM_TEMPERATURE, args.llm_provider
                )

                if expansion_llm_result and expansion_llm_result.get("success"):
                    response_data = expansion_llm_result.get("response", {})
                    raw_keywords = response_data.get("keywords", [])
                    if isinstance(raw_keywords, list):
                        parsed_keywords = set()
                        for phrase in raw_keywords:
                            if isinstance(phrase, str) and phrase.strip():
                                # Normalize the phrase/keyword and split into individual words
                                normalized_phrase = normalize_concept(phrase)
                                parsed_keywords.update(kw for kw in normalized_phrase.split() if len(kw) > 2) # Apply length filter

                        if parsed_keywords:
                            expanded_keywords = parsed_keywords - input_keywords # Only keep *new* keywords
                            if expanded_keywords:
                                 logger.info(f"LLM expansion successful. Added {len(expanded_keywords)} unique expanded keywords: {expanded_keywords}")
                                 llm_expansion_successful = True
                            else:
                                 logger.info("LLM expansion returned keywords, but they were already present in the input concept.")
                        else:
                            logger.warning("LLM expansion call succeeded but returned no usable keywords.")
                    else:
                         logger.error("LLM expansion response['keywords'] was not a list.")
                         expansion_diag["error"] = "LLM response['keywords'] was not a list."
                else:
                     error_msg = expansion_llm_result.get("error", "Unknown error") if expansion_llm_result else "Expansion LLM result was None"
                     logger.error(f"LLM keyword expansion call failed: {error_msg}")
                     expansion_diag["error"] = error_msg

            except Exception as e_expand:
                logger.error(f"Error during LLM keyword expansion phase: {e_expand}", exc_info=True)
                expansion_diag["error"] = str(e_expand)
        elif is_weak_concept and args.llm_provider == "none":
             logger.warning(f"Concept '{normalized_concept}' is weak, but LLM provider is 'none'. Skipping expansion.")
        else:
             logger.info(f"Concept '{normalized_concept}' has sufficient initial keyword matches ({len(initial_keyword_matched_uris)}). Skipping LLM expansion.")

        # Store expansion diagnostics
        expansion_diag = {
            "attempted": llm_expansion_attempted,
            "successful": llm_expansion_successful,
            "expanded_keywords_count": len(expanded_keywords),
            "expanded_keywords": list(expanded_keywords) if args.debug else None, # Only log list in debug
             # This count will be calculated later
            "final_keyword_candidates_count": 0,
            "error": expansion_diag.get("error") # Keep error if it occurred
        }
        logger.debug(f"Stage 1 Expansion Diagnostics (Initial): {expansion_diag}")


    # --- Step 1: Embedding Similarity Pool --- (Unchanged)
    ...
    # --- Step 2: Keyword Matching Pool (Uses original + expanded keywords) ---
    keyword_matched_uris = set() # Reset and recalculate
    if _keyword_label_index is not None:
        search_keywords = input_keywords.union(expanded_keywords) # Use initial + expanded
        if not search_keywords:
             logger.warning(f"No keywords (original or expanded) to search for '{normalized_concept}'.")
        else:
             logger.debug(f"Keywords for final index search ({len(search_keywords)}): {search_keywords}")
             for keyword in search_keywords:
                  if keyword in _keyword_label_index:
                       keyword_matched_uris.update(_keyword_label_index[keyword])
             logger.info(f"Found {len(keyword_matched_uris)} candidates via combined keyword matching (original + expanded).")
             expansion_diag["final_keyword_candidates_count"] = len(keyword_matched_uris) # Update final count
    # else: logger already warned about missing index


    # --- Step 3: Combine Pools and Select Candidates --- (Unchanged logic)
    ...
    if not combined_candidate_uris:
        ...
        # Add expansion_diag to the return tuple when returning early
        return [], {}, None, candidate_evidence_scores, expansion_diag
    ...
    for uri in combined_candidate_uris:
        if uri in all_candidate_details_map:
            ...
            details['similarity_score'] = candidate_evidence_scores.get(uri, 0.0)
    ...
    # --- Step 4: Rank and Select (Strict Priority for Keywords) --- (Unchanged logic)
    ...
    # --- Step 5: Determine Anchor and Finalize --- (Unchanged logic)
    ...
    # --- <<< MODIFIED: Return expansion_diag >>> ---
    return selected_candidates_details, original_candidates_map, anchor_candidate, candidate_evidence_scores, expansion_diag
...
def apply_rules_and_finalize(
    ...
    if failed_must_have_initial and original_candidates_map_for_reprompt and args.llm_provider != "none": # Also check provider isn't none
    ...
    elif failed_must_have_initial and args.llm_provider == "none":
         logger.warning(f"[{normalized_concept}] Cannot attempt fallback for failed rules because LLM provider is 'none'.")
        ...
        if assigned_uris_final and normalized_weight_for_theme > 0 and total_initial_score_in_theme > 0:
            ...
            for uri in assigned_uris_final: # Iterate over all URIs now associated with the theme
                ...
                if final_attribute_weight >= theme_attribute_min_weight:
                     ...
                     if is_fallback_add: final_attr["comment"] = "Added via Fallback"
    ...
    tc = stage2_output["travel_category"]
...
def generate_affinity_definitions_loop(
    ...
    effective_cache_version = config.get("CACHE_VERSION", "unknown_v33.11")
    ...
    for concept in tqdm(concepts_subset, desc="Processing Concepts", disable=disable_tqdm):
        ...
        affinity_definition: Dict[str, Any] = {
            ...
            "diagnostics": {
                "stage1": {"status": "Not Started", "error": None, "expansion": {}}, # Add expansion placeholder
        ...
        diag_stage1 = affinity_definition["diagnostics"]["stage1"] # Local ref
        ...
        try:
            ...
            # --- Stage 1: Prepare Evidence (Now includes LLM Expansion) ---
            stage1_start_time = time.time()
            # Call the modified prepare_evidence, capture expansion diagnostics
            candidates_for_llm_details, original_candidates_map, anchor_candidate, candidate_evidence_scores, stage1_expansion_diag = prepare_evidence(
                 concept, concept_embedding, primary_embeddings, config, args
            )
            stage1_duration = time.time() - stage1_start_time

            # --- Merge Stage 1 expansion diagnostics ---
            diag_stage1["expansion"] = stage1_expansion_diag

            diag_stage1.update({ # Use update to merge with existing keys
                "status": "Completed", "duration_seconds": round(stage1_duration, 2),
                "candidate_evidence_count_initial_pool": len(candidate_evidence_scores),
            ...
            if not candidates_for_llm_details:
                ...
                # Construct and make the LLM call (using the reverted prompt)
            ...
            affinity_definition.update({ k: v for k, v in stage2_output_data.items() if k != 'diagnostics' })
            ...
            if "diagnostics" in stage2_output_data:
                ...
                affinity_definition["diagnostics"]["stage2"] = stage2_diags.get("stage2", {"status": "Completed", "duration_seconds": round(stage2_duration, 2), "error": None})
            ...
            affinity_definition["processing_metadata"]["duration_seconds"] = total_duration # Deprecated
...
def main():
    ...
    # --- Load Configuration FIRST to get correct CACHE_VERSION ---
    # This also populates _config_data globally for other functions
    config = load_affinity_config(args.config)
    if config is None:
        # load_affinity_config logs critical error, just exit
        sys.exit(1)

    # --- Determine Effective CACHE_VERSION and paths ---
    # Use loaded config's version if present, otherwise the script default
    effective_cache_version = config.get("CACHE_VERSION", CACHE_VERSION)
    logger.info(f"Using effective CACHE_VERSION for filenames: {effective_cache_version}")

    log_filename = LOG_FILE_TEMPLATE.format(cache_version=effective_cache_version)
    log_filepath = os.path.join(args.output_dir, log_filename)
    output_filename = OUTPUT_FILE_TEMPLATE.format(cache_version=effective_cache_version)
    output_filepath = os.path.join(args.output_dir, output_filename)
    # --- END Path setup using effective cache version ---

    # Setup logging (now that we have the final log path)
    log_level = logging.DEBUG if args.debug else logging.INFO
    log_format = '%(asctime)s - %(levelname)s - [%(funcName)s:%(lineno)d] - %(message)s' # Added lineno
    date_format = '%Y-%m-%d %H:%M:%S'
    for handler in logging.root.handlers[:]: logging.root.removeHandler(handler) # Remove existing handlers
    ...
    # Apply command-line LLM overrides AFTER initial config load
    ...
    except ValueError as ve: logger.critical(f"Error processing input concepts file '{args.input_concepts_file}': {ve}"); sys.exit(1)
    ...
    concepts_cache_file = os.path.join(args.cache_dir, f"concepts_cache_{effective_cache_version}.json")
    taxonomy_concepts = load_taxonomy_concepts(args.taxonomy_dir, concepts_cache_file, args.rebuild_cache, effective_cache_version, args.debug)
    if taxonomy_concepts is None: logger.critical("Failed to load taxonomy concepts. Exiting."); sys.exit(1)
    ...
    except Exception as e: logger.critical(f"Failed to load SBERT model via utils.py: {e}", exc_info=True); sys.exit(1)

    # --- Load/Build Embeddings Cache ---
    embeddings_cache_file = os.path.join(args.cache_dir, f"embeddings_cache_{effective_cache_version}.pkl")
    # Pass args to precompute_taxonomy_embeddings as it uses args.debug and args.rebuild_cache
    embeddings_data = precompute_taxonomy_embeddings(taxonomy_concepts, sbert_model, embeddings_cache_file, args)
    if embeddings_data is None: logger.critical("Failed to load or compute embeddings. Exiting."); sys.exit(1)
    ...
    all_results = generate_affinity_definitions_loop(
        input_concepts,
        config, # Pass the already loaded and potentially overridden config
        args,
        sbert_model,
    ...
    except Exception as e: logger.error(f"Failed to save results to {output_filepath}: {e}")
