...
(v34.0.5 - Conceptual - BM25 + ACS + Lodging Context Prompts + Namespace Biasing).
...
SCRIPT_VERSION = "affinity-rule-engine-v34.0.5 (BM25 + ACS + Lodging Context + Namespace Biasing)" # Updated Version
DEFAULT_CACHE_VERSION = "v20250420.affinity.34.5.0" # Increment cache version for new logic/config
DEFAULT_CONFIG_FILE = "./affinity_config_v34.5.json" # Expect new config filename
...
def apply_namespace_bias(
    ...
    v34.0.5: Uses more granular namespace categories, treats contextual namespaces favorably.
    ...
    context_boost_factor = float(bias_config.get("context_boost_factor", 1.02)) # Slight boost for context
    ...
    # Read namespace lists from config
    lodging_amenity_ns = tuple(bias_config.get("lodging_amenity_ns", ()))
    activity_ns = tuple(bias_config.get("activity_ns", ()))
    ...
    if metadata_ns and candidate_uri.startswith(metadata_ns):
        new_score *= metadata_penalty_factor
        reason = f"Penalty (Metadata NS {metadata_penalty_factor:.2f})"
        return new_score, reason # Apply penalty and stop further checks for metadata

    # Apply low priority penalty but allow potential boosts later
    initial_penalty_reason = None
    if low_priority_ns and candidate_uri.startswith(low_priority_ns):
        new_score *= penalty_factor # Use regular penalty
        initial_penalty_reason = f"Penalty (LowPrio NS {penalty_factor:.2f})"
        reason = initial_penalty_reason # Tentative reason

    # --- Domain-specific biasing ---
    bias_applied = False
    if inferred_domain == "Lodging/Amenity":
        if lodging_amenity_ns and candidate_uri.startswith(lodging_amenity_ns):
            new_score *= boost_factor; reason = f"Boost (Lodging Domain Match {boost_factor:.2f})"; bias_applied=True
        elif location_context_ns and candidate_uri.startswith(location_context_ns):
             new_score *= context_boost_factor; reason = f"Boost (Location Context {context_boost_factor:.2f})"; bias_applied=True
        elif visual_context_ns and candidate_uri.startswith(visual_context_ns):
             new_score *= context_boost_factor; reason = f"Boost (Visual Context {context_boost_factor:.2f})"; bias_applied=True
        elif preference_context_ns and candidate_uri.startswith(preference_context_ns):
             new_score *= context_boost_factor; reason = f"Boost (Preference Context {context_boost_factor:.2f})"; bias_applied=True
        elif activity_ns and candidate_uri.startswith(activity_ns):
            if not initial_penalty_reason: # Don't overwrite low_priority penalty unless boosting
                 new_score *= penalty_factor; reason = f"Penalty (Activity NS vs Lodging {penalty_factor:.2f})"; bias_applied=True
        elif clearly_wrong_ns and candidate_uri.startswith(clearly_wrong_ns):
            new_score *= strong_penalty_factor; reason = f"Penalty (Wrong Domain {strong_penalty_factor:.2f})"; bias_applied=True

    elif inferred_domain == "Activity":
        if activity_ns and candidate_uri.startswith(activity_ns):
            new_score *= boost_factor; reason = f"Boost (Activity Domain Match {boost_factor:.2f})"; bias_applied=True
        elif location_context_ns and candidate_uri.startswith(location_context_ns):
             new_score *= context_boost_factor; reason = f"Boost (Location Context {context_boost_factor:.2f})"; bias_applied=True
        elif preference_context_ns and candidate_uri.startswith(preference_context_ns):
             new_score *= context_boost_factor; reason = f"Boost (Preference Context {context_boost_factor:.2f})"; bias_applied=True
        elif lodging_amenity_ns and candidate_uri.startswith(lodging_amenity_ns):
             if not initial_penalty_reason:
                 new_score *= penalty_factor; reason = f"Penalty (Lodging NS vs Activity {penalty_factor:.2f})"; bias_applied=True
        elif clearly_wrong_ns and candidate_uri.startswith(clearly_wrong_ns):
            new_score *= strong_penalty_factor; reason = f"Penalty (Wrong Domain {strong_penalty_factor:.2f})"; bias_applied=True

    elif inferred_domain == "General" or inferred_domain == "Unknown":
         if clearly_wrong_ns and candidate_uri.startswith(clearly_wrong_ns):
             new_score *= strong_penalty_factor # Apply full strong penalty now
             reason = f"Penalty (Wrong Domain vs General {strong_penalty_factor:.2f})"; bias_applied=True
         # Only apply low_priority penalty if no other bias was applied
         elif initial_penalty_reason and not bias_applied:
             reason = initial_penalty_reason; bias_applied = True # Confirm penalty was applied

    # If no specific bias was applied, reset reason to Neutral (unless initial low prio was set)
    if not bias_applied:
        reason = initial_penalty_reason if initial_penalty_reason else "Neutral"
...
def prepare_evidence(
    ...
    and refined contextual namespace biasing.
    """
    ...
    logger.info(f"[{normalized_concept}] Namespace Biasing Enabled: {ns_bias_enabled}")
    ...
    # ... (keyword expansion logic remains the same) ...
    ...
    if bm25_model and keyword_corpus_uris:
        if final_query_texts:
            ...
            logger.debug(f"[{normalized_concept}] Found {kw_cand_count_init} BM25 candidates >= {bm25_min} (out of {len(kw_cands_raw)} raw).")
    ...
    if concept_embedding is None: logger.error(f"[{normalized_concept}] No anchor embed. Skipping SBERT."); sim_scores={}
        ...
        logger.debug(f"[{normalized_concept}] Found {sbert_cand_count_init} SBERT candidates >= {sbert_min_sim} (out of {len(sbert_cands_raw_map)} raw).")
    ...
    # Infer Domain based on *initial raw* candidates
    inferred_domain = "Unknown"
    if ns_bias_enabled:
        initial_candidate_uris = list(set([c['uri'] for c in kw_cands_raw] + list(sbert_cands_raw_map.keys())))
        inferred_domain = infer_concept_domain(initial_candidate_uris, config) # Pass config
        logger.info(f"[{normalized_concept}] Inferred domain: {inferred_domain}")

    # Get Details & Score/Rank/Bias
    ...
    for uri in all_uris_list:
        ...
        if s_boosted < abs_min_sbert: abs_filt += 1; continue
        ...
        if ns_bias_enabled and not boosted:
            combined, bias_reason = apply_namespace_bias(combined_unbiased, uri, inferred_domain, config) # Pass config
            if abs(combined - combined_unbiased) > 1e-9: biased = True; bias_cnt +=1
        scored_list.append({"uri":uri, "details":all_details[uri], "sim_score":s_orig, "boosted_sim_score":s_boosted if boosted else None, "kw_score":k_raw, "dampened_kw_score":k_damp if dampened else None, "alpha_overridden":alpha_overridden, "effective_alpha":effective_alpha, "combined_score_unbiased": combined_unbiased, "combined_score": combined, "biased": biased, "bias_reason": bias_reason})

    logger.info(f"[{normalized_concept}] Excluded {abs_filt} (SBERT<{abs_min_sbert}). Dampened KW for {damp_cnt} (SBERT<{damp_thresh}). Biased {bias_cnt} scores (Domain:{inferred_domain}).")
    if alpha_overridden: logger.info(f"[{normalized_concept}] Applied alpha override: {effective_alpha:.2f}.")
    if not scored_list: logger.warning(f"[{normalized_concept}] No candidates remain."); return [], {}, None, {}, exp_diag, sbert_cand_count_init, kw_cand_count_init, unique_cands_before_rank
    ...
    if args.debug:
        ...
        for i, c in enumerate(scored_list[:5]):
            ...
            logger.debug(f"{i+1}. URI:{c.get('uri','?')} {prio} L:'{lbl}' Comb:{c.get('combined_score_unbiased'):.6f}{bias_info}{alpha} (SBERT:{c.get('sim_score'):.4f}{boost}, KW:{c.get('kw_score'):.4f}{damp})")
    ...
    if not selected: logger.warning(f"[{normalized_concept}] No candidates selected."); return [], {}, None, {}, exp_diag, sbert_cand_count_init, kw_cand_count_init, unique_cands_before_rank
    ...
    if anchor: logger.info(f"[{normalized_concept}] Anchor: {anchor.get('prefLabel', anchor_uri)} (Score: {anchor_data.get('combined_score'):.6f}{' - Biased' if anchor_data.get('biased') else ''})")
