...
# -*- coding: utf-8 -*-
"""
Generate affinity definitions for travel concepts using combined scoring (v33.0.13).
...
- LLM-assisted theme slotting with prompt enhancements (v33.0.12).
...
# --- Config Defaults ---
SCRIPT_VERSION = "affinity-rule-engine-v33.0.13.1 (Combined Score Selection + Split Keywords)" # Updated version marker
...
COMBINED_SCORE_ALPHA = 0.6 #played w/ 0.4 and results were bad for aikido
...
def construct_keyword_expansion_prompt(input_concept: str) -> Tuple[str, str]:
    """Creates prompts for Stage 1 LLM keyword expansion (v33.0.13 - Enhanced)."""
...
def prepare_evidence(
    ...
    v33.0.13.1 - Includes mandatory splitting of input concept for TF-IDF.
    ...
    """
    normalized_concept = normalize_concept(input_concept); logger.info(f"Starting evidence prep for: '{normalized_concept}' (Combined Scoring)")
    ...
    # --- MODIFICATION START: Ensure base split words are always considered ---
    # 1. Get base keywords by splitting the normalized concept
    ...
    # Initialize query texts with the base split words
    query_texts_set = set(base_split_kws)

    initial_kw_count = 0
    if keyword_label_index:
        # Check simple index matches based on base split words
    ...
    if needs_exp:
        ...
        # Call LLM expansion function
        expanded_terms_list = expand_keywords_with_llm(input_concept, config, args) # This returns unique terms including original normalized words

        # Process expanded terms: split phrases and add individual words > 2 chars
        for phrase in expanded_terms_list:
            if isinstance(phrase, str) and phrase.strip(): # Check if it's a valid string
                 normalized_phrase = normalize_concept(phrase)
                 # Add individual terms longer than 2 characters
                 expanded_kws.update(term for term in normalized_phrase.split() if len(term) > 2)

        # Remove the original normalized concept parts if they were re-added by LLM
        # We already have them in base_split_kws
        original_words_from_expansion_input = set(w for w in normalize_concept(input_concept).split() if len(w) > 2)
        # Only keep expanded keywords that were NOT part of the original input
        newly_expanded_kws = expanded_kws - original_words_from_expansion_input

        exp_diag["successful"] = bool(newly_expanded_kws) # Successful if LLM added *new* terms
        if exp_diag["successful"]:
             logger.info(f"LLM expansion added {len(newly_expanded_kws)} new keyword terms.")
             # Add the *new* expanded keywords to our query set
             query_texts_set.update(newly_expanded_kws)
        else:
             logger.info("LLM expansion did not add new useful keyword terms.")
             exp_diag["error"] = exp_diag.get("error", "LLM returned no new terms") # Keep existing error if API failed

    else:
        logger.info("Skipping LLM KW expansion.")

    # Final list of keywords for TF-IDF search is the union of base split words and new expanded words
    final_query_texts = list(query_texts_set)
    exp_diag["count"] = len(final_query_texts)
    exp_diag["terms"] = final_query_texts if args.debug else ["..."] # Log final terms if debug enabled

    logger.info(f"Using {len(final_query_texts)} unique keywords for TF-IDF search: {final_query_texts if args.debug else '...'}") # Log final keywords list

    # Proceed with TF-IDF search using the final_query_texts
    tfidf_candidate_count_initial = 0 # Initialize count
    if tfidf_vectorizer and tfidf_matrix is not None and tfidf_corpus_uris:
        if final_query_texts: # Check if there are any keywords to search
            ...
            tfidf_candidate_count_initial = len(kw_scores) # Capture initial count
            exp_diag["tfidf_count"] = tfidf_candidate_count_initial # Update diagnostic count based on actual results
    ...
    # --- SBERT Similarity Calculation (Remains the same) ---
    sbert_candidate_count_initial = 0 # Initialize count
    if concept_embedding is None:
        ...
        sim_scores = {uri: s for uri, s in sim_scores.items() if s >= min_sim}
        sbert_candidate_count_initial = len(sim_scores) # Capture initial count
        logger.info(f"Found {sbert_candidate_count_initial} SBERT candidates >= {min_sim}.")

    # --- Candidate Combination and Ranking (Remains the same) ---
    all_uris = set(kw_scores.keys()) | set(sim_scores.keys())
    unique_candidates_before_ranking = len(all_uris) # Capture count
    if not all_uris:
        logger.warning(f"No candidates found from either TF-IDF or SBERT for '{normalized_concept}'.")
        # Return empty/None values along with counts
    ...
    for uri in all_uris:
        ...
        # Assuming scores are roughly [0,1], simple clipping is sufficient
    ...
    logger.info(f"Selected top {len(selected)}/{len(scored_list)} candidates based on combined score (alpha={alpha}).")

    if not selected:
        logger.warning(f"No candidates selected after scoring for '{normalized_concept}'.")
        # Return empty/None values along with counts
        return [], {}, None, {}, exp_diag, sbert_candidate_count_initial, tfidf_candidate_count_initial, unique_candidates_before_ranking

    # Prepare outputs
    llm_details = [c["details"] for c in selected]
    # Ensure SBERT score is included for downstream weighting
    ...
    anchor_data = selected[0] # Anchor is the top candidate after combined scoring
...
def apply_rules_and_finalize(
    ...
    original_candidates_map_for_reprompt: Dict[str, Dict], # Includes sbert_score
    candidate_evidence_scores: Dict[str, float], # SBERT scores for weighting
    ...
    final_cfg = config.get('STAGE2_CONFIG', {}); min_weight = float(final_cfg.get('THEME_ATTRIBUTE_MIN_WEIGHT', THEME_ATTRIBUTE_MIN_WEIGHT)); top_n_attrs = int(final_cfg.get('TOP_N_DEFINING_ATTRIBUTES', TOP_N_DEFINING_ATTRIBUTES_DEFAULT)) # Use default constant
    ...
    for name, base_data in base_themes.items():
        ...
        scores = {u: candidate_evidence_scores.get(u, 0.0) for u in uris if u in candidate_evidence_scores}; total_score = sum(scores.values()); logger.debug(f"[{norm_concept}][{name}] URIs:{len(uris)}, Score:{total_score:.4f}, Weight:{theme_w:.4f}")
        theme_attrs = []
        if uris and theme_w > 0:
            n_uris = len(uris)
            if total_score < 1e-9 and n_uris > 0:
                logger.warning(f"Zero scores for '{name}'. Equal weight."); eq_w = (theme_w / n_uris)
                if eq_w >= min_weight:
                    for u in uris:
                ...
                else: logger.warning(f"Equal weight {eq_w:.6f} < {min_weight} for '{name}'.")
    ...
    # --- Calculate Top Defining Attributes (Corrected Logic) ---
    ...
    for attr in all_final_attrs:
        ...
        # Use CORRECT variables: uri and current_weight
...
def generate_affinity_definitions_loop(
    ...
    """ Main loop processing each concept (v33.0.13.1 version with diagnostics update). """ # Updated version marker
