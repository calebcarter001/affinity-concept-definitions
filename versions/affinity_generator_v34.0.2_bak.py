...
def construct_llm_slotting_prompt(input_concept: str, theme_definitions: List[Dict[str, Any]], candidate_details: List[Dict[str, Any]], args: argparse.Namespace) -> Tuple[str, str]:
    ...
    cand_details_str = "".join(f"\n{i+1}. URI:{c.get('uri','?')} L:'{c.get('prefLabel','?')}' Alt:{c.get('skos:altLabel',[])} Def:'{(c.get('skos:definition', '') or '')[:150]}...' T:{c.get('type_labels',[])}" for i, c in enumerate(candidate_details)) or "\nNo candidates."
    schema_example = '```json\n{\n  "theme_assignments": {\n    "URI_1": ["ThemeA"],\n    "URI_2": [],\n    "URI_3": ["ThemeC"]\n    // ... ALL URIs ...\n  }\n}\n```'
    task = f"""Task: For EACH candidate URI, list relevant themes from above. Ensure candidates matching '{input_concept}' are slotted. EVERY URI must be a key in output. Focus on semantics."""
...
def build_reprompt_prompt(input_concept: str, theme_name: str, theme_config: Dict, original_candidates_details_map: Dict[str, Dict]) -> Tuple[str, str]:
    system_prompt = """You are assisting travel definition refinement. A mandatory theme needs assignments. Review original candidates ONLY for relevance to the specific theme. Output ONLY JSON like {"theme_assignments": {"URI_Relevant_1": ["ThemeName"], ...}}. If none relevant, output {"theme_assignments": {}}."""
    ...
    user_prompt = f"""Re-evaluating '{input_concept}' for MANDATORY theme '{theme_name}'.\nTheme:\n- Name:{theme_name}\n- Desc:{desc}\n{hints_str}\nCandidates:\n{cand_list}\nInstructions: Identify candidates relevant to '{theme_name}'. Assign >=1 if plausible. Output ONLY valid JSON per schema.\nSchema:\n{schema}\nOutput:"""
    return system_prompt.strip(), user_prompt.strip()
