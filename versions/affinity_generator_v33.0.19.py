...
# -*- coding: utf-8 -*-
"""
Generate affinity definitions for travel concepts using combined scoring (v33.0.19).

Implements:
- Testing COMBINED_SCORE_ALPHA = 0.5.
- Retains Aikido seeding and Aikido-specific no-expansion from v33.0.18.
- Combined TF-IDF Keyword + SBERT Similarity score for candidate selection.
- Absolute SBERT Filter (0.15).
- Conditional TF-IDF Dampening (Threshold=0.50, Factor=0.25).
- TF-IDF indexing and search functionality.
- Optional LLM keyword expansion for weak concepts (except Aikido).
...
try:
    # These are optional for core logic but used in utils
...
try:
    # Required for TF-IDF if enabled
...
# --- Config Defaults & Constants for Run 7 ---
SCRIPT_VERSION = "affinity-rule-engine-v33.0.19 (Testing COMBINED_SCORE_ALPHA=0.6)" # Run 7 version
DEFAULT_CACHE_VERSION = "v20250422.affinity.33.0.19" # Suggest new cache version
DEFAULT_TAXONOMY_DIR = "./datasources/"
DEFAULT_CACHE_DIR = "./cache_v33/"
DEFAULT_CONFIG_FILE = "./affinity_config_v33.0.json" # Ensure this has correct base settings if used
OUTPUT_FILE_TEMPLATE = "affinity_definitions_{cache_version}.json"
LOG_FILE_TEMPLATE = "affinity_generation_{cache_version}.log"

# --- Core Parameters for Run 7 ---
COMBINED_SCORE_ALPHA = 0.6 # Key change for this run
ABSOLUTE_MIN_SBERT_SCORE = 0.15 # Keep from previous runs
DAMPENING_SBERT_THRESHOLD = 0.50 # Keep from previous runs (Explicitly set)
DAMPENING_FACTOR = 0.25 # Keep from previous runs (Explicitly set)
ENABLE_KW_EXPANSION = True # Keep default expansion logic (except Aikido)

# --- Other Tunable Parameters (Load overrides from config file) ---
MAX_CANDIDATES_FOR_LLM = 75
EVIDENCE_MIN_SIMILARITY = 0.30 # Initial SBERT filter before absolute minimum
KEYWORD_MIN_SCORE = 0.05 # Initial TF-IDF filter
...
MIN_KEYWORD_CANDIDATES_FOR_EXPANSION = 5 # Threshold to trigger expansion
KEYWORD_TOP_N_SEARCH = 500 # How many TF-IDF candidates to consider initially
...
def build_keyword_index(
    ...
    Includes logging for specific URI document content during build.
    """
    ...
    # --- START: Define URIs to log document content for TF-IDF ---
    uris_to_log_tfidf_doc = {
        # Airconditioning related
        "urn:expediagroup:taxonomies:acs:#1f3da634-0df6-4498-a8d9-603f895c8f3f", # AirConditioning:Unavailable (ACS Template)
        "urn:expediagroup:taxonomies:acsPCS:#AirConditioning",                 # AIR_CONDITIONING (PCS)
        "urn:expediagroup:taxonomies:lcm:#25b3661e-3640-3195-8907-6b06e3a21051", # Air conditioning (LCM) - *Confirm this URI if possible*
        # Allinclusive related
        "urn:expediagroup:taxonomies:lcm:#34528832-c2d8-312a-b52f-b27c483e5ec1", # All-inclusive resort (LCM) - *Confirm this URI if possible*
        ...
        "urn:expediagroup:taxonomies:activities:#fa6bdce9-7958-44bc-a5c2-0cf54d652dd6", # Transport included (Activities)
        # Aikido seeds (for reference)
        ...
        if not cache_valid:
            ...
            try:
                ...
                for uri, data in tqdm(sorted(taxonomy_concepts_cache.items()), desc="Prepare TF-IDF Docs"):
                    ...
                    # --- START: Log document content for specific URIs ---
                    if uri in uris_to_log_tfidf_doc:
                        label = get_primary_label(uri, taxonomy_concepts_cache, fallback=uri)
                        logger.debug(f"[TF-IDF DOC LOG] URI: {uri} Label: '{label}'")
                        logger.debug(f"[TF-IDF DOC LOG] Raw Text: '{raw_doc_text}'")
                        logger.debug(f"[TF-IDF DOC LOG] Normalized Text: '{norm_doc}'")
                    # --- END: Log document content ---
...
# --- Updated Stage 1: Evidence Preparation ---
# --- v33.0.22 - Run 14: Aircon Filter Override, Re-enable Expansion, Strict Damp ---
def prepare_evidence(
    ...
    """
    Prepares evidence candidates using combined score (Alpha=0.6), absolute SBERT filtering,
    conditional TF-IDF dampening (Threshold=0.35, Factor=0.15), specific seeding/no-expansion for 'aikido',
    applies SBERT boost to aikido seeds, and FILTERS OUT the 'AirConditioning:Unavailable' URI.
    Expansion is RE-ENABLED for aircon/allinc.
    v33.0.22 - Testing Aircon Filter Override, Re-enabled Expansion, DampThresh=0.35, DampFactor=0.15. Unigrams. Retains Aikido handling & Alpha=0.6.
    ...
    concepts_to_skip_expansion = {'aikido'} # REMOVED aircon & allinc

    # Use Dampening params from Run 13
    dampening_sbert_threshold = 0.35
    dampening_factor = 0.15

    # URI to explicitly filter out for 'airconditioning'
    aircon_unavailable_uri = "urn:expediagroup:taxonomies:acs:#1f3da634-0df6-4498-a8d9-603f895c8f3f"

    logger.info(f"Starting evidence prep for: '{normalized_concept}' (Alpha=0.6, Filters, DampThresh={dampening_sbert_threshold}, DampFactor={dampening_factor}, Specific NoExp, Aikido Seed/Boost, Aircon Filter)") # Log updated params
    ...
    alpha = 0.6
    abs_min_sbert = ABSOLUTE_MIN_SBERT_SCORE

    # Config checks (keep as before)
    # ... (keep checks for alpha, damp_thresh, damp_factor) ...
    ...
    logger.info(f"Using Combined Score Alpha: {alpha}")
    ...
    logger.info(f"Aikido Seed Boost enabled (Threshold: 0.80)")
    logger.info(f"LLM Keyword Expansion DISABLED only for: {concepts_to_skip_expansion}")
    if normalized_concept == 'airconditioning':
         logger.info(f"FILTERING OUT URI: {aircon_unavailable_uri}")
    ...
    # Expansion Check (Reverted to only skip aikido)
    ...
    if needs_exp:
        ...
        logger.info(f"Attempting LLM keyword expansion for '{normalized_concept}' (Trigger: initial matches {initial_kw_count} < {kw_trigger} or abstract concept, and NOT in skip list).")
        try:
            expanded_terms_list = expand_keywords_with_llm(input_concept, config, args)
            # ... (rest of expansion logic as before) ...
        ...
        if kw_exp_enabled and normalized_concept in concepts_to_skip_expansion: # Only Aikido now
            ...
            exp_diag["error"] = f"Skipped for '{normalized_concept}' (in skip list)"
             ...
             logger.info(f"Skipping LLM KW expansion for '{normalized_concept}' (Initial matches {initial_kw_count} >= {kw_trigger} and not abstract).")
    ...
    # Define Aikido seed URIs
    seed_uris = {
        "urn:expediagroup:taxonomies:activities:#c7b903d8-14be-4bcd-bcb3-d6d35724e7cc", # Martial Arts classes
        "urn:expediagroup:taxonomies:core:#f02b51fe-eabe-4dab-961a-456bed9664e8"      # Martial Arts
    }

    # Aikido Seeding Logic
    ...
    # --- START: Explicitly filter out AirConditioning:Unavailable ---
    if normalized_concept == 'airconditioning':
        if aircon_unavailable_uri in all_uris_set:
            logger.info(f"Applying specific filter: Removing {aircon_unavailable_uri} for '{normalized_concept}'.")
            all_uris_set.remove(aircon_unavailable_uri)
        else:
            logger.debug(f"Specific filter URI {aircon_unavailable_uri} not found in candidate set for '{normalized_concept}'.")
    # --- END: Filter ---

    all_uris_list = list(all_uris_set)
    unique_candidates_before_ranking = len(all_uris_list)
    if initial_unique_count != unique_candidates_before_ranking: # Will likely differ now due to filter
        logger.info(f"Total unique candidates changed from {initial_unique_count} to {unique_candidates_before_ranking} after seeding/filtering.")
    ...
    # --- Scoring Loop (Dampening threshold=0.35, factor=0.15) ---
    for uri in all_uris_list:
        # ... (SBERT score fetch, Aikido boost logic remains the same) ...
        ...
        if uri not in sim_scores and is_aikido_seed_uri:
            if uri in primary_embeddings and concept_embedding is not None:
                 ...
                 if args.debug:
                     logger.debug(f"[Aikido Seed] Calculated SBERT score for seeded URI {uri} ('{pref_label_for_log}'): {s_score:.6f}")
            else:
                 logger.warning(f"Could not calculate SBERT for seeded URI {uri}")
                 s_score = 0.0
        elif args.debug and is_aikido_seed_uri and not calculated_seed_sbert:
             logger.debug(f"[Aikido Seed] Found SBERT score for seed URI {uri} ('{pref_label_for_log}') in initial batch: {s_score:.6f}")
        ...
        if is_aikido_seed_uri:
            ...
            if s_score < boost_threshold:
                ...
                boost_applied = True
                logger.info(f"[Aikido Seed BOOST] Boosting SBERT score for {uri} ('{pref_label_for_log}') from {original_s_score_before_boost:.4f} to {boosted_s_score_for_calc:.4f}")
            elif args.debug:
                 logger.debug(f"[Aikido Seed BOOST] Seed URI {uri} ('{pref_label_for_log}') SBERT score {s_score:.4f} already >= boost threshold {boost_threshold:.4f}. No boost needed.")

        if args.debug and is_aikido_seed_uri:
             logger.debug(f"[Aikido Seed] Checking Filter for URI {uri} ('{pref_label_for_log}'): SBERT={boosted_s_score_for_calc:.6f} (Boost Applied: {boost_applied}) vs AbsMin={abs_min_sbert:.4f}")

        # ** Absolute SBERT Filter ** (Uses potentially boosted score for Aikido seeds)
        if boosted_s_score_for_calc < abs_min_sbert:
            if args.debug:
                 filter_reason = f"due to SBERT score {boosted_s_score_for_calc:.4f} < {abs_min_sbert:.4f}"
                 if is_aikido_seed_uri:
                     logger.warning(f"[Aikido Seed] FILTERED seed URI {uri} ('{pref_label_for_log}') {filter_reason}. (TFIDF: {k_score:.4f})")
                 else:
                     logger.debug(f"Excluding candidate {uri} ('{pref_label_for_log}') {filter_reason}. (TFIDF: {k_score:.4f})")
            abs_sbert_filtered_count += 1
            continue

        # *** Conditional TF-IDF Dampening *** (Condition uses potentially boosted score, Factor=0.15, Threshold=0.35)
        dampened_k_score = k_score
        was_dampened = False
        # Use boosted_s_score_for_calc for the condition check
        if boosted_s_score_for_calc < dampening_sbert_threshold: # Now Threshold=0.35
            dampened_k_score *= dampening_factor # Apply the stronger 0.15 factor
            if k_score > 0:
                 was_dampened = True
                 if args.debug: logger.debug(f"Dampening TF-IDF for {uri} ('{pref_label_for_log}'): SBERT={boosted_s_score_for_calc:.4f} < {dampening_sbert_threshold:.4f}. TF-IDF reduced from {k_score:.4f} to {dampened_k_score:.4f} (Factor={dampening_factor}).")
                 dampened_count += 1

        # Normalize scores
        norm_s = max(0.0, min(1.0, boosted_s_score_for_calc)) # Use potentially boosted score for ranking
        norm_k = max(0.0, min(1.0, dampened_k_score))        # Use potentially dampened score for ranking

        # Calculate combined score (Using ALPHA=0.6)
        combined = (alpha * norm_k) + ((1.0 - alpha) * norm_s)

        scored_list.append({
            "uri": uri,
            "details": all_details[uri],
            "sim_score": original_s_score_before_boost, # Store original SBERT score
            "boosted_sim_score": boosted_s_score_for_calc if boost_applied else None,
            "kw_score": k_score,           # Raw TFIDF
            "dampened_kw_score": dampened_k_score if was_dampened else None, # Dampened TFIDF (only if different)
            "combined_score": combined     # Final score used for sorting
        })


    logger.info(f"Excluded {abs_sbert_filtered_count} candidates with SBERT score < {abs_min_sbert}.")
    logger.info(f"Conditionally dampened TF-IDF score (Factor={dampening_factor}) for {dampened_count} candidates with SBERT score < {dampening_sbert_threshold}.")
    ...
    if args.debug:
        logger.debug(f"--- Top 5 Candidates for '{normalized_concept}' AFTER Sorting (Alpha={alpha}, DampThresh={dampening_sbert_threshold}, DampFactor={dampening_factor}) ---")
        for i, c in enumerate(scored_list[:5]):
            damp_info = f"(Dampened TFIDF: {c.get('dampened_kw_score'):.4f})" if c.get('dampened_kw_score') is not None else ""
            boost_info = f"(Boosted SBERT: {c.get('boosted_sim_score'):.4f})" if c.get('boosted_sim_score') is not None else ""
            label = c.get('details', {}).get('prefLabel', 'N/A')
            logger.debug(f"{i+1}. URI: {c.get('uri', 'MISSING')} Label: '{label}' "
                         f"Combined: {c.get('combined_score', 0.0):.6f} "
                         f"(Orig SBERT: {c.get('sim_score', 0.0):.4f} {boost_info}, TFIDF: {c.get('kw_score', 0.0):.4f} {damp_info})")
        logger.debug("--- End Top 5 ---")


    selected = scored_list[:max_cands]
    logger.info(f"Selected top {len(selected)}/{len(scored_list)} candidates based on combined score (alpha={alpha}) after filtering/dampening.")
    ...
    for c in selected:
         ...
         if uri and details:
              orig_map_entry = {
                  **details,
                  "sbert_score": c.get("sim_score", 0.0), # Original score
             ...
             logger.warning(f"Skipping candidate in orig_map due to missing URI or details: {c}")
    ...
    if anchor_uri_from_data:
        ...
        logger.error(f"Top candidate in 'selected' list has no URI! Data: {anchor_data}")
    ...
    if args.debug:
        if anchor and 'uri' in anchor:
            if anchor['uri'] == anchor_uri_from_data:
                logger.debug(f"DEBUG SANITY CHECK: Anchor URI lookup successful. Top Scored URI ({anchor_uri_from_data}) matches Looked-up Anchor URI ({anchor['uri']})")
            else:
                logger.error(f"DEBUG SANITY CHECK: MISMATCH! Anchor URI lookup returned wrong item. Top Scored URI ({anchor_uri_from_data}) != Looked-up Anchor URI ({anchor['uri']})")
        elif anchor_uri_from_data:
            logger.error(f"DEBUG SANITY CHECK: Anchor URI lookup FAILED. Top Scored URI ({anchor_uri_from_data}) not found as key in orig_map.")
        else:
             logger.error(f"DEBUG SANITY CHECK: Cannot perform check because Top Scored URI is missing.")

    # --- Anchor Logging ---
    if anchor:
        damp_info_log = f"(Dampened TFIDF: {anchor_data.get('dampened_kw_score'):.4f})" if anchor_data.get('dampened_kw_score') is not None else ""
        boost_info_log = f"(Boosted SBERT: {anchor_data.get('boosted_sim_score'):.4f})" if anchor_data.get('boosted_sim_score') is not None else ""
        logger.info(f"Anchor selected: {anchor.get('prefLabel', anchor.get('uri', 'MISSING URI'))} "
                    f"(Combined: {anchor_data.get('combined_score', 0.0):.6f}, "
                    f"Orig SBERT: {anchor_data.get('sim_score', 0.0):.4f} {boost_info_log}, "
                    f"TFIDF: {anchor_data.get('kw_score', 0.0):.4f} {damp_info_log})")
    else:
        logger.warning(f"Could not determine anchor details for '{normalized_concept}'. Anchor will be null.")
        if anchor_uri_from_data:
             logger.warning(f"    Reason: URI '{anchor_uri_from_data}' (from top candidate) was not found in orig_map keys.")
             if args.debug: logger.debug(f"    orig_map keys: {list(orig_map.keys())[:20]}...")
        else:
...
# --- Stage 2: Finalization (No changes needed here for Alpha test) ---
...
def generate_affinity_definitions_loop(
    ...
    print(">>> DEBUG: generate_affinity_definitions_loop entered.")
    all_definitions = []
    ...
    print(f">>> DEBUG: Subset size for loop: {len(concepts_subset)}")
    if not concepts_subset: print(">>> DEBUG: concepts_subset is EMPTY!"); return []

    disable_tqdm = not logger.isEnabledFor(logging.INFO) or args.debug
    print(f">>> DEBUG: Starting concept iteration...")

    for concept in tqdm(concepts_subset, desc="Processing Concepts", disable=disable_tqdm):
        print(f"\n>>> DEBUG: START LOOP ITERATION for concept: '{concept}'")
        ...
        affinity_def = {
            ...
            "diagnostics": {
                "stage1": {
                    "status": "Not Started", "error": None, "selection_method": "Combined TFIDF+SBERT",
                    "expansion": {}, "sbert_candidate_count_initial": 0, "tfidf_candidate_count_initial": 0,
                    "unique_candidates_before_ranking": 0, "llm_candidate_count": 0
                },
        ...
        try:
            ...
            if concept_emb is None:
                logger.error(f"Embedding failed for '{norm_concept}'."); diag1["error"] = "Embedding failed"; diag1["status"] = "Failed"
            else: logger.debug(f"Successfully generated embedding for '{norm_concept}'.")
            ...
            stage2_out = apply_rules_and_finalize(
                ...
                orig_map, sbert_scores, args, _taxonomy_concepts_cache
            )
            ...
            if final_status == "Started":
                ...
                elif diag1["status"] == "Failed": final_status = "Failed - Stage 1 Error"
            ...
            print(f">>> DEBUG: END LOOP ITERATION for concept: '{concept}'")

    print(f">>> DEBUG: Exiting generate_affinity_definitions_loop.")
...
def main():
    ...
    # Use script constants first, allow config override if needed
    ...
    logger.info(f"--- Key Parameters for Run ---")
    ...
    logger.info(f"Aikido Specific Handling: Seeded = YES, Expansion Disabled = YES")
    ...
    try:
        ...
        logger.info(f"SBERT model load successful. Dimension: {sbert_model.get_sentence_embedding_dimension()}")
    ...
    print(f">>> DEBUG: Returned from loop. Result count: {len(results)}")
