...
# -*- coding: utf-8 -*-
"""
Generate affinity definitions for travel concepts using combined scoring (v33.0.25 - Concept-Specific Alpha).
...
Changes from v33.0.24:
- Removed Dynamic Alpha logic.
- Added Concept-Specific Alpha override for 'airconditioning'.
- Updated logging to reflect the new alpha mechanism.
"""
...
SCRIPT_VERSION = "affinity-rule-engine-v33.0.25 (Concept-Specific Alpha)" # Updated version
DEFAULT_CACHE_VERSION = "v20250420.affinity.33.0.25" # Keep base cache version unless TFIDF params change
...
ABSOLUTE_MIN_SBERT_SCORE = 0.15 # Retained from previous run
DAMPENING_SBERT_THRESHOLD = 0.35 # Retained from previous run
DAMPENING_FACTOR = 0.15 # Retained from previous run
...
KEYWORD_MIN_SCORE = 0.05 # Increased from 0.01 in the provided context, using the value from v33.0.24 script
...
# (build_keyword_index, get_candidate_concepts_keyword unchanged from v33.0.22)
            ...
            try:
                ...
                for uri, data in tqdm(sorted(taxonomy_concepts_cache.items()), desc="Prepare TF-IDF Docs"):
                    ...
                    if uri in uris_to_log_tfidf_doc:
                        ...
                        logger.debug(f"[TF-IDF DOC LOG] URI: {uri} Label: '{label}' Raw: '{raw_doc_text}' Norm: '{norm_doc}'")
...
# --- START: Updated Stage 1: Evidence Preparation (v33.0.25) ---
# --- v33.0.25 - Run 16: Concept-Specific Alpha Override (Aircon=0.05), Specific Filter, No Expansion, Manual Split, DampThresh=0.35, DampFactor=0.15 ---
def prepare_evidence(
    ...
    v33.0.25 - Testing Concept-Specific Alpha Override (Aircon=0.05). Removes general dynamic alpha.
    Retains Specific Aircon Filter, NoExp, Manual Split, DampThresh=0.35, DampFactor=0.15.
    ...
    # Ensure float conversion and provide defaults matching previous run's constants
    ...
    max_cands = int(stage1_cfg.get('MAX_CANDIDATES_FOR_LLM', 75)) # Use value from v33.0.24 script
    min_sim = float(stage1_cfg.get('EVIDENCE_MIN_SIMILARITY', 0.30)) # Use value from v33.0.24 script
    min_kw = float(stage1_cfg.get('KEYWORD_MIN_SCORE', 0.05)) # Use value from v33.0.24 script
    kw_trigger = int(stage1_cfg.get('MIN_KEYWORD_CANDIDATES_FOR_EXPANSION', 5)) # Use value from v33.0.24 script
    kw_top_n = int(stage1_cfg.get('KEYWORD_TOP_N', 500)) # Use value from v33.0.24 script
    abs_min_sbert = float(stage1_cfg.get("ABSOLUTE_MIN_SBERT_SCORE", 0.15)) # Use value from v33.0.24 script
    alpha = float(config.get("COMBINED_SCORE_ALPHA", 0.6)) # Default alpha
    kw_exp_enabled = config.get("ENABLE_KW_EXPANSION", True) and args.llm_provider != "none" # Use value from v33.0.24 script
    # --- END: Get parameters ---

    # --- Parameter Logging Block ---
    logger.info(f"[{normalized_concept}] Using Default Combined Score Alpha: {alpha}")
    logger.info(f"[{normalized_concept}] Applying Concept-Specific Alpha Overrides (e.g., Aircon=0.05)") # Note specific override
    ...
    if needs_exp:
        ...
        try:
            # Assuming expand_keywords_with_llm is defined elsewhere in this script
            ...
            if exp_diag["successful"]:
                 ...
                 logger.info(f"[{normalized_concept}] LLM expansion added terms: {newly_expanded_kws}") # Added log
            else:
    ...
    if tfidf_vectorizer and tfidf_matrix is not None and tfidf_corpus_uris:
        if final_query_texts:
            # Assuming get_candidate_concepts_keyword is defined elsewhere in this script
    ...
    if concept_embedding is None:
        ...
        # Original script's method: calculate for all primary embeddings
    ...
    if normalized_concept == 'aikido':
        ...
        if added_seeds:
            ...
            # Calculate SBERT for newly added seeds if not already done (Important!)
    ...
    if uris_to_filter_for_concept:
        original_count = len(all_uris_set) # Check count before filtering
        uris_to_remove = set()
        for uri_to_filter in uris_to_filter_for_concept:
            if uri_to_filter in all_uris_set:
                # Assuming get_primary_label is defined elsewhere
    ...
    # Log change only if not due to explicit filter
    ...
    # Assuming get_kg_data is defined elsewhere
    ...
    concept_alpha_overrides = {
        'airconditioning': 0.05, # Rely almost entirely on SBERT
        # 'allinclusive': 0.0 # Example: Could also force pure SBERT if needed
    }
    # --- END: Concept-Specific Alpha Overrides ---


    for i, uri in enumerate(all_uris_list): # Added enumerate for potential first-pass logging
        if uri not in all_details:
            logger.warning(f"[{normalized_concept}] Details not found for candidate URI: {uri}. Skipping.")
            continue

        # Ensure scores are present, default to 0.0 if missing (e.g., seed added late)
        ...
        # Apply filter *after* potential boost
        ...
        # Apply dampening based on *boosted* SBERT score
        dampened_k_score = k_score
        was_dampened = False
        if boosted_s_score_for_calc < dampening_sbert_threshold:
            dampened_k_score *= dampening_factor
            # Check if dampening actually changed the value (avoid logging no-ops)
        ...
        effective_alpha = alpha # Start with default alpha
        ...
        if normalized_concept in concept_alpha_overrides:
            ...
            if abs(new_alpha - effective_alpha) > 1e-9: # Check if override is meaningfully different
                 if args.debug and i == 0: # Log only once per concept on first candidate check
                 ...
                 if i == 0: # Only set the concept-level flag once
                     alpha_override_applied = True

        # Normalize scores
        norm_s = max(0.0, min(1.0, boosted_s_score_for_calc)) # Use boosted score for calc
        norm_k = max(0.0, min(1.0, dampened_k_score))         # Use dampened score for calc
        ...
        scored_list.append({
            ...
            "sim_score": original_s_score_before_boost, # Store original SBERT
            "boosted_sim_score": boosted_s_score_for_calc if boost_applied else None,
            "kw_score": k_score, # Store original TFIDF
            "dampened_kw_score": dampened_k_score if was_dampened else None,
            "alpha_overridden": alpha_was_overridden, # Flag if alpha for *this* URI was non-default
            "effective_alpha": effective_alpha, # Alpha used in calculation
    ...
    if alpha_override_applied:
         ...
         # Ensure override_val is float before formatting if it's not 'N/A'
    ...
    if args.debug:
        logger.debug(f"--- Top 5 Candidates for '{normalized_concept}' AFTER Sorting (DefaultAlpha={alpha}, ConceptOverrides={concept_alpha_overrides.get(normalized_concept,'None')}, DampThresh={dampening_sbert_threshold}, DampFactor={dampening_factor}) ---")
        for i, c in enumerate(scored_list[:5]):
            ...
            alpha_info = f"(EffAlpha: {c.get('effective_alpha', alpha):.2f})" # Changed label for clarity
    ...
    for c in selected:
         ...
         if uri and details:
              orig_map_entry = {
                  **details,
                  "sbert_score": c.get("sim_score", 0.0), # Original SBERT
                  "keyword_score": c.get("kw_score", 0.0), # Original Keyword
                  "combined_score": c.get("combined_score", 0.0),
                  "effective_alpha": c.get("effective_alpha", alpha) # Effective alpha used
              }
    ...
    anchor_data = selected[0] # Top ranked candidate after sorting
    anchor_uri_from_data = anchor_data.get('uri')
    anchor = None
    if anchor_uri_from_data:
        # Retrieve the full data we stored in orig_map using the URI
        ...
        if anchor is None:
             ...
             # As a fallback, try to construct a minimal anchor from anchor_data if possible
    ...
    # Debug Sanity Check (Same as before, safe to keep)
    if args.debug:
    ...
    if anchor:
        ...
        alpha_log = f"(EffAlpha: {anchor_data.get('effective_alpha', alpha):.2f})" # Changed label
    ...
    # Return values
    sbert_scores_final = {uri: d["sbert_score"] for uri, d in orig_map.items()} # Original SBERT scores
...
# --- END: Updated Stage 1: Evidence Preparation (v33.0.25) ---
...
def apply_rules_and_finalize(
    ...
    candidate_evidence_scores: Dict[str, float], # Note: Unused here
...
# (generate_affinity_definitions_loop unchanged from v33.0.22)
def generate_affinity_definitions_loop(
    ...
    for concept in tqdm(concepts_subset, desc="Processing Concepts", disable=disable_tqdm):
        ...
        try:
            # *** CORRECTED CALL to get_concept_embedding ***
...
# --- Main Execution ---
# (main function unchanged from v33.0.22/v33.0.24)
def main():
    ...
    # Updated Log for Alpha Mechanism
    ...
    # Make sure precompute_taxonomy_embeddings uses the loaded sbert_model
    ...
    # Ensure get_concept_embedding function uses the loaded sbert_model correctly if it's not passed explicitly
